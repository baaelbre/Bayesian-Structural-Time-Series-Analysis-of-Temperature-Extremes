...
Version 2.5.0 keeps one model compiler, one fitting entry point, and one result
...
`PresentationConfig` resolves a runtime profile and window. `WorkflowPaths`
maps each scientific stage and HPC chain to a deterministic filename.
`PresentationWorkflow` calls ordinary fit/data/diagnostic APIs, saves archives,
exports tidy tables, and updates a locked manifest. `report()` consumes only
saved fits.
...
Schema 2.5.0 archives contain allowlisted JSON metadata plus compressed NumPy
...
exports one compatible multiseries draw without changing the next fit's target.
