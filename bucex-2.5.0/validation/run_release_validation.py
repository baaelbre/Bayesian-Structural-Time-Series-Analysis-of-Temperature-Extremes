#!/usr/bin/env python3
"""Fixed-seed numerical release validation for bucex 2.5.0."""
...
def run() -> dict[str, object]:
    ...
    if bx.__version__ != "2.5.0":
        raise RuntimeError(f"Expected bucex 2.5.0, found {bx.__version__}.")
    default_hierarchy = bx.HierarchicalPrior()
    if default_hierarchy.model_space != "componentwise":
        raise RuntimeError("The 2.5.0 hierarchy must default to componentwise SSVS.")
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("validation/release_validation_2.5.0.json"),
    )
