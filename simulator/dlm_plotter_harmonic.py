...
def _find_latest_run(root: str) -> Optional[str]:
    """Return path to latest posterior.npz under root (depth 2)."""
    best = None
...
def load_posterior(target_or_dir: str) -> Tuple[Dict[str, Any], Dict[str, Any], str]:
    ...
    if os.path.isdir(target_or_dir):
        # try exact file then search within
...
class DLMHarmonicPlotter:
    ...
    def _fig_overview(self, save_dir: Optional[str], show: bool) -> Optional[str]:
        ...
        # m0 subset
        ax = axs[4]
        m0_keys = [("m0_alpha", "m0_α"), ("m0_beta", "m0_β")]
        plotted = False
        for k, title in m0_keys:
            if k in self.d:
        ...
        # P0 subset
        ax = axs[5]
        P0_keys = [("P0_alpha", "P0_α"), ("P0_beta", "P0_β")]
        plotted = False
        for k, title in P0_keys:
            if k in self.d:
                s = np.asarray(self.d[k]).ravel()
                ax.hist(s, bins=40, density=True, alpha=0.55, label=title)
                plotted = True
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="DLM Harmonic Plotter (compact, truth‑aware)",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--target", type=str, default=None, help="Run dir or posterior.npz. If omitted, search --root.")
    p.add_argument("--root", type=str, default="results/simulations/DLM_harm", help="Search root when --target omitted.")
