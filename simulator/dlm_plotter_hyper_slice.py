...
"""
DLM Plotter — concise, truth-aware (no separate resolver)
...
Assumes posterior bundle (npz) may include:
  ...
  true_m0_alpha_det, true_m0_beta_det, true_season_det.
...
def _geweke(x: np.ndarray, a: float = .1, b: float = .5) -> float:
...
def _layout_idxs(meta: Dict[str, Any], period: int) -> Dict[str, Optional[int]]:
    ...
    try:
        ...
        if gnames:
            ...
            if last in lay: idx_g_end = lay.index(last)
    except Exception:
        pass
...
def _to_Q(draws: Dict[str, Any], w: str) -> Optional[np.ndarray]:
    ...
    if f"s_{w}" in draws: 
        s = np.asarray(draws[f"s_{w}"]).ravel(); return s*s
    return None

def _uniq_legend(ax):
    h,l = ax.get_legend_handles_labels()
    if l: 
        u = dict(zip(l,h)); ax.legend(u.values(), u.keys(), fontsize=8, loc="best")
...
def _truth_sigma(draws: Dict[str, Any]) -> Optional[float]:
    v = _maybe(draws, "true_sigma"); return None if v is None else float(v)
...
def _truth_Q(draws: Dict[str, Any], comp: str, idxs: Dict[str, Optional[int]]) -> Optional[float]:
    ...
    if j is None:
        if QQ.ndim == 1: j = {"alpha":0, "beta": (1 if QQ.size>=2 else 0), "gamma":-1}[comp]
        else:            j = {"alpha":0, "beta": (1 if QQ.shape[0]>=2 else 0), "gamma":-1}[comp]
    try:
...
def _truth_m0(draws: Dict[str, Any], name: str) -> Optional[float]:
    ...
    if m:
        ...
        j = int(m.group(1))
        if 0 <= j < v.size: return float(v[j])
    return None
...
def _truth_det(draws: Dict[str, Any], name: str) -> Optional[float]:
    if name == "α_det": return _maybe(draws, "true_m0_alpha_det")
    if name == "β_det": return _maybe(draws, "true_m0_beta_det")
    m = re.fullmatch(r"season_det\[(\d+)\]", name)
    if m:
        v = _maybe(draws, "true_season_det")
        if v is None: return None
        v = np.asarray(v, float).ravel(); j = int(m.group(1))
        if 0 <= j < v.size: return float(v[j])
    return None
...
class DLMPlotter:
    def __init__(self, draws: Dict[str, np.ndarray], meta: Dict[str, Any], level: float = 0.90):
        ...
        if not (0 < self.level < 1): raise ValueError("level in (0,1)")
        self.T = int(meta.get("T") or draws["mu"].shape[1]); self.period = int(meta.get("period", 12))
        ...
        self.sigma = (np.asarray(draws["sigma"], float).ravel() if "sigma" in draws
                      else (np.sqrt(np.clip(np.asarray(draws["sigma2"], float),0,None)).ravel() if "sigma2" in draws else None))
    ...
    def _fig_traces(self, fam, items, outdir, show, L):
        ...
        R = len(items); fig,axs = plt.subplots(R,2, figsize=(12,3.0*R), squeeze=False)
        for r,(name, s) in enumerate(items):
            s = np.asarray(s).ravel(); ac=_acf(s,L); ess=_ess(s,L); gz=_geweke(s)
            axs[r,0].plot(s, lw=1); axs[r,0].set_title(f"{name} (trace)"); axs[r,0].set_xlabel("iter")
            axs[r,1].bar(np.arange(ac.size), ac, width=0.9); axs[r,1].set_xlim(-0.5, ac.size-0.5)
            axs[r,1].set_title(f"{name} (ACF, ESS≈{ess:.0f}, z≈{gz:.2f})"); axs[r,1].set_xlabel("lag")
        fig.suptitle(f"Trace + ACF — {fam}", y=0.995); fig.tight_layout(rect=[0,0,1,0.98])
        path=None
        if outdir: _ensure_dir(outdir); path=os.path.join(outdir, f"traces_acf__{_san(fam)}.png"); fig.savefig(path, dpi=200, bbox_inches="tight"); print(f"[save] {path}")
        plt.show() if show else plt.close(fig); return path

    def _fig_posts(self, fam, items, outdir, show):
        if not items: return None
        C = 2 if len(items)>=4 else 1; R = int(np.ceil(len(items)/C))
        fig,axs = plt.subplots(R,C, figsize=(6*C+1, 2.8*R), squeeze=False)
        for k,(name, s) in enumerate(items):
            r,c = divmod(k,C); ax=axs[r,c]; s=np.asarray(s).ravel()
            ...
            elif name.startswith("m0_"): tv=_truth_m0(self.d, name)
            elif name.startswith("P0_"): tv=_truth_P0(self.d, name)
            elif name in {"α_det","β_det"} or name.startswith("season_det"): tv=_truth_det(self.d, name)
            if tv is not None and np.isfinite(tv): ax.axvline(float(tv), color="k", lw=1.6, ls="-", label="truth")
            ax.set_title(name); _uniq_legend(ax)
        for k in range(len(items), R*C): r,c=divmod(k,C); axs[r,c].axis("off")
        fig.suptitle(f"Posteriors — {fam}", y=0.995); fig.tight_layout(rect=[0,0,1,0.98])
        path=None
        if outdir: _ensure_dir(outdir); path=os.path.join(outdir, f"posteriors__{_san(fam)}.png"); fig.savefig(path, dpi=200, bbox_inches="tight"); print(f"[save] {path}")
        plt.show() if show else plt.close(fig); return path

    # overview
    def figure_overview(self, save_dir: Optional[str]=None, fname_prefix="overview", show=True):
        mu = np.asarray(self.d["mu"], float); ctr,lo,hi = _qtiles(mu, self.level)
        fig,axs = plt.subplots(2,3, figsize=(13,8)); axs=axs.ravel(); t=np.arange(self.T)
        axs[0].plot(ctr,lw=1.6,label="μ median"); axs[0].fill_between(t,lo,hi,alpha=.25,label=self.band)
        if self.y is not None and len(self.y)==self.T: axs[0].plot(self.y,lw=1.0,alpha=.6,label="y")
        ...
        if self.sigma is not None:
            ...
            axs[2].hist(self.sigma,bins=40,density=True); es=_ess(self.sigma); gz=_geweke(self.sigma)
        ...
        else: axs[1].axis("off"); axs[2].axis("off")
        ax=axs[3]; plotted=False
        for Q,label in ((self.Qa,"α"),(self.Qb,"β"),(self.Qg,"γ")):
            if Q is not None: ax.hist(np.log10(np.clip(Q,1e-20,None)),bins=40,density=True,alpha=.55,label=f"log10 Q[{label}]"); plotted=True
        ...
        lamA,lamB,lamG = _maybe(self.d,"lambda_alpha"), _maybe(self.d,"lambda_beta"), _maybe(self.d,"lambda_gamma")
        if any(v is not None for v in (lamA,lamB,lamG)):
            ax=axs[4]
            if lamA is not None: ax.hist(np.asarray(lamA).ravel(),bins=40,density=True,alpha=.6,label="λ_α")
            if lamB is not None: ax.hist(np.asarray(lamB).ravel(),bins=40,density=True,alpha=.6,label="λ_β")
            if lamG is not None: ax.hist(np.asarray(lamG).ravel(),bins=40,density=True,alpha=.6,label="λ_γ")
            ax.set_title("PC rate posteriors λ"); _uniq_legend(ax)
        else: axs[4].axis("off")
        if self.t_mu is not None and len(self.t_mu)==self.T:
            err = np.mean((mu - self.t_mu.reshape(1,-1))**2, axis=1)**0.5
            axs[5].plot(np.cumsum(err)/np.arange(1,err.size+1), lw=1.2); axs[5].set_title("running RMSE(μ)"); axs[5].set_xlabel("kept draw")
        else: axs[5].axis("off")
        fig.tight_layout()
        if save_dir: _ensure_dir(save_dir); p=os.path.join(save_dir,f"{fname_prefix}.png"); fig.savefig(p,dpi=200,bbox_inches="tight"); print(f"[save] {p}")
    ...
    def figure_states(self, save_dir: Optional[str]=None, fname_prefix="states", show=True):
        ...
        fig,axes = plt.subplots(rows,1, figsize=(12,3.0*rows), sharex=True); axes = axes if isinstance(axes,np.ndarray) else np.array([axes])
        t=np.arange(self.T); r=0
        mu = np.asarray(self.d["mu"], float); c,lo,hi=_qtiles(mu,self.level); ax=axes[r]
        if self.y is not None and len(self.y)==self.T: ax.plot(self.y,lw=1.0,alpha=.6,label="y")
        ax.plot(c,lw=1.6,label="μ median"); ax.fill_between(t,lo,hi,alpha=.25,label=self.band)
        if self.t_mu is not None and len(self.t_mu)==self.T: ax.plot(self.t_mu,lw=1.2,ls="--",label="true μ")
        ax.set_title("Posterior μ_t"); ax.legend(); r+=1
        for arr,lab,truth in ((A,"α",self.t_a),(B,"β",self.t_b),(G,"γ(last)",self.t_g)):
            if arr is None: continue
            c,lo,hi=_qtiles(arr,self.level); ax=axes[r]
            ax.plot(c,lw=1.6,label=f"{lab} median"); ax.fill_between(t,lo,hi,alpha=.25,label=self.band)
            if truth is not None and len(truth)==self.T: ax.plot(truth,lw=1.2,ls="--",label=f"true {lab.split('(')[0]}")
            ax.set_title(f"{'Level' if lab=='α' else ('Trend' if lab=='β' else 'Seasonal')} {lab}"); ax.legend(); r+=1
        fig.tight_layout()
        if save_dir: _ensure_dir(save_dir); p=os.path.join(save_dir,f"{fname_prefix}.png"); fig.savefig(p,dpi=200,bbox_inches="tight"); print(f"[save] {p}")
        plt.show() if show else plt.close(fig)
    ...
    def quick_report(self, save_dir: Optional[str]=None, fname_prefix="quick_report", show=True):
        ...
        axs[0].plot(c,lw=1.6,label="μ median"); axs[0].fill_between(t,lo,hi,alpha=.25,label=self.band)
        if self.t_mu is not None and len(self.t_mu)==self.T: axs[0].plot(self.t_mu,lw=1.2,ls="--",label="true μ")
        axs[0].set_title("μ_t"); axs[0].legend()
        if self.sigma is not None:
            axs[1].hist(self.sigma,bins=40,density=True,label="σ"); ts=_truth_sigma(self.d)
        ...
        for comp,series,tag in (("alpha",self.Qa,"Q_α"),("beta",self.Qb,"Q_β"),("gamma",self.Qg,"Q_γ")):
            ...
            axs[2].set_title(f"log10 {tag} | y"); _uniq_legend(axs[2]); done=True; break
        if not done: axs[2].axis("off")
        fig.tight_layout()
        if save_dir: _ensure_dir(save_dir); p=os.path.join(save_dir,f"{fname_prefix}.png"); fig.savefig(p,dpi=200,bbox_inches="tight"); print(f"[save] {p}")
        plt.show() if show else plt.close(fig)
...
if __name__ == "__main__":
    import argparse
    p = argparse.ArgumentParser(description="DLM plotter (concise, truth-aware)", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    p.add_argument("--skip-grouped-traces", action="store_true")
    p.add_argument("--skip-grouped-post",   action="store_true")
    p.add_argument("--max-lag", type=int, default=200, help="ACF/ESS max lag")
    a = p.parse_args()

    run = a.target or find_latest_run(root=a.root)
    if run is None:
        print(f"[error] no posterior.npz under {a.root!r}; provide --target or change --root."); sys.exit(1)
    bundle = load_posterior(run); draws, meta, npz_path = bundle.draws, bundle.meta, bundle.npz_path
    if "layout" in meta:
        meta = dict(meta); meta.update(_layout_idxs(meta, meta.get("period", 12)))
    out_dir = a.out or os.path.join(os.path.dirname(npz_path), "figures"); _ensure_dir(out_dir)
