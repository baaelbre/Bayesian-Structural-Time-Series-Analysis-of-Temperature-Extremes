...
"""
DLM Plotter — concise, truth-aware (m0/P0 included)
...
Seasonal convention (updated):
  • State is NEWEST-FIRST: [γ_t, γ_{t-1}, ..., γ_{t-(p-2)}]
  • Observation loads the FIRST seasonal coord (γ_t).
  • Process variance for season is on the FIRST seasonal coord.
"""
...
def _layout_idxs(meta: Dict[str, Any], period: int) -> Dict[str, Optional[int]]:
    """
    From meta["layout"], find indices of alpha, beta, the FIRST and LAST gamma components.
    For the newest-first convention we care about idx_g_first (γ_t).
    """
...
def _truth_Q(draws: Dict[str, Any], comp: str, idxs: Dict[str, Optional[int]]) -> Optional[float]:
    """
    Pull the true process variance for a component.
    For gamma we now use idx_g_first (γ_t).
    """
    ...
    # Fallbacks if layout indices are unavailable
    if j is None:
        if QQ.ndim == 1:
            j = {"alpha": 0, "beta": (1 if QQ.size >= 2 else 0), "gamma": 0}[comp]
        else:
...
def _truth_m0(draws: Dict[str, Any], name: str, period: int) -> Optional[float]:
    ...
    if m:
        ...
        if v is None:
            return None
        v = np.asarray(v, float).ravel()  # expected length p-1 (newest-first)
        j = int(m.group(1))
        if j < v.size:
            return float(v[j])
...
class DLMPlotter:
    ...
    def figure_states(self, save_dir: Optional[str] = None, fname_prefix="states", show=True):
        ...
        # NEW: plot FIRST seasonal coord (γ_t), not last
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(
        description="DLM plotter (concise, truth-aware; newest-first seasonal with γ_t = first coord)",
