...
"""
DLM Plotter — concise, truth-aware (now includes full m0/P0 plotting + spike&slab z)
-----------------------------------------------------------------------------------
...
Now also plots spike&slab model evidence indicators: z_alpha, z_beta, z_gamma.
"""
...
def _layout_idxs(meta: Dict[str, Any], period: int) -> Dict[str, Optional[int]]:
    """
    From meta["layout"], find indices of alpha, beta, and seasonal coords.
    Prefers g1 (newest-first) if present, else falls back to last g* for old runs.
    """
    idx_alpha = idx_beta = idx_g1 = idx_g_end = None
    ...
    try:
        ...
        if gnames:
            nums = [int(s[1:]) for s in gnames]
            first = f"g{min(nums)}"
            last = f"g{max(nums)}"
            if first in lay:
                idx_g1 = lay.index(first)
            if last in lay:
                idx_g_end = lay.index(last)
    except Exception:
        pass
    return {"idx_alpha": idx_alpha, "idx_beta": idx_beta, "idx_g1": idx_g1, "idx_g_end": idx_g_end}
...
def _truth_Q(draws: Dict[str, Any], comp: str, idxs: Dict[str, Optional[int]]) -> Optional[float]:
    ...
    j = {"alpha": idxs["idx_alpha"], "beta": idxs["idx_beta"], "gamma": (idxs["idx_g1"] if idxs["idx_g1"] is not None else idxs["idx_g_end"])}[comp]
    if j is None:
        if QQ.ndim == 1:
...
class DLMPlotter:
    ...
    def _families(self) -> Dict[str, List[Tuple[str, np.ndarray]]]:
        ...
        # NEW: spike&slab indicators as "model evidence"
        for k, name in (("z_alpha", "z_α"), ("z_beta", "z_β"), ("z_gamma", "z_γ")):
            if k in d:
                add("z", name, d[k])

        # m0 (dynamic or deterministic)
        if "m0_alpha" in d:
    ...
    def _fig_posts(self, fam, items, outdir, show):
        ...
        for k, (name, s) in enumerate(items):
            ...
            # Special bins for Bernoulli z indicators to highlight posterior model probability
            if fam == "z" and set(np.unique(s)).issubset({0, 1}):
                ax.hist(s, bins=[-0.5, 0.5, 1.5], density=True, alpha=0.85, label=name)
                p = float(s.mean())
                ax.axvline(p, ls="--", lw=1.0, label=f"mean={p:.3f}")
                ax.set_xticks([0, 1])
            else:
    ...
    def figure_overview(self, save_dir: Optional[str] = None, fname_prefix="overview", show=True):
        ...
        if m0_items:
            for i, (name, s) in enumerate(m0_items[:2]):
                ...
                tv = _truth_m0(self.d, name, self.period)
                if tv is not None:
                    ax.axvline(float(tv), color="k", lw=1.4, label="truth")
            ax.set_title("m0 (subset)")
            _uniq_legend(ax)
        else:
            ax.axis("off")

        # NEW: posterior inclusion probs in overview if z’s present
        ax = axs[5]
        zA = self.d.get("z_alpha", np.array([]))
        zB = self.d.get("z_beta",  np.array([]))
        zG = self.d.get("z_gamma", np.array([]))
        if any(z.size for z in (zA, zB, zG)):
            labs, vals = [], []
            for name, arr in (("α", zA), ("β", zB), ("γ", zG)):
                if arr.size:
                    labs.append(name); vals.append(float(np.asarray(arr).mean()))
            ax.bar(labs, vals)
            ax.set_ylim(0, 1)
            ax.set_title("Posterior inclusion probs P(z=1)")
            ax.grid(True, axis="y")
        else:
            ax.axis("off")
    ...
    def figure_states(self, save_dir: Optional[str] = None, fname_prefix="states", show=True):
        ...
        # Prefer g1 (newest-first), else fall back to last g*
        ig = self.idxs["idx_g1"] if self.idxs["idx_g1"] is not None else self.idxs["idx_g_end"]

        A, B, G = get(self.idxs["idx_alpha"]), get(self.idxs["idx_beta"]), get(ig)
        ...
        ax.legend()
        r += 1
        for arr, lab, truth in ((A, "α", self.t_a), (B, "β", self.t_b), (G, "γ (seasonal coord)", self.t_g)):
            if arr is None:
                continue
            ...
            ax.set_title(f"{'Level' if lab.startswith('α') else ('Trend' if lab.startswith('β') else 'Seasonal')} {lab}")
            ax.legend()
            r += 1
        fig.tight_layout()
        if save_dir:
    ...
    def quick_report(self, save_dir: Optional[str] = None, fname_prefix="quick_report", show=True):
        ...
        # third panel: prefer z posteriors if available, else Q → m0 → P0
        fams = self._families()
        panel_done = False
        ...
        if z_items:
            name, s = z_items[0]
            s = np.asarray(s).ravel()
            axs[2].hist(s, bins=[-0.5, 0.5, 1.5], density=True, label=name)
            axs[2].axvline(float(s.mean()), ls="--", lw=1.0, label=f"mean={float(s.mean()):.3f}")
            axs[2].set_xticks([0, 1])
            axs[2].set_title(f"{name} | y (model inclusion)")
            _uniq_legend(axs[2])
            panel_done = True
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(
        description="DLM plotter (concise, truth-aware) + spike&slab model evidence",
