#!/usr/bin/env python3
"""Fixed-seed univariate compatibility validation for bucex 2.0.0."""
...
def run(data_dir: str | Path, particles: int) -> dict[str, object]:
    ...
    if bx.__version__ != "2.0.0":
        raise RuntimeError(f"Expected bucex 2.0.0, found {bx.__version__}.")
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("validation/univariate_validation_2.0.0.json"),
    )
