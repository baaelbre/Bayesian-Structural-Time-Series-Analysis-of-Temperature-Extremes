...
from ..components import LocalLevel, LocalLinearTrend
...
def make_uccle_factor_model(
    *,
    ...
    individual: str = "static",
) -> FactorModel:
    """Construct a documented six-channel Uccle factor graph.

    ``structure='estimated'`` creates one anchored common factor.  The
    ...
    for name in UCCLE_SERIES:
        ...
        components = (
            ...
            if individual_key == "none"
                ...
                LocalLevel(mode="static" if individual_key == "static" else "dynamic"),
            )
        )
        channels.append(
            Channel(
...
def fit_uccle_factor(
    ...
    **kwargs,
    ...
    resolved_model = (
        make_uccle_factor_model(structure=structure, individual=individual)
        if model is None
        else model
    )
