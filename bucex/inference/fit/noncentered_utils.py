...
def validate_ncp_model(model: StateSpaceModel) -> None:
    """Validate that the model matches the specialized v0.1 NCP scope.
    ...
    if other:
        raise NotImplementedError("Non-centred fitters support only LocalLinearTrend plus optional DummySeasonal in v0.1.")
...
def design_matrix_ncp(z_path: Array, layout: NCPLayout) -> tuple[Array, list[str]]:
    z_path = np.asarray(z_path, dtype=float)
    Tn = z_path.shape[0] - 1
    cols: list[Array] = []
    names: list[str] = []
...
def _posterior_gaussian_linear(
    X: Array,
    y: Array,
    prior_mean: Array,
    prior_sd: Array,
    ...
    s0 = np.asarray(prior_sd, dtype=float).reshape(-1)
    d = X.shape[1]
    if m0.size != d or s0.size != d:
        raise ValueError("prior_mean and prior_sd must match X.shape[1].")
    P0inv = np.diag(1.0 / np.maximum(s0 * s0, 1e-12))
    prec = P0inv + (X.T @ X) / max(float(sigma2), 1e-12)
    cov = spd_solve(prec, np.eye(d))
    cov = _symmetrize(cov)
    mean = cov @ (P0inv @ m0 + (X.T @ y) / max(float(sigma2), 1e-12))
    return _sample_gaussian(mean, cov, rng)


def _posterior_gaussian_linear_weighted(
    X: Array,
    y: Array,
    obs_var: Array,
    prior_mean: Array,
    ...
    X = np.asarray(X, dtype=float)
    y = np.asarray(y, dtype=float).reshape(-1)
    obs_var = np.asarray(obs_var, dtype=float).reshape(-1)
    m0 = np.asarray(prior_mean, dtype=float).reshape(-1)
    s0 = np.asarray(prior_sd, dtype=float).reshape(-1)
    d = X.shape[1]
    ...
    cov = spd_solve(prec, np.eye(d))
    cov = _symmetrize(cov)
    mean = cov @ (P0inv @ m0 + X.T @ (w * y))
    return _sample_gaussian(mean, cov, rng)


def prior_vectors_from_gaussian_priors(priors: Any, layout: NCPLayout) -> tuple[Array, Array, list[str]]:
    names: list[str] = ["alpha0"]
    means: list[float] = [float(priors.alpha0.mean)]
    sds: list[float] = [float(priors.alpha0.sd)]

    if layout.has_beta:
        names.append("beta0")
        means.append(float(priors.beta0.mean))
        sds.append(float(priors.beta0.sd))

    if layout.season_dim > 0:
        if priors.gamma0_season is None:
            raise ValueError("gamma0_season prior is required when the model has a dynamic seasonal block.")
        gm = priors.gamma0_season.mean_array()
        gs = priors.gamma0_season.sd_array()
        if gm.size != layout.season_dim or gs.size != layout.season_dim:
            raise ValueError("gamma0_season prior dimension mismatch.")
        for j in range(layout.season_dim):
            names.append(f"gamma0_season_{j+1}")
            means.append(float(gm[j]))
            sds.append(float(gs[j]))

    names.append("s_level")
    means.append(float(priors.s_level.mean))
    sds.append(float(priors.s_level.sd))

    if layout.has_beta:
        if priors.s_trend is None:
            raise ValueError("s_trend prior is required when the model has a dynamic trend block.")
        names.append("s_trend")
        means.append(float(priors.s_trend.mean))
        sds.append(float(priors.s_trend.sd))

    if layout.season_dim > 0:
        if priors.s_season is None:
            raise ValueError("s_season prior is required when the model has a dynamic seasonal block.")
        names.append("s_season")
        means.append(float(priors.s_season.mean))
        sds.append(float(priors.s_season.sd))

    return np.asarray(means, dtype=float), np.asarray(sds, dtype=float), names


def apply_theta_draw(params_state: ParamDict, theta: Array, theta_names: list[str], layout: NCPLayout) -> ParamDict:
    out = dict(params_state)
    season_vals: list[float] = []
    for val, nm in zip(theta, theta_names):
        if nm.startswith("gamma0_season_"):
            season_vals.append(float(val))
        else:
            out[nm] = float(val)
    if layout.season_dim > 0:
        out["gamma0_season"] = np.asarray(season_vals, dtype=float)
...
def gaussian_theta_update(
    ...
    X, _ = design_matrix_ncp(z_path, layout)
    pm, ps, names = prior_vectors_from_gaussian_priors(priors, layout)
    theta = _posterior_gaussian_linear(X, y, pm, ps, sigma2, rng)
    return apply_theta_draw({}, theta, names, layout)
...
def gev_theta_update(
    ...
    X, _ = design_matrix_ncp(z_path, layout)
    pm, ps, names = prior_vectors_from_gaussian_priors(priors, layout)
    theta = _posterior_gaussian_linear_weighted(X, z_pseudo, R_t, pm, ps, rng)
    return apply_theta_draw({}, theta, names, layout)


def random_sign_switches(z_path: Array, params_state: ParamDict, layout: NCPLayout, rng: np.random.Generator) -> tuple[Array, ParamDict]:
