...
bucex/
  ...
  diagnostics/      chain, engine, scores, PIT, and leave-future-out refits
...
The univariate GEV SSVS strategy composes PGAS with an exact
trans-dimensional independence-MH kernel. Laplace pseudo-information builds a
proposal, never the target. Gaussian SSVS retains its conjugate model-space
enumeration; Laplace GEV SSVS remains an explicitly approximate alternative.

The shared FS FFBS primitive uses Joseph-form filtering and backward
conditional covariances. Symmetric solves preserve legal singular directions,
and strict eigenvalue checks distinguish round-off from materially invalid
covariances.

ASIS adds the complementary sweep but does not change the stored semantic
state representation.

The v2.2 factor FS strategy composes loading kernels by observation family.
