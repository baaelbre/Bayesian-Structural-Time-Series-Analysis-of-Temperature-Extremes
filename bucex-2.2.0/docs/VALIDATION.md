...
The v2.2.0 release check has four layers:

1. retained univariate compatibility and general factor source tests;
2. v2.2 tests for graph eligibility, FS algebra, fixed initial coefficients,
...
3. `validation/run_factor_validation_v2_2.py` for fixed-seed numerical checks;
...
```bash
python -m pytest -q
python validation/run_factor_validation_v2_2.py
