...
from ..priors import factor as factor_priors
...
FORMAT = "bucex-fit"
SCHEMA_VERSION = "2.1"
SUPPORTED_SCHEMA_VERSIONS = {"1.2", "2.0", "2.1"}


def _prior_classes() -> dict[str, type]:
    classes: list[type] = []
    for module in (process_priors, structural_priors, factor_priors):
...
def load_fit(path: str | Path) -> FitResult:
    ...
    model = (
        FactorModel.from_dict(model_payload)
        if model_payload.get("kind") == "factor"
        else Model.from_dict(model_payload)
    )
