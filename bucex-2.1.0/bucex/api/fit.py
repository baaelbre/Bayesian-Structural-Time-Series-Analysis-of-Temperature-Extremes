...
def _fit_fruehwirth_schnatter(
    ...
    *,
    ...
    if plan.engine == "pgas" and getattr(priors, "ssvs", None) is not None:
        raise ValueError("Exact structural SSVS is currently available with Laplace, not PGAS.")
...
def fit(
    y: Array,
    model: Model | FactorModel | str | None = None,
    *,
    ...
    if isinstance(model, FactorModel):
        conflicts = {
            "family": family,
            "trend": trend,
            "period": period,
        ...
        if used:
            raise ValueError(
                f"These univariate-only arguments cannot be used with FactorModel: {used}."
            )
    ...
    if model is None:
        ...
        raise TypeError("model must be a bucex.Model.")
