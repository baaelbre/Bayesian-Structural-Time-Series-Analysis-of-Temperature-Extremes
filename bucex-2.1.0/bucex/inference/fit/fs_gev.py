...
class FSGEVKernel:
    ...
    def fit(
        ...
        if state_method == "pgas" and self.priors.ssvs is not None:
            raise ValueError(
                "Exact structural SSVS is currently paired with the Laplace engine; "
                "use a lasso, horseshoe, or normal prior with PGAS."
            )
        ...
        return FSOutput(
            ...
            meta={
                "sampler": (
                    f"fruehwirth_schnatter_gev_{state_method}_ssvs_gibbs"
                    if self.priors.ssvs is not None
                    else (
