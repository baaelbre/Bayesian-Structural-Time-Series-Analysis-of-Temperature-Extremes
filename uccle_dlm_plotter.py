...
"""
Uccle Harmonic DLM Plotter (TXm, TNm) — traces, posteriors, states
==================================================================

Usage
-----
python uccle_plotter.py --target results/uccle/TX/posterior.npz --show
python uccle_plotter.py --root results/uccle --level 0.90
"""
...
# ----------------------- layout & truths -----------------------
def _layout_from_meta_or_infer(draws: Dict[str, Any], meta: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not lay and "x" in draws and np.ndim(draws["x"]) == 3:
        # infer order "alpha, beta, c1, s1, c2, s2, ... , nyq?"
        dim = draws["x"].shape[2]
        # heuristic: if dim>=1 assume alpha; if dim>=2 assume beta; then pairs; last maybe nyq
        idx_alpha = 0 if dim >= 1 else None
        idx_beta  = 1 if dim >= 2 else None
        start = (1 if idx_alpha is not None else 0) + (1 if idx_beta is not None else 0)
        rem = dim - start
        k_pairs = rem // 2
        for k in range(k_pairs):
            pairs.append((start + 2*k, start + 2*k + 1))
        if rem % 2 == 1:
            idx_nyq = dim - 1  # last coord
        lay = []
        if idx_alpha is not None: lay.append("alpha")
        if idx_beta  is not None: lay.append("beta")
        for j in range(len(pairs)):
            lay += [f"c{j+1}", f"s{j+1}"]
        if idx_nyq is not None: lay.append("nyq")
...
class DLMHarmonicPlotter:
    def __init__(self, draws: Dict[str, Any], meta: Dict[str, Any], level: float = 0.90):
        ...
        self.period = int(meta.get("period", 4))
        self.y = _maybe(draws, "y")
        self.band = f"{int(round(self.level * 100))}% band"

        self.layout = _layout_from_meta_or_infer(draws, meta)
    ...
    def _fig_overview(self, save_dir: Optional[str], show: bool) -> Optional[str]:
        ...
        axs[0].set_title("Posterior μ_t"); _uniq_legend(axs[0])
        ...
        # m0 subset
        ax = axs[4]; plotted = False
        for k, lab in (("m0_alpha", "m0_α"), ("m0_beta", "m0_β")):
            if k in self.d:
                ax.hist(np.asarray(self.d[k]).ravel(), bins=40, density=True, alpha=0.55, label=lab); plotted = True
        if plotted: ax.set_title("m0 (subset)"); _uniq_legend(ax)
        else: ax.axis("off")

        # P0 subset
        ax = axs[5]; plotted = False
        for k, lab in (("P0_alpha", "P0_α"), ("P0_beta", "P0_β"), ("P0_harm", "P0_harm")):
            if k in self.d:
                ax.hist(np.asarray(self.d[k]).ravel(), bins=40, density=True, alpha=0.55, label=lab); plotted = True
        if plotted: ax.set_title("P0 (subset)"); _uniq_legend(ax)
        else: ax.axis("off")

        fig.tight_layout()
        path = None
        if save_dir:
            _ensure_dir(save_dir); path = os.path.join(save_dir, "overview.png")
    ...
    def _fig_states(self, save_dir: Optional[str], show: bool) -> Optional[str]:
        ...
        def get(idx): return (self.d["x"][:, :, idx] if (has_x and idx is not None) else None)
        ...
        G = get(self.layout["pairs"][0][0]) if self.layout["pairs"] else (get(self.layout["idx_nyq"]) if self.layout["idx_nyq"] is not None else None)
        ...
        ax.set_title(r"Posterior $\mu_t$"); _uniq_legend(ax)

        r = 1
        for arr, lab, truth in ((A, "α", self.t_a), (B, "β", self.t_b), (G, "season (cos1/nyq)", self.t_g)):
        ...
        path = None
        if save_dir:
            _ensure_dir(save_dir); path = os.path.join(save_dir, "states.png")
            fig.savefig(path, dpi=200, bbox_inches="tight"); print(f"[save] {path}")
        plt.show() if show else plt.close(fig)
        return path
    ...
    def _families(self) -> Dict[str, List[Tuple[str, np.ndarray]]]:
        ...
        for nm, tag in (("alpha", "α"), ("beta", "β"), ("gamma", "γ")):
            Q = _to_Q(d, nm)
            if Q is not None: add("Q", f"Q_{tag}", Q)
        if "m0_alpha" in d: add("m0", "m0_α", d["m0_alpha"])
        if "m0_beta"  in d: add("m0", "m0_β", d["m0_beta"])
        if "m0_cos"   in d and np.ndim(d["m0_cos"]) == 2:
            mgc = np.asarray(d["m0_cos"]); mgs = np.asarray(d.get("m0_sin", np.zeros_like(mgc)))
            for j in range(mgc.shape[1]):
                add("m0", f"m0_cos[{j+1}]", mgc[:, j]); add("m0", f"m0_sin[{j+1}]", mgs[:, j])
        if "m0_nyq"   in d and np.size(d["m0_nyq"]) > 0: add("m0", "m0_nyq", d["m0_nyq"])
        if "P0_alpha" in d: add("P0", "P0_α", d["P0_alpha"])
        if "P0_beta"  in d: add("P0", "P0_β", d["P0_beta"])
        if "P0_harm"  in d: add("P0", "P0_harm", d["P0_harm"])
        return f
    ...
    def quick_report(self, save_dir: Optional[str] = None, show: bool = True):
        ...
        if self.t_mu is not None and len(self.t_mu) == self.T:
            axs[0].plot(self.t_mu, lw=1.2, ls="--", label="true μ")
        axs[0].set_title("μ_t"); _uniq_legend(axs[0])
        ...
        for comp, series, tag in (("gamma", self.Qg, "Q_γ"), ("alpha", self.Qa, "Q_α"), ("beta", self.Qb, "Q_β")):
            ...
            axs[2].set_title(f"log10 {tag} | y"); _uniq_legend(axs[2])
            panel_done = True; break
        if not panel_done:
            for fam in ("m0", "P0"):
                items = fams.get(fam, [])
                if items:
                    ...
                    axs[2].set_title(f"{name} | y"); _uniq_legend(axs[2])
                    panel_done = True; break
        if not panel_done:
            axs[2].axis("off")
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="Uccle Harmonic DLM Plotter (TXm/TNm)",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--target", type=str, default=None, help="Run dir or posterior.npz. If omitted, search --root.")
    p.add_argument("--root", type=str, default="results/uccle/TN", help="Search root when --target omitted.")
