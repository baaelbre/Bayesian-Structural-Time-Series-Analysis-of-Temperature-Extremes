...
def test_joint_trend_prior_is_the_default_clean_model_space():
    prior = bx.HierarchicalPrior(pool="selection")
    assert prior.model_space == "joint_trend"
    np.testing.assert_allclose(prior.initial_model_probabilities(), 0.25)
    np.testing.assert_allclose(prior.initial_probabilities("trend")[0], 0.0)
    ...
    sensitivity = bx.HierarchicalPrior(
        model_space="componentwise",
        trend_states=("zero", "fixed", "dynamic"),
        trend_concentration=(1.0, 1.0, 1.0),
    )
    assert sensitivity.initial_probabilities("trend")[0] == pytest.approx(1.0 / 3.0)
