...
class FitResult:
    ...
    def is_factor_model(self) -> bool:
        return self.is_multiseries_model and bool(
            getattr(self.compiled, "factor_names", ())
        )
    ...
    def hierarchical_probabilities(self, credible_interval: float = 0.90):
        ...
        if not rows:
            raise ValueError("This fit has no hierarchical SSVS probability draws.")
        try:
            import pandas as pd
    ...
    def hierarchical_slab_summary(self, credible_interval: float = 0.90):
        ...
        rows = []
        prefix = "hierarchical_ssvs.slab_scale."
        alpha = 1.0 - float(credible_interval)
        ...
        if not rows:
            raise ValueError("This fit has no hierarchical SSVS slab-scale draws.")
        try:
            import pandas as pd
    ...
    def channel_rate_summary(
        ...
        *,
        ...
        This method has the same contract for ``MultiSeriesModel`` and
        ``FactorModel`` results.  It deliberately summarizes the complete
        channel predictor; factor-only rates remain available separately via
        :meth:`factor_rate_summary`.
        """
