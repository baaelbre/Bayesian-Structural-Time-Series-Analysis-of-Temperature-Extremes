...
class CompiledFactorFS:
    ...
    ) -> tuple[Array, dict[str, float]] | tuple[
        Array, dict[str, float], dict[str, bool]
    ]:
        output = np.asarray(path, dtype=float).copy()
        switches = {
            process: False
            for block in self.blocks
            for process in block.process_by_component.values()
        }
        for block in self.blocks:
            layout = block.layout
...
def sample_factor_fs_posterior(
    ...
    *,
    ...
    sign_switch_counts = {
        name: np.zeros(chains, dtype=int) for name in compiled.noise_names
    }
    ...
    for chain, sequence in enumerate(sequences):
        ...
        for iteration in range(mcmc.iterations):
            ...
            centered_before_sign = fs.to_centered(path, params)
            path, params, sign_switches = fs.sign_switch(
                path, params, rng, return_switches=True
            )
            _sync_signed_scales(params, compiled)
            centered_after_sign = fs.to_centered(path, params)
            sign_error = float(
                np.max(np.abs(centered_before_sign - centered_after_sign))
            )
            sign_tolerance = 1e-10 * (
                1.0 + float(np.max(np.abs(centered_before_sign)))
            )
            if sign_error > sign_tolerance:
                raise RuntimeError(
                    "Factor FS sign switch changed the centred path by "
                    f"{sign_error:.3g}."
                )
            last_metrics["sign_invariance_error"] = sign_error
            for name, switched in sign_switches.items():
                sign_switch_counts[name][chain] += int(switched)
    ...
    diagnostics: dict[str, Any] = {
        ...
        "sign_switch_counts": {
            name: values.tolist() for name, values in sign_switch_counts.items()
        },
    }
    return FitResult(
