"""Joint hierarchical SSVS for structurally related time series.
...
from ...priors.hierarchical import HierarchicalSSVSPriors
...
def _update_hierarchy(
    ...
    probabilities = {
        name: rng.dirichlet(hierarchy.concentration(name) + counts[name])
        for name in _COMPONENTS
    }
    slab = {}
    for name in _COMPONENTS:
        current = float(current_slab[name])
        if hierarchy.learn_slab_scale:
...
def _current_prior(
    state: _ChannelState,
    resolved: HierarchicalSSVSPriors,
...
def _parameter_storage(
    ...
    for item in states:
        name = item.name
        output[f"intercept.{name}"] = np.zeros((chains, draws))
    ...
    for component, names in labels.items():
        for index, name in enumerate(names):
            output[f"hierarchical_ssvs.prob.{component}.{name}"] = np.zeros(
                (chains, draws)
            )
        output[f"hierarchical_ssvs.slab_scale.{component}"] = np.zeros(
            (chains, draws)
        )
    return output
...
def _store_parameters(
    ...
    for item in states:
        name = item.name
        output[f"intercept.{name}"][chain, draw] = float(item.params_state["alpha0"])
    ...
    for component, names in labels.items():
        for index, name in enumerate(names):
            output[f"hierarchical_ssvs.prob.{component}.{name}"][chain, draw] = float(
                probabilities[component][index]
            )
        output[f"hierarchical_ssvs.slab_scale.{component}"][chain, draw] = float(
            slab[component]
        )


def sample_hierarchical_ssvs_posterior(
    y: Array,
    compiled: CompiledFactorModel,
    priors: HierarchicalSSVSPriors,
    plan: InferencePlan,
    *,
    ...
    if not np.all(np.isfinite(values)):
        raise ValueError(
            "Hierarchical FS SSVS currently requires aligned finite observations."
        )
    if plan.parameterization != "fruehwirth_schnatter":
        raise ValueError("Hierarchical SSVS requires parameterization='fs'.")
    if plan.engine not in {"ffbs", "pgas"}:
        raise ValueError("Hierarchical SSVS supports exact FFBS or PGAS, not Laplace.")
    if plan.asis:
    ...
    for chain, sequence in enumerate(seed_sequences):
        ...
        probabilities = {
            component: priors.hierarchy.concentration(component)
            / np.sum(priors.hierarchy.concentration(component))
    ...
    return FitResult(
        ...
        metadata={
            ...
            "inference_contract": "joint_hierarchical_structural_ssvs",
            "joint_model": True,
            "joint_likelihood": True,
            "hierarchical_model_selection": True,
            "shared_temporal_state": False,
            "conditional_channel_independence": True,
            "structural_ssvs": True,
            "model_selection_exact": True,
...
__all__ = ["sample_hierarchical_ssvs_posterior"]
