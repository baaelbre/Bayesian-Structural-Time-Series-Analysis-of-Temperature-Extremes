...
def as_factor_array(y: Any, model: FactorModel | MultiSeriesModel) -> Array:
...
def factor_dates(y: Any, model: FactorModel | MultiSeriesModel) -> Array | None:
...
def _factor_exog(
    exog: Any,
    model: FactorModel | MultiSeriesModel,
    n_time: int,
) -> dict[str, Array] | None:
...
def compile_factor_model(
    ...
    if not isinstance(model, (FactorModel, MultiSeriesModel)):
        raise TypeError("model must be a FactorModel or MultiSeriesModel.")
    ...
    if not transitions:
        raise ValueError("The multiseries model compiled to an empty state.")
    ...
    for index, channel in enumerate(model.channels):
        ...
        channel_location[channel.name] = float(np.median(finite_values))
...
# The representation is intentionally shared: factors are simply absent for a
# MultiSeriesModel, while channel blocks retain the same compiler contract.
CompiledMultiSeriesModel = CompiledFactorModel
compile_multiseries_model = compile_factor_model
