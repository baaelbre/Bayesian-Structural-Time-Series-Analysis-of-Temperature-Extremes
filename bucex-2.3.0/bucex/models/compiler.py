...
def compile_model(model: Any, y: Any, exog: Any = None):
    """Compile either a univariate :class:`Model` or a :class:`FactorModel`.
    ...
    if isinstance(model, (FactorModel, MultiSeriesModel)):
        from .factor_compiler import compile_factor_model

        return compile_factor_model(model, y, exog=exog)
    if not isinstance(model, Model):
        raise TypeError(
            "model must be a bucex.Model, bucex.MultiSeriesModel, or "
            "bucex.FactorModel."
        )
