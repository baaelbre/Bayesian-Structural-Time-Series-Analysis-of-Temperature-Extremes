...
def _structural_prior_density(fit, name: str, grid):
    ...
    if hierarchy is not None:
        ...
        if component is None:
            return None
        concentration = hierarchy.concentration(component)
        dynamic_probability = float(concentration[-1] / np.sum(concentration))
...
def plot_hierarchical_ssvs(fit, *, figsize=(9, 6)):
...
def plot_fit(fit, kind: str = "state", **kwargs):
    ...
    if getattr(fit, "is_multiseries_model", False):
        if key in {"factor", "shared_factor", "factor_state"}:
            if not fit.is_factor_model:
                raise ValueError("MultiSeriesModel has no shared factor state.")
        ...
        if key in {"decomposition", "factor_decomposition"}:
            if not fit.is_factor_model:
                raise ValueError("MultiSeriesModel has no factor decomposition.")
        ...
        if key in {"hierarchy", "hierarchical_ssvs", "population_ssvs"}:
            return finish(plot_hierarchical_ssvs(fit, **kwargs))
        if key in {"loading_deviation", "loading_deviation_joint"}:
            if not fit.is_factor_model:
                raise ValueError("MultiSeriesModel has no factor loadings.")
            return finish(plot_loading_deviation_joint(fit, **kwargs))
        if key in {"identification", "loading_deviation_correlations"}:
            if not fit.is_factor_model:
                raise ValueError("MultiSeriesModel has no factor identification split.")
            return finish(plot_loading_deviation_correlations(fit, **kwargs))
        if key in {"idiosyncratic_innovations", "idio_innovations"}:
            if not fit.is_factor_model:
                raise ValueError("Use process_sd/traces for MultiSeriesModel innovations.")
            return finish(plot_idiosyncratic_innovations(fit, **kwargs))
        raise ValueError(
            "Multiseries kind must be channel, process_sd, parameter_density, "
            "traces, acf, component_probabilities, or hierarchy; factor fits "
            "also support factor, decomposition, loading_deviation, "
            "identification, and idiosyncratic_innovations."
        )
    if key in {"process_sd", "process_sds", "prior_posterior_sd"}:
        return finish(plot_process_sds(fit, **kwargs))
