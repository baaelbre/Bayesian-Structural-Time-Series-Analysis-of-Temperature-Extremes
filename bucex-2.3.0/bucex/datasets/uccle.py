...
def make_uccle_multiseries_model(
    *,
    series: Iterable[str] = UCCLE_SERIES,
    seasonal: bool = True,
    period: int = 12,
) -> MultiSeriesModel:
    """Construct the six-series hierarchical-SSVS alternative.

    Unlike :func:`make_uccle_factor_model`, this graph contains no shared
    latent climate path or loadings.  Its channels are coupled only through
    population-level SSVS probabilities and learned dynamic-slab scales.
    Each channel starts with a local-linear-trend grammar; SSVS decides whether
    its level, trend, and optional seasonal block are zero/fixed/dynamic.
    """
    ...
    if unknown:
        raise ValueError(f"Unknown Uccle series: {unknown}.")
    if len(selected) < 2:
        raise ValueError("The Uccle multiseries model requires at least two series.")
    if int(period) < 2:
        raise ValueError("period must be at least 2.")
    channels = []
    for name in selected:
    ...
    return MultiSeriesModel(
        channels=tuple(channels),
        name="Uccle hierarchical structural SSVS",
        description=(
            "Related summaries with shared structural-selection probabilities "
            "and slab scales, but no common latent path."
        ),
    )


def fit_uccle_multiseries(
    data_dir: str | Path | None = None,
    *,
    ...
    **kwargs,
    ...
    """Load aligned Uccle summaries and fit joint hierarchical SSVS."""
