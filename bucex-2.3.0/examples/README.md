...
| 14 | `14_hierarchical_ssvs.py` | Joint hierarchical SSVS for several related Gaussian series, from simulation through plots and forecasts |
| 15 | `15_uccle_hierarchical_ssvs.py` | Fit all six Uccle summaries with shared structural-selection probabilities and PGAS |
| 16 | `16_hierarchy_or_factor.py` | Fit hierarchical SSVS and a common factor to the same data and compare their estimands |
...
10. For hierarchical SSVS, allocation switching within each series and mixing
    of the population probabilities and shared slab multipliers.
11. For every FS fit, the stored sign-switch invariance error should be at
    numerical zero; the signed innovation scale itself is not scientifically
    identified, whereas its absolute value is.
...
## Choosing between Examples 9, 10, and 15

- Example 9 gives six fully separate analyses. It asks what happens in each
  summary without pooling information.
- Example 15 uses hierarchical SSVS. It asks whether zero/fixed/dynamic
  components recur across summaries and partially pools that model-selection
  evidence. It does **not** estimate a shared warming trajectory.
- Example 10 uses a dynamic factor. It asks for a common warming path,
  heterogeneous loadings, and idiosyncratic departures. Its shared/deviation
  decomposition requires the identification checks printed by the script.

Example 16 puts the latter two choices side by side. For a manuscript whose
central question is heterogeneity *around a common warming signal*, the factor
model is the natural primary model and hierarchical SSVS is a strong structural
sensitivity analysis. If the main question is instead which kinds of dynamics
are present across six summaries, hierarchical SSVS can be primary.
