#!/usr/bin/env python3
"""Fixed-seed factor and v2.2 numerical validation for bucex 2.2.0."""
...
def v220_numerics_prediction_check() -> dict[str, object]:
    """Gate the corrected seasonal algebra, Joseph FFBS, exact SSVS, and LFO."""
    ...
    seasonal_model = bx.Model(
        bx.Gaussian(),
        (bx.LocalLinearTrend(), bx.DummySeasonal(4)),
    )
    layout = infer_ncp_layout(seasonal_model)
    ...
    for name in names:
        if name == "alpha_c":
            ...
            theta.append(
                params["gamma0_season"][int(name.rsplit("_", 1)[1]) - 1]
            )
        else:
    ...
    exact_ssvs = bx.fit(
        ...
        priors="ssvs",
        engine="pgas",
        mcmc=bx.MCMC(draws=3, warmup=3, chains=1, seed=223),
        particles=bx.Particles(n=20),
    )
    forecast = exact_ssvs.forecast(2, draws=3, seed=224)
    held_out = simulation.y[-2:]
    log_score = forecast.log_score(held_out)
    pit = forecast.pit(held_out)
    passed = bool(
        seasonal_error < 1e-12
        and long_path.shape == (1_001, 2)
        and np.all(np.isfinite(long_path))
        and exact_ssvs.metadata["model_selection_exact"] is True
        and "exact_gev_rjmh" in exact_ssvs.metadata["model_selection_basis"]
        and np.all(np.isfinite(log_score))
        and np.all((pit >= 0.0) & (pit <= 1.0))
    )
    return {
        "seasonal_design_reconstruction_max_abs_error": seasonal_error,
        "long_joseph_ffbs_shape": list(long_path.shape),
        "long_joseph_ffbs_finite": bool(np.all(np.isfinite(long_path))),
        "gev_ssvs_exact": exact_ssvs.metadata["model_selection_exact"],
        "gev_ssvs_basis": exact_ssvs.metadata["model_selection_basis"],
        "log_score": log_score.tolist(),
        "pit": pit.tolist(),
        "passed": passed,
    }
...
def run(data_dir: str | Path) -> dict[str, object]:
    if bx.__version__ != "2.2.0":
        raise RuntimeError(f"Expected bucex 2.2.0, found {bx.__version__}.")
    started = time.perf_counter()
    ...
    v220 = v220_numerics_prediction_check()
    sections = {
        ...
        "v220_numerics_and_prediction": v220,
    }
    return {
        "release": "bucex 2.2.0",
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("validation/factor_validation_2.2.0.json"),
    )
