...
fit.channel_rate_summary("TXx", 1950, 2022)
...
## Relationship to hierarchical SSVS

Version 2.3 also provides `MultiSeriesModel`, but it is not a cheaper spelling
of this factor model. Hierarchical SSVS shares categorical structural
probabilities and dynamic-slab scales while keeping a separate latent path for
every series. It answers whether dynamic levels, slopes, or seasons recur
across summaries. It has no (f_t), no (lambda_i), and no shared/deviation
decomposition.

For the manuscript question “do temperature summaries respond differently to
a common warming signal?”, retain the factor model as primary. Use the
hierarchical model to test whether the factor conclusion depends on forcing a
common trajectory and to learn which idiosyncratic structures the data prefer.
Example 16 fits both to the same data and prints their distinct estimands.

The factor FS sampler also randomizes every unidentified signed innovation
coefficient together with its standardized path. It verifies the reconstructed
centered path is invariant; switch counts and numerical errors are retained in
sampler diagnostics. This sign symmetry is distinct from factor sign/scale
identification, which still requires fixed loading anchors.
