...
def load_uccle_series(
    ...
    *,
    ...
    if data_dir is not None and (Path(data_dir) / f"{series}.csv").is_file():
        root = Path(data_dir)
    else:
...
class UccleFitCollection:
    ...
    def save(self, directory: str | Path) -> None:
        ...
        for name, fit in self.fits.items():
            fit.save(directory / f"{name}_bucex_v{__version__}.bucex")
...
def fit_uccle_series(
    ...
    *,
    ...
    if mcmc is not None:
        if config is not None or any(value is not None for value in (n_iter, burn, thin, progress)):
            raise ValueError("Give mcmc or compact Gibbs arguments, not both.")
        config = GibbsConfig(
            n_iter=int(mcmc.iterations),
            burn=int(mcmc.warmup),
            thin=int(mcmc.thin),
            seed=mcmc.seed,
            progress=bool(mcmc.progress),
        )
        chains = int(mcmc.chains)
        seed = None
    if engine != "auto":
        if state_method != "auto":
            raise ValueError("Give engine or state_method, not both.")
        state_method = engine
    options = {} if state_kwargs is None else dict(state_kwargs)
    if particles is not None:
        options.setdefault("particles", int(particles.n))
        options.setdefault("particle_proposal", str(particles.proposal))
    if laplace is not None:
        options.setdefault("laplace_max_iterations", int(laplace.max_iterations))
        options.setdefault("laplace_tolerance", float(laplace.tolerance))
        options.setdefault("curvature_floor", float(laplace.curvature_floor))
        options.setdefault("maximum_variance", float(laplace.maximum_variance))
        options.setdefault("draw_attempts", int(laplace.draw_attempts))
