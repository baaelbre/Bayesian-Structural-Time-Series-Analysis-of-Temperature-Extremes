...
from .sampler import Laplace, MCMC
...
def load_uccle_series(
    ...
    *,
    ...
    ``data_dir`` is optional.  Supplying it makes a research run explicitly use
    the repository copy.
    """
    ...
    if data_dir is not None:
        path = Path(data_dir) / f"{series}.csv"
        if not path.is_file():
            raise FileNotFoundError(path)
    else:
        path = _resolve_data_dir(None) / f"{series}.csv"
...
def _compatibility_mcmc(
    ...
    *,
    ...
    resolved_burn = 0 if burn is None else int(burn)
    resolved_thin = 1 if thin is None else int(thin)
    if int(n_iter) <= resolved_burn:
        raise ValueError("Require n_iter > burn.")
    draws = len(range(resolved_burn, int(n_iter), resolved_thin))
    return MCMC(
        draws=draws,
