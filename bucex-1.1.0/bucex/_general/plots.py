...
def plot_process_sds(fit, *, bins: int = 35, credible_interval: float = 0.90, figsize=None):
    ...
    for axis, name in zip(axes[:, 0], names):
        ...
        if not isinstance(prior, FixedSD):
            prior_sample = np.asarray(prior.sample(np.random.default_rng(140), size=4000))
