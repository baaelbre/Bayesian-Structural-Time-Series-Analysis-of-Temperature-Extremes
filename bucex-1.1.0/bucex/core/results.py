...
class _StateDrawAccessor:
    """Callable state selector that also exposes the chain-shaped draw array."""

    def __init__(self, fit: "PosteriorBundle") -> None:
        self.fit = fit

    def __call__(self, name: str, *, original_scale: bool = True) -> Array:
        return self.fit._state_draws_by_name(name, original_scale=original_scale)

    def __array__(self, dtype=None, copy=None) -> Array:
        values = self.fit.chain_state_draws
        if dtype is not None:
            values = values.astype(dtype, copy=False)
        return np.array(values, copy=bool(copy)) if copy is not None else values

    def __getitem__(self, item):
        return self.fit.chain_state_draws[item]

    @property
    def shape(self) -> tuple[int, ...]:
        return self.fit.chain_state_draws.shape

    @property
    def ndim(self) -> int:
        return self.fit.chain_state_draws.ndim
...
class PosteriorBundle:
    ...
    def n_chains(self) -> int:
        return int(self.meta.get("n_chains", 1))

    @property
    def draws_per_chain(self) -> Optional[int]:
        stored = self.meta.get("draws_per_chain")
        if stored is not None:
            return int(stored)
        return self.n_draws if self.n_chains == 1 else None
    ...
    def methods(self) -> Dict[str, Any]:
        return {
            "engine": self.meta.get("state_method"),
            "parameterization": self.meta.get("parameterization"),
            "asis": bool(self.meta.get("asis", False)),
        }
    ...
    def draws_aux(self) -> Dict[str, Array]:
        return {
            ...
            for key, value in self.meta.get("engine_diagnostics", {}).items()
        }

    @property
    def parameter_draws(self) -> Dict[str, Array]:
        return self.draws_static
    ...
    def _chain_slices(self) -> list[tuple[int, int]]:
        stored = self.meta.get("chain_slices")
        if stored is None:
            return [(0, self.n_draws)]
        return [(int(start), int(stop)) for start, stop in stored]

    def _as_chains(self, values: Array) -> Array:
        array = np.asarray(values)
        blocks = [array[start:stop] for start, stop in self._chain_slices()]
        lengths = {block.shape[0] for block in blocks}
        if len(lengths) != 1:
            raise ValueError("Chain diagnostics require equally long retained chains.")
        return np.stack(blocks, axis=0)

    @property
    def chain_state_draws(self) -> Array:
        if self.draws_states is None:
            raise ValueError("This posterior does not contain state draws.")
        return self._as_chains(self.draws_states)

    def parameter(self, name: str, *, by_chain: bool = False) -> Array:
        if name not in self.draws_static:
            raise KeyError(f"Unknown posterior parameter {name!r}.")
        values = np.asarray(self.draws_static[name])
        return self._as_chains(values) if by_chain else values
    ...
    @property
    def state_draws(self) -> _StateDrawAccessor:
        return _StateDrawAccessor(self)

    def _state_draws_by_name(self, name: str, *, original_scale: bool = True) -> Array:
    ...
    def exceedance_probability_draws(
        ...
        *,
        ...
        if not annual:
            return (p, self.time) if return_labels else p
        groups, labels = self._annual_groups()
        annual_p = np.column_stack(
            [1.0 - np.prod(1.0 - p[:, idx], axis=1) for idx in groups]
        )
        annual_p = np.clip(annual_p, 0.0, 1.0)
        return (annual_p, labels) if return_labels else annual_p
    ...
    def return_period_draws(
        ...
        *,
        ...
        p, labels = self.exceedance_probability_draws(
            threshold, annual=annual, return_labels=True
        )
        periods = 1.0 / np.maximum(p, min_probability)
        return (periods, labels) if return_labels else periods
    ...
    def diagnostics(self) -> Dict[str, Any]:
        """Return rank-normalized MCMC and engine-specific diagnostics."""
        from ..diagnostics import ess_bulk, rhat

        rows = []
        for name, values in self.draws_static.items():
            array = np.asarray(values)
            if array.ndim != 1:
                continue
            chains = self._as_chains(array)
            rows.append(
                {
                    "parameter": name,
                    ...
                    "acceptance": float(self.acceptance.get(name, np.nan)),
                }
            )
        try:
            import pandas as pd

            parameters = pd.DataFrame(rows).set_index("parameter")
        except ImportError:
            parameters = rows

        metrics = self.meta.get("engine_diagnostics", {})
        engine: Dict[str, float] = {}
        if self.meta.get("state_method") == "laplace" and metrics:
            engine = {
                ...
                "median_relative_change": float(
                    np.nanmedian(metrics["laplace_relative_change"])
                ),
                "mean_support_rejections": float(
                    np.nanmean(metrics["laplace_support_rejections"])
                ),
                "restored_fraction": float(self.meta.get("restored_fraction", 0.0)),
            }
        elif self.meta.get("state_method") == "pgas" and metrics:
            engine = {
                "median_min_particle_ess": float(
                    np.nanmedian(metrics["particle_min_ess"])
                ),
                "mean_unique_ancestors": float(
                    np.nanmean(metrics["particle_mean_unique_ancestors"])
                ),
                "path_change_rate": float(
                    np.nanmean(metrics["particle_path_changed"])
                ),
                "mean_changed_fraction": float(
                    np.nanmean(metrics["particle_changed_fraction"])
                ),
                "restored_fraction": float(self.meta.get("restored_fraction", 0.0)),
            }
        return {
    ...
    def inclusion_probabilities(self) -> Dict[str, float]:
        """Probability that each SSVS component is dynamic."""
        output: Dict[str, float] = {}
        for component in ("level", "trend", "season"):
            key = f"state_{component}"
            if key in self.draws_static:
                output[component] = float(
                    np.mean(np.asarray(self.draws_static[key], dtype=int) == 2)
                )
        if not output:
            raise ValueError("This fit does not contain structural SSVS draws.")
        return output
    ...
    def forecast(self, horizon: int, **kwargs):
        """Generate a posterior predictive forecast with all uncertainties."""
        from ..api.forecast import posterior_predict_fs

        return posterior_predict_fs(self, horizon, **kwargs)
    ...
    def save(self, path: str) -> None:
        """Save a checksummed JSON/NumPy archive; no pickle is executed."""
        from ..io.posterior import save_posterior_bundle

        save_posterior_bundle(self, path)

    @classmethod
    def load(cls, path: str) -> "PosteriorBundle":
        from ..io.posterior import load_posterior_bundle

        return load_posterior_bundle(path)
