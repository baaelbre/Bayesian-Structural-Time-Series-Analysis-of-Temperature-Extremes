...
class RegularizedHorseshoePrior:
    """Regularized horseshoe prior for signed innovation scales.
    ...
    Conditional on ``lambda_k``, ``tau`` and ``c`` the signed scales retain a
    Gaussian prior, so the exact Gaussian FS regression update and the exact
    GEV elliptical-slice update remain available.
    """
    ...
    def __post_init__(self) -> None:
        required = {"level", "trend", "season"}
        missing = required - set(self.coefficient_scale)
        if missing:
            raise ValueError(
                f"coefficient_scale is missing components: {sorted(missing)}"
            )
        if any(float(self.coefficient_scale[key]) <= 0.0 for key in required):
...
# ---------------------------------------------------------------------------
# v1.1 regularized-horseshoe profiles
# ---------------------------------------------------------------------------
