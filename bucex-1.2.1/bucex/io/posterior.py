...
def _prior_classes() -> dict[str, type]:
    classes: list[type] = []
    for module in (process_priors, structural_priors):
...
def load_fit(path: str | Path) -> FitResult:
    """Load and validate a v1.2 fit archive without executing serialized code."""
    ...
    return FitResult(
        ...
        transform_sign=float(payload.get("transform_sign", 1.0)),
