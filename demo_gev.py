...
from bucex.inference.fit.centered_gev import CenteredGEVGibbs
...
STATE_METHOD = "particle"  # "laplace" or "particle"
STATE_KWARGS = {
    "particle_method": "auxiliary",
    "particle_n_particles": 100,
}
T = 240
RESULTS_DIR = "results"
...
def build_priors() -> CenteredGEVPriors:
...
def save_obs_figure(
    ...
    ax.set_title(f"GEV observation model and latent mean ({STATE_METHOD})")
    ax.set_xlabel("time")
    ax.set_ylabel("value")
    ax.legend()
    ax.grid(True, alpha=0.3)
    outpath = outdir / "gev_fit_obs_eta.png"
...
def save_trace_figure(
    ...
    axes[2].plot(q_level)
    axes[2].set_title("Trace: q_level")
    axes[2].grid(True, alpha=0.3)
    axes[3].plot(q_trend)
    axes[3].set_title("Trace: q_trend")
    axes[3].grid(True, alpha=0.3)
    axes[4].plot(q_season)
    axes[4].set_title("Trace: q_season")
    axes[4].set_xlabel("saved draw index")
    axes[4].grid(True, alpha=0.3)
    outpath = outdir / "gev_fit_parameter_traces.png"
    fig.savefig(outpath, dpi=200, bbox_inches="tight")
    print(f"Saved figure to: {outpath}")
    plt.show()
...
def main() -> None:
    ...
    print(f"\n--- GEV Gibbs fit complete ({STATE_METHOD}) ---")
    ...
    save_state_figure(
        outdir=outdir,
        filename="gev_fit_level_alpha.png",
    ...
    save_state_figure(
        outdir=outdir,
        filename="gev_fit_slope_beta.png",
    ...
    save_state_figure(
        outdir=outdir,
        filename="gev_fit_seasonality_g1.png",
