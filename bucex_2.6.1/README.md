...
extreme-value observations. Version 2.6.1 provides one focused, reproducible
...
| Profile | Uccle window | Draws/warmup | Chains | Particles | Simulation blocks | Period |
|---|---|---:|---:|---:|---:|---:|
| `smoke` | 2015–2022 | 2/2 | 1 | 24 | 48 | 4 |
| `pilot` | 1892–2022 | 250/250 | 2 | 128 | 400 | 4 |
| `publication` | 1892–2022 | 2,000/2,000 | 4 | 512 | 800 | 4 |
...
Tail-class illustrations use one long period-4 local-level realization and vary
only `xi`: `-0.30`, `0`, and `+0.30`. A second matched experiment fixes
...
the UC structure. The default 800-block record makes the selection evidence
clear without requiring implausibly large innovations or seasonal amplitudes:

- stationary;
- linear trend;
- random walk;
- local linear trend without seasonality;
- stationary level with changing seasonality;
- local linear trend with fixed seasonality.

The public `bx.make_tail_scenarios()`, `bx.make_scale_scenarios()`, and
`bx.make_structural_scenarios()` constructors expose length, period, `sigma`,
`xi`, slopes, process standard deviations, seasonal amplitudes, and seeds.
Every numbered simulation example calls those constructors and then the core
`bx.simulate_scenario()` API.
...
bucex-presentation run simulation-fit \
  --profile pilot \
  --engine laplace \
  --scenario local_linear_trend_fixed_season \
