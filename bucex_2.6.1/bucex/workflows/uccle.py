...
from ..priors import InverseGammaPrior, UniformPrior, ssvs_gev_priors
...
def presentation_gev_prior(*, alpha_mean: float, period: int = 12):
    ...
    return ssvs_gev_priors(
        ...
        beta_mean=0.0,
        beta_sd=0.01,
        seasonal_initial_sd=2.5 if simulation else 2.25,
        sigma2_prior=InverseGammaPrior(2.0, 2.25 if simulation else 2.0),
        xi_prior=UniformPrior(-0.5, 0.5),
        xi_max_abs=0.5,
        innovation_slab_sd=(
            {"level": 0.10, "trend": 0.0008, "season": 0.07}
            if simulation
            else {"level": 0.12, "trend": 0.0015, "season": 0.10}
        ),
...
class PresentationWorkflow:
    """Run, combine, and report the v2.6.1 presentation analyses."""
    ...
    def run_simulations(
        ...
        *,
        ...
        for base in selected:
            current = replace(
                base.resized(self.config.runtime.simulation_months),
                period=self.config.runtime.simulation_period,
            )
    ...
    def fit_simulations(
        ...
        *,
        ...
        for design in selected:
            ...
            simulation_period = int(truth.get("period", design.period))
            ...
            fit_result = fit(
                ...
                priors=presentation_gev_prior(
                    alpha_mean=float(np.median(y)), period=simulation_period
                ),
                engine=engine,
