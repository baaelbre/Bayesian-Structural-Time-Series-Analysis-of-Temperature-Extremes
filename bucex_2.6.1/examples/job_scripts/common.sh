...
if [[ -n "${BUCEX_SIMULATION_LENGTH:-}" ]]; then
  CLI_CONFIG+=(--simulation-length "$BUCEX_SIMULATION_LENGTH")
elif [[ -n "${BUCEX_SIMULATION_MONTHS:-}" ]]; then
  CLI_CONFIG+=(--simulation-months "$BUCEX_SIMULATION_MONTHS")
fi
if [[ -n "${BUCEX_SIMULATION_PERIOD:-}" ]]; then
  CLI_CONFIG+=(--simulation-period "$BUCEX_SIMULATION_PERIOD")
fi
