...
def _series_out_root(series: str) -> Path:
    ...
    Map series name to required output root directories:
      ...
      Precm → results/uccle/Prec/Precm/Seasonal/
    """
    base = Path("results/uccle")
    mapping = {
        "Precm": base / "Prec" / "Precm" / "Seasonal",
    }
...
def load_series(csv_path: Path) -> pd.Series:
    ...
    if n <= 0:
        raise ValueError(
            f"Series in {csv_path} is shorter than one full seasonal cycle (4 points)."
        )
    return ser.iloc[:n]
...
def run_one(
    ...
    ----------
    series : {"Precm"}
        Short series code used in filenames ("Precm").
    y_ser : pd.Series
        Time series of seasonal mean precipitation.
    out_root : Path
        Base directory (e.g., results/uccle/Prec/Precm/Seasonal).
    ...
    # ----- Run sampler ----- #
    print(f"Running harmonic precip DLM for {series} with modes={modes_tag} ...")
    ...
    sampler.save_posterior(
        ...
        extra_meta={
            "series": series,
            "label": f"{series}_seasonal_precip",
...
def main() -> None:
    ...
    # Base root for precipitation
    prec_root = _series_out_root("Precm")  # results/uccle/Prec/Precm/Seasonal
    ...
    run_one("Precm", precm_seasonal, prec_root, level_mode, trend_mode, seasonal_mode)
