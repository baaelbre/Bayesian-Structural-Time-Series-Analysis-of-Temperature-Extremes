# run_uccle_harmonic.py  (formerly run_uccle_dummy_ncp.py)
import os
import math
...
from optimization.dlm_harm_lognormal import (
    DLMGibbsHarmonic,
    Priors,
    SamplerConfig,
)
...
def run_one(label: str, y_ser: pd.Series, outdir: Path) -> None:
    ...
    sampler = DLMGibbsHarmonic(
        ...
        m0_beta_init=0.0,
        P0_beta_init=0.05,
        # seasonal initialisation: no need to pass harmonics here; defaults to zero means
        m0_cos_init=None,
        m0_sin_init=None,
        m0_nyq_init=0.0,
        P0_harm_init=0.25,
...
def main() -> None:
    ...
    run_one("TXm_seasonal", tx, Path("results/uccle/TX_harm/"))
    run_one("TNm_seasonal", tn, Path("results/uccle/TN_harm/"))

    print("Saved results under results/uccle/{TX_harm,TN_harm}")
