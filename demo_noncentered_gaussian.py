...
def build_priors() -> NonCenteredGaussianPriors:
    return NonCenteredGaussianPriors(
        sigma2=InverseGammaPrior(a=2.0, b=1.0),
        alpha0=NormalPrior(mean=0.0, sd=2.0),
...
def save_obs_figure(
    ...
    ax.set_title("Gaussian observation model and latent mean (non-centred)")
    ax.set_xlabel("time")
    ax.set_ylabel("value")
    ax.legend()
    ax.grid(True, alpha=0.3)
    outpath = outdir / "noncentered_gaussian_fit_obs_eta.png"
...
def save_trace_figure(
    ...
    fig, axes = plt.subplots(7, 1, figsize=(12, 14), constrained_layout=True)
    axes[0].plot(sigma)
    axes[0].set_title("Trace: sigma")
    axes[1].plot(s_level)
    axes[1].set_title("Trace: s_level")
    axes[2].plot(s_trend)
    axes[2].set_title("Trace: s_trend")
    axes[3].plot(s_season)
    axes[3].set_title("Trace: s_season")
    axes[4].plot(q_level)
    axes[4].set_title("Trace: q_level = s_level^2")
    axes[5].plot(q_trend)
    axes[5].set_title("Trace: q_trend = s_trend^2")
    axes[6].plot(q_season)
    axes[6].set_title("Trace: q_season = s_season^2")
    axes[6].set_xlabel("saved draw index")
    for ax in axes:
        ax.grid(True, alpha=0.3)
    outpath = outdir / "noncentered_gaussian_parameter_traces.png"
    fig.savefig(outpath, dpi=200, bbox_inches="tight")
    print(f"Saved figure to: {outpath}")
    plt.show()
...
def main() -> None:
    ...
    print("\n--- Non-centred Gaussian Gibbs fit complete ---")
    ...
    save_state_figure(
        outdir=outdir,
        filename="noncentered_gaussian_level_alpha.png",
    ...
    save_state_figure(
        outdir=outdir,
        filename="noncentered_gaussian_slope_beta.png",
    ...
    save_state_figure(
        outdir=outdir,
        filename="noncentered_gaussian_seasonality_g1.png",
