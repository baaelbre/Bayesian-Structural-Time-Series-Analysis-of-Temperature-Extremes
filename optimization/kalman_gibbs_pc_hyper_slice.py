...
# =============================================================================
# Priors & Config (Normal on log σ & det. params; PC priors on sds)
# with Gamma hyperpriors on the PC rates λ; Slice sampling for log sds
# =============================================================================

@dataclass
class PCPrior:
    """
    PC prior for innovation sd s>0: p(s | λ) = λ exp(-λ s).
    If lambda_s is None, λ ~ Gamma(a_lambda, b_lambda) (shape–rate).
    When λ is not fixed, we still initialize it by data calibration:
        λ_init = -log(alpha_prob) / (frac * scale)
    """
...
@dataclass
class Priors:
    # Observation variance prior: log σ ~ N(m_sigma, s_sigma^2)
    m_sigma: float = 0.0
    s_sigma: float = 1.0
    ...
    # PC priors (with optional hyperpriors) for process sds s = sqrt(q)
...
class SamplerConfig:
    ...
    # RW–MH step sizes (kept for logsigma & deterministic params)
    step_logsigma: float = 0.05
    step_level: float = 0.05
    step_slope: float = 0.02
    step_season: float = 0.05

    # ---- Adaptive RW–MH options for the above (Robbins–Monro; windowed) ----
    ...
    # ---- Slice sampler knobs for log s updates (rarely need changing) ----
    slice_w_logsd: float = 0.4     # initial bracket width in z=log s
    slice_m_logsd: int = 40        # max stepping-out expansions
    slice_max_shrink: int = 1000   # cap on shrinkage draws

# =============================================================================
# DLM with FFBS + (RW–MH for logsigma/det params) + (Slice for process sds)
# and Gamma hyperpriors on PC rates λ
# =============================================================================
...
class DLMGibbs:
    ...
    Steps per iteration:
      1) FFBS sample latent states x_{0:T}
      2) Update process sds s_α, s_β, s_γ with **slice sampling** on z=log s
      3) Gibbs for PC rates λ if not fixed
      4) RW–MH updates for deterministic params (if present)
      5) RW–MH update for log σ (Normal prior)
    """
    ...
    def __init__(
        ...
        # Initial variances (used to start chains)
        ...
        # ---- Bookkeeping (MH stats retained for logsigma/det params;
        #                  for slice steps we count accepted=proposed)
        self.accept = {
    ...
    def _slice_sample_logsd(self, z0: float, SS: float, T_eff: int, lam: float) -> float:
        """
        Univariate log-slice sampler for z = log s.

        Target in z:
            f(z) = -T z - 0.5*SS*exp(-2z) - lam*exp(z) + z  (up to const)
        which is strictly concave (great for slice).

        Returns new z.
        """
        w   = float(self.cfg.slice_w_logsd)
        m   = int(self.cfg.slice_m_logsd)
        max_shrink = int(self.cfg.slice_max_shrink)

        def f(z: float) -> float:
            return -(T_eff * z) - 0.5 * SS * math.exp(-2.0 * z) - lam * math.exp(z) + z

        y = f(z0) - np.random.exponential(1.0)

        # Stepping-out interval [L, R]
        u = np.random.rand()
        L = z0 - u * w
        R = L + w
        ...
        # Shrinkage until inside the slice
        for _ in range(max_shrink):
            ...
            if z_prop < z0:
                ...
                R = z_prop
        return z0  # extremely rare fallback
    ...
    def update_logsigma(self) -> None:
        ...
        e = self.y - mu_vec
        ll_old = self._gaussian_loglike(mu_vec)                 # uses current self.sigma2
    ...
    # ---- Build innovation SS terms for each coordinate (given current x)
    ...
    def _innovation_ss_beta(self) -> Tuple[float, int]:
        ...
        return float(np.sum(diffs * diffs)), self.T
    ...
    # ---- Slice updates for process sds (given current λ)
    def update_process_sds(self) -> None:
        ...
        if self.idx_alpha is not None and (self.lambda_alpha is not None):
            ...
            z0 = math.log(max(1e-18, float(self.s_alpha)))
            z_new = self._slice_sample_logsd(z0, ss, T_eff, float(self.lambda_alpha))
            self.s_alpha = float(math.exp(z_new))
            self.proposals["log_s_alpha"] += 1
            self.accept["log_s_alpha"] += 1  # slice is acceptance-free
        # β
        if self.idx_beta is not None and (self.lambda_beta is not None):
            ss, T_eff = self._innovation_ss_beta()
            z0 = math.log(max(1e-18, float(self.s_beta)))
            z_new = self._slice_sample_logsd(z0, ss, T_eff, float(self.lambda_beta))
            self.s_beta = float(math.exp(z_new))
            self.proposals["log_s_beta"] += 1
            self.accept["log_s_beta"] += 1
        # γ (last seasonal coord)
        if self.seasonal_mode == "dynamic" and (self.lambda_gamma is not None):
            ss, T_eff = self._innovation_ss_gamma()
            z0 = math.log(max(1e-18, float(self.s_gamma)))
            z_new = self._slice_sample_logsd(z0, ss, T_eff, float(self.lambda_gamma))
            self.s_gamma = float(math.exp(z_new))
            self.proposals["log_s_gamma"] += 1
            self.accept["log_s_gamma"] += 1

    # ---- Gibbs updates for λ given s (Gamma hyperprior)
    #      λ | s  ~ Gamma(a_lambda + 1, b_lambda + s)  (shape–rate)
    ...
    def run(self) -> Dict[str, np.ndarray]:
        ...
        for it in range(cfg.n_iter):
            ...
            # 2) Update process sds (PC priors) via **slice sampling** given current λ
            if self.dim > 0:
            ...
            # --- adapt RW–MH proposal scales if enabled (does not affect slice) ---
            ...
            if cfg.progress and ((it + 1) % print_every == 0 or it == cfg.n_iter - 1):
                ...
                pc_info = (f" | acc(log s_α/β/γ)="
                           f"{self._fmt_acc('log_s_alpha') if self.idx_alpha is not None else 'n/a'}/"
                           f"{self._fmt_acc('log_s_beta')  if self.idx_beta  is not None else 'n/a'}/"
                           f"{self._fmt_acc('log_s_gamma') if self.seasonal_mode == 'dynamic' else 'n/a'}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Gaussian DLM (FFBS) with RW–MH (log σ & det params), slice on process sds, and PC priors with Gamma hyperpriors on λ (DLMGibbs)")
    # Modes
    ...
    # RW–MH steps (only used for logsigma & deterministic params)
    ...
    # Slice sampler knobs (optional)
    parser.add_argument("--slice-w-logsd", type=float, default=0.4)
    parser.add_argument("--slice-m-logsd", type=int, default=40)
    parser.add_argument("--slice-max-shrink", type=int, default=1000)
    ...
    cfg = SamplerConfig(
        ...
        slice_w_logsd=float(args.slice_w_logsd),
        slice_m_logsd=int(args.slice_m_logsd),
