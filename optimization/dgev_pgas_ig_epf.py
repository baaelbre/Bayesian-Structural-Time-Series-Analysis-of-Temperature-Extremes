...
# =============================================================================
# Priors & Config  (Inverse-Gamma priors on Q)
# =============================================================================

@dataclass
class Priors:
    # Observation params (Gaussian priors on logσ and ξ)
    m_sigma: float = 0.0
    s_sigma: float = 10.0
    m_xi: float = 0.0
    s_xi: float = 1.0
    ...
    # Process noise variances for dynamics: Q ~ IG(a, b) with density b^a / Γ(a) * Q^{-(a+1)} exp(-b/Q)
    a_q_alpha: float = 1.1
...
class DGEVParticleGibbs:
    def __init__(
        ...
        # ---- Innovation variances Q (IG priors). Initialize at IG means if a>1
    ...
    # --------- IG conjugate updates for process variances Q ------------------ #
    def update_Q(self) -> None:
        """Sample each active variance from IG(a0 + T/2, b0 + 0.5 * RSS_k)."""
        if self.dim == 0:
            return
        ...
        if self.idx_beta is not None:
            ...
            rss = float(np.sum((self.x[1:, self.idx_beta] - self.x[:-1, self.idx_beta])**2))
            a = a0 + 0.5 * self.T
            b = b0 + 0.5 * rss
            self.Q[self.idx_beta] = 1.0 / np.random.gamma(a, 1.0 / b)

        # seasonal last component (others are deterministic shifts)
        if self.seasonal_mode == "dynamic":
            g0, gL = self.idx_gamma_start, self.idx_gamma_end
    ...
    def _conditional_pgas(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, float, dict]:
        ...
        # Precompute for transition density
    ...
    def _progress_line(self, it: int, ema_logZ: Optional[float]) -> str:
        ...
        q_alpha = self._format_Q_slot("Q_α", self.idx_alpha) if self.level_mode == "dynamic" else "/"
        q_beta  = self._format_Q_slot("Q_β", self.idx_beta)  if self.trend_mode == "dynamic" else "/"
        q_gamma = self._format_Q_slot("Q_γ", self.idx_gamma_end) if self.seasonal_mode == "dynamic" else "/"
    ...
    def run(self) -> Dict[str, np.ndarray]:
        ...
        for it in range(cfg.n_iter):
            ...
            # 2) dynamic process variances (IG conjugate)
            if self.dim > 0:
                self.update_Q()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="DGEV PGAS Sampler (Laplace proposal; IG priors for Q)")
