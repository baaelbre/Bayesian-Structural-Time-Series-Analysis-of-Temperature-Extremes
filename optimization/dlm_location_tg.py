...
class Priors:
    ...
    # === Triple-gamma (TG) prior hyperparameters on precisions ω_k = 1 / Q_k ===  ### TG
...
# =============================================================================
# DLM Sampler (conjugate Gibbs with Triple-Gamma priors on process precisions)
# =============================================================================
class DLMGibbsConjugate:
    ...
    Gaussian structural DLM with:
      ...
      • Triple-Gamma prior on process precisions ω_k = 1/Q_k (pure Gibbs)
    ...
    def __init__(
        ...
        # Store process *precisions* ω_k and local scales λ_k; also a global δ.  ### TG
        # Initialize from given s_* (convert to ω=1/Q).
    ...
    def _Q(self) -> np.ndarray:
        """
        Build Q from current precisions with deterministic flips if needed.     ### TG
        """
        if self.dim == 0:
            return np.zeros((0, 0))
    ...
    # ------------- Innovation sums of squares (for Q/ω updates) ------------- #
    ...
    # =============================================================================
    # Pure Gibbs for process precisions under triple-gamma prior               ### TG
    # =============================================================================
    def _update_tg_block(self, SS: float, T_eff: int, omega: float, lam: float) -> Tuple[float, float]:
        """
        One TG block update for a single component (returns new (omega, lambda)).
        Priors (shape–rate):
            ω | λ ~ Ga(α0, λ)
            λ | δ ~ Ga(β0, δ)
        Likelihood of ω from innovations: ω^{T/2} exp(-(SS/2) ω)
        ⇒ ω | λ, data ~ Ga(α0 + T/2, rate = λ + SS/2)
           λ | ω, δ    ~ Ga(α0 + β0, rate = δ + ω)
        """
        if T_eff <= 0:
            # no information: just update λ given ω, and keep ω as-is
            α0, β0, δ = self.priors.tg_alpha0, self.priors.tg_beta0, self.delta
    ...
    def update_process_Q_triplegamma(self) -> None:
        # Update per-component (ω_k, λ_k)
        if self.idx_alpha is not None:
            SS, T_eff = self._innovation_ss_alpha()
            self.omega_alpha, self.lambda_alpha = self._update_tg_block(SS, T_eff, self.omega_alpha, self.lambda_alpha)
        if self.idx_beta is not None:
            SS, T_eff = self._innovation_ss_beta()
            self.omega_beta,  self.lambda_beta  = self._update_tg_block(SS, T_eff, self.omega_beta,  self.lambda_beta)
        if self.seasonal_mode == "dynamic":
            SS, T_eff = self._innovation_ss_gamma()
            self.omega_gamma, self.lambda_gamma = self._update_tg_block(SS, T_eff, self.omega_gamma, self.lambda_gamma)
        ...
        rate  = r0 + float(np.sum(lam_list)) if m > 0 else r0
    ...
    def update_deterministic_params(self) -> None:
        ...
        if self.trend_mode == "deterministic":
            if self.idx_alpha is not None:
                ...
                # Note: if Q_alpha is zero, this update still uses a tiny jitter via prec.
    ...
    def run(self) -> Dict[str, np.ndarray]:
        ...
        for it in range(cfg.n_iter):
            ...
            # 2) process Q via triple-gamma (pure Gibbs on ω, λ, δ)           ### TG
            if self.dim > 0:
                self.update_process_Q_triplegamma()
    ...
    def save_posterior(self, out_npz_path: str, extra_meta: Optional[dict] = None) -> None:
        ...
        meta = {
            ...
            # Save TG hyperparams and last (ω, λ, δ) for reproducibility         ### TG
            "tg": {
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(
        description=(
            ...
            "Process precisions use a TRIPLE-GAMMA global–local prior; "
    ...
    p.add_argument("--tg-alpha0", type=float, default=0.5, help="shape for ω|λ") # best practice: 0.5
    p.add_argument("--tg-beta0",  type=float, default=0.5, help="shape for λ|δ") # best practice: 0.5
    p.add_argument("--tg-gamma0", type=float, default=0.5, help="shape for δ prior") # best practice: 0.5
    p.add_argument("--tg-r0",     type=float, default=0.5, help="rate for δ prior") # best practice: 0.5
    ...
    if args.plot:
        ...
        ttl = f"DLM ({args.level_mode}/{args.trend_mode}/{args.seasonal_mode}) — Triple-Gamma"
        plt.title(ttl)
