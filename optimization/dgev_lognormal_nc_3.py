# %% optimization/dgev_pgas_ig_unif.py
...
#       p(v) ∝ v^{-(a+1)} exp(-b / v)  for v > 0
#    ⇒ p(σ) up to const:  log p(σ) = -(2a+1) log σ − b / σ²
#
#  • ξ ~ Uniform[xi_lower, xi_upper]
#
#  • Process SDs s_α, s_β, s_γ still have log-normal priors on ln s.
# =============================================================================

@dataclass
class Priors:
    # Observation priors
    # Inverse-Gamma prior on σ², with shape a_sigma and scale b_sigma
    a_sigma: float = 2.0
    b_sigma: float = 1.0
    ...
    # Log-Normal priors for process SDs (unchanged)
...
class DGEVParticleGibbs:
    ...
    def __init__(
        ...
        # Ensure initial xi lies in Uniform support
        ...
        self.w = np.zeros((self.T + 1, self.dim), float)
    ...
    # --- IG prior for σ² (expressed in terms of σ) ---------------------- #
    def _log_prior_sigma(self, sigma: float) -> float:
        """
        σ² ~ InvGamma(a_sigma, b_sigma).
        For v = σ², p(v) ∝ v^{-(a+1)} exp(-b/v). Transforming to σ gives, up to constants:
            log p(σ) = -(2a + 1) log σ - b / σ²
        """
        if sigma <= 0.0:
            return -np.inf
        ...
        if a <= 0.0 or b <= 0.0:
            # technically improper, but allow; treat as flat
            return 0.0
        return -(2.0 * a + 1.0) * math.log(sigma) - b / (sigma ** 2)
    ...
    def update_logsigma(self) -> None:
        ...
        if ll_new == -np.inf:
            return
        lp_old = self._log_prior_sigma(sigma_cur)
        lp_new = self._log_prior_sigma(sigma_prop)
        if self._mh_accept((ll_new + lp_new) - (ll_old + lp_old)):
            self.logsigma = prop
            self.sigma = sigma_prop
            self.accept["logsigma"] += 1

    # -------- Uniform prior for xi: helper & MH update ------------- #
    def _log_prior_xi(self, xi: float) -> float:
        """
        Uniform prior on [xi_lower, xi_upper].
        Inside the interval: log prior = 0 (up to constants).
        Outside: -inf.
        """
    ...
    def _conditional_pgas(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, float, dict]:
        ...
        if D == 0:
            ...
            pf_diag = {
                "ess_mean": float(N), "ess_min": float(N),
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=("DGEV PGAS Sampler — disturbance-space NCP with log-normal priors on process SDs "
                     "+ Inv-Gamma prior on σ² and Uniform prior on ξ.")
    )
    ...
    parser.add_argument("--prior-a-sigma",  type=float, default=2.0,
                        help="shape parameter a for Inv-Gamma prior on σ²")
    parser.add_argument("--prior-b-sigma",  type=float, default=1.0,
                        help="scale parameter b for Inv-Gamma prior on σ²")
    parser.add_argument("--prior-xi-lower", type=float, default=-0.5,
                        help="lower bound for Uniform prior on ξ")
    parser.add_argument("--prior-xi-upper", type=float, default=0.5,
                        help="upper bound for Uniform prior on ξ")
