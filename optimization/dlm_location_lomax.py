...
class DLMGibbsConjugate:
    ...
    def _slice_logsd_lomax(self, z0: float, SS: float, T_eff: int, a: float, b: float) -> float:
        def f(z: float) -> float:
            return -(T_eff * z) - 0.5 * SS * math.exp(-2 * z) - (a + 1.0) * math.log(b + math.exp(z)) + z
...
if __name__ == "__main__":
    ...
    # --- Collapsed PC hyperpriors for process sds (no fixed lambdas anywhere) ---
    p.add_argument("--pc-a-alpha",  type=float, default=2.0)
    p.add_argument("--pc-b-alpha",  type=float, default=1.0)
    p.add_argument("--pc-a-beta",   type=float, default=2.0)
    p.add_argument("--pc-b-beta",   type=float, default=1.0)
    p.add_argument("--pc-a-gamma",  type=float, default=2.0)
    p.add_argument("--pc-b-gamma",  type=float, default=1.0)
    ...
    if m0_season is None:
        m0_season = [0.0] * (args.period - 1)     # neutral newest-first (length p-1)
