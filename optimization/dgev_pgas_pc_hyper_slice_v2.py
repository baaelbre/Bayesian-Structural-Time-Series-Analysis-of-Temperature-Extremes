...
# =============================================================================
# Priors & Config (PC priors on s = sqrt(Q)) — matches Kalman Gibbs layout
# =============================================================================
...
class Priors:
    ...
    # PC priors for dynamic state-noise sds (with optional fixed lambdas)
...
class SamplerConfig:
    ...
    # Slice sampler (for log s only; mirrors Kalman defaults)
    slice_w: float = 0.4
    slice_m: int = 40
...
#
# Matches Kalman-Gibbs state convention:
...
class DGEVParticleGibbs:
    ...
    def __init__(
        ...
        # ---- Deterministic parameters (outside state), mirrors Kalman
        ...
        if self.seasonal_mode == "deterministic":
            ...
            # full length-p vector with sum-zero
        ...
        # ---- Process sds and lambdas (PC priors)
    ...
    # ----------- Update process sds (slice on log s, PC prior) ----------- #
    ...
    def _transition_logpdf(self, x_prev: np.ndarray, x_cur: np.ndarray, t: int) -> float:
        ...
        out = 0.0
        # per-dimension Gaussian transition with diag Q
    ...
    def _conditional_pgas(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, float, dict]:
        ...
        for t in it:
            ...
            # weights at time t (bootstrap: only likelihood; carry forward if no resample)
            y_idx = t - 1
            lw = np.zeros(N, float)
            for n in range(N):
    ...
    # ---------------- Wrapper for deterministic params (mirror Kalman API) --------------- #
    ...
    def run(self) -> Dict[str, np.ndarray]:
        ...
        # allocate storage (mirror Kalman)
        ...
        for it in range(cfg.n_iter):
            ...
            # 6) Deterministic params (MH)
            self.update_deterministic_params()

            # 7) Observation params (MH)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "— CLI structured to parallel the Kalman Gibbs example."
        )
    )
