...
"""
Gaussian structural DLM with harmonic seasonality (cos/sin pairs + optional Nyquist)
— **FFBS in Non-Centered Parameterization (NCP)** + Gibbs —
• Define S from process SDs (s_alpha, s_beta, s_gamma) and scale states: x̃ = S^{-1} x
• Run FFBS with **Q = I** on (A, H̃=H S, ũ=S^{-1}u, R=σ²); draw x̃_{0:T}
• Map back: x = S x̃; compute ε, η on original scale
• σ² via Gamma on precision; process SDs via **log-normal priors** with **slice on z=ln s**
• Deterministic blocks, m0/P0, CLI, saving/plotting consistent with your previous API
"""
...
class Priors:
    ...
    ln_s_alpha_mu: float = -5.0; ln_s_alpha_sd: float = 1.0
    ln_s_beta_mu:  float = -7.0; ln_s_beta_sd:  float = 1.0
    ln_s_gamma_mu: float = -6.0; ln_s_gamma_sd: float = 1.0
...
class DLMGibbsHarmonic:
    """
    **Non-Centered** FFBS for Gaussian structural DLM with harmonic seasonality.

    Workhorse:
      - Build S from current process SDs (s_alpha, s_beta, s_gamma)
      - Transform to x̃ = S^{-1} x so Q̃ = I
      - Run FFBS with (A, H̃ = H S, ũ = S^{-1} u, R = σ², Q = I)
      - Map back x = S x̃ and continue updates as usual.
    """

    # --------------- Construct --------------- #
    def __init__(self, y: np.ndarray, period: int, harmonics: Optional[int] = None,
        ...
        self.s = int(period);  assert self.s >= 2, "period must be >= 2"

        # harmonics/nyquist
        K_full = (self.s - 1)//2
        self.K = K_full if harmonics is None else int(harmonics)
        assert 0 <= self.K <= K_full, f"K in [0,{K_full}] for s={self.s}"
        even = (self.s % 2) == 0
    ...
    def _Q(self) -> np.ndarray:
        """Original-scale Q (for diagnostics only)."""
        if self.dim == 0: return np.zeros((0,0))
    ...
    def _S_and_transforms(self):
        """Build S, S^{-1}, transformed (H̃, ũ, m̃0, P̃0) for Q̃=I FFBS."""
        if self.dim == 0:
            Z = np.zeros((0,0)); return Z, Z, Z, Z, Z, Z, Z
        svec = []
        if self.idx_alpha is not None: svec.append(self.s_alpha)
        if self.idx_beta  is not None: svec.append(self.s_beta)
        if self.seasonal_mode == "dynamic":
            for _ in range(self.K): svec += [self.s_gamma, self.s_gamma]
            if self.use_nyq: svec.append(self.s_gamma)
        svec = np.asarray(svec, float)
        svec = np.maximum(svec, 1e-16)  # guard
        S = np.diag(svec)
        Sinv = np.diag(1.0/svec)
        ...
        m0, P0diag = self._current_m0_P0()
        m0_tilde = Sinv @ m0
        P0_tilde = P0diag / (np.diag(S)**2)  # diagonal
        return S, Sinv, A, H_tilde, u_tilde, m0_tilde, P0_tilde

    # --------------- FFBS with Q=I on x̃ --------------- #

    def _ffbs_ncp(self) -> np.ndarray:
        if self.dim == 0: return self.x.copy()
        S, Sinv, A, Ht, ut, m0t, P0t = self._S_and_transforms()
        R = float(self.sigma2)

        T, d = self.T, self.dim
        m = np.zeros((T+1, d)); C = np.zeros((T+1, d, d))
        a = np.zeros((T+1, d)); Rm = np.zeros((T+1, d, d))
        m[0] = m0t; C[0] = np.diag(P0t) + 1e-12*np.eye(d)

        # forward (Q=I)
        for t in range(1, T+1):
            a[t]  = A @ m[t-1] + ut
            Rm[t] = A @ C[t-1] @ A.T + np.eye(d)
            Rm[t] = 0.5*(Rm[t] + Rm[t].T) + 1e-12*np.eye(d)

            resid = float(self.y[t-1] - self._mu_det(t-1))
            Syy = float(Ht @ Rm[t] @ Ht.T + R)
            if Syy <= 0:
                Syy = float(Ht @ (Rm[t] + 1e-10*np.eye(d)) @ Ht.T + R)
            Kt = (Rm[t] @ Ht.T).reshape(-1) / Syy
            v  = resid - float(Ht @ a[t])
            m[t] = a[t] + Kt*v
            C[t] = Rm[t] - np.outer(Kt, (Ht @ Rm[t]).reshape(-1))
            C[t] = 0.5*(C[t] + C[t].T) + 1e-12*np.eye(d)

        # backward (Carter–Kohn) on x̃
        xtilde = np.zeros((T+1, d))
        xtilde[T] = np.random.multivariate_normal(m[T], C[T])
        for t in range(T-1, -1, -1):
            J = C[t] @ A.T @ _spd_solve(Rm[t+1], np.eye(d))
            mean = m[t] + J @ (xtilde[t+1] - (A @ m[t] + ut))
            cov  = C[t] - J @ Rm[t+1] @ J.T
            cov  = 0.5*(cov + cov.T)
            ev = float(np.linalg.eigvalsh(cov).min())
            if ev < 1e-12: cov += (1e-12 - ev) * np.eye(d)
            xtilde[t] = np.random.multivariate_normal(mean, cov)
    ...
    def _disturbances_from_x(self) -> Tuple[np.ndarray, np.ndarray]:
        """ε_t and η_t on original scale (for σ² and s updates)."""
        H, A, u = self._H(), self._A(), self._u()
        eps = np.zeros(self.T, float)
        eta = np.zeros((self.T, self.dim), float) if self.dim>0 else np.zeros((0,0))
        for t in range(1, self.T+1):
            eps[t-1] = self.y[t-1] - self._mu_det(t-1) - float(H @ self.x[t])
            if self.dim>0: eta[t-1] = self.x[t] - (A @ self.x[t-1] + u)
        return eps, eta

    # --------------- σ² | ε --------------- #

    def update_sigma2(self) -> None:
        eps, _ = self._disturbances_from_x()
        a = self.priors.a_sigma + 0.5 * self.T
        b = self.priors.b_sigma + 0.5 * float(eps @ eps)
        tau = np.random.gamma(shape=a, scale=1.0 / b)  # precision
        self.sigma2 = 1.0 / max(tau, 1e-300)

    # --------------- Innovation SS (for log-normal s updates) --------------- #

    def _eta_ss_alpha(self, eta: np.ndarray) -> Tuple[float, int]:
        if self.idx_alpha is None: return 0.0, 0
        idx = self.idx_alpha
        return float(np.sum(eta[:, idx]**2)), self.T

    def _eta_ss_beta(self, eta: np.ndarray) -> Tuple[float, int]:
        if self.idx_beta is None: return 0.0, 0
        idx = self.idx_beta
        return float(np.sum(eta[:, idx]**2)), self.T

    def _eta_ss_gamma(self, eta: np.ndarray) -> Tuple[float, int]:
        if self.seasonal_mode != "dynamic": return 0.0, 0
        ss = 0.0; count = 0
        for k in range(1, self.K+1):
            i = self._idx_pair(k)
            ss += float(np.sum(eta[:, i]**2) + np.sum(eta[:, i+1]**2))
            count += 2
        if self.use_nyq:
            j = self.idx_nyq
            ss += float(np.sum(eta[:, j]**2))
            count += 1
        return ss, self.T * count

    # --------------- Log-normal s updates (slice on z=ln s) --------------- #

    def _logpost_z(self, z: float, SS: float, T_eff: int, mu: float, sd: float) -> float:
        ll = -T_eff * z - 0.5 * SS * math.exp(-2.0 * z)  # η ~ N(0, s^2)
        lp = -0.5 * ((z - mu) ** 2) / (sd ** 2)
        return ll + lp

    def update_process_Q_lognormal(self, eta: np.ndarray) -> None:
        rng = self._rng; w, m = float(self.cfg.slice_w), int(self.cfg.slice_m)
        if self.idx_alpha is not None:
            SS, T_eff = self._eta_ss_alpha(eta)
        ...
        if self.idx_beta is not None:
            SS, T_eff = self._eta_ss_beta(eta)
        ...
        if self.seasonal_mode == "dynamic":
            SS, T_eff = self._eta_ss_gamma(eta)
    ...
    def update_m0(self) -> None:
        ...
        if self.idx_alpha is not None:
            self.m0_alpha = self._gibbs_m0_scalar(float(self.x[0, pos]),
                                                  self.priors.m_m0_alpha, self.priors.s_m0_alpha, self.P0_alpha); pos += 1
        if self.idx_beta is not None:
            self.m0_beta  = self._gibbs_m0_scalar(float(self.x[0, pos]),
                                                  self.priors.m_m0_beta,  self.priors.s_m0_beta,  self.P0_beta);  pos += 1
        if self.seasonal_mode == "dynamic":
    ...
    def run(self) -> Dict[str, np.ndarray]:
        ...
        for it in range(cfg.n_iter):
            # 1) NCP FFBS draw for x̃ then map back to x
            if self.dim>0:
                self.x = self._ffbs_ncp()

            # 2) σ² | ε (Gibbs on precision)
            self.update_sigma2()

            # 3) process SDs s_k via log-normal priors using η from x
            eps, eta = self._disturbances_from_x()
            if self.dim>0:
                self.update_process_Q_lognormal(eta)

            # 4) m0 and 5) P0 for dynamic coords
            if self.dim>0:
                self.update_m0()
                self.update_P0()
    ...
    def save_posterior(self, out_npz_path: str, extra_meta: Optional[dict] = None) -> None:
        ...
        meta = {
            "T": int(self.T), "dim": int(self.dim),
            "period": int(self.s), "harmonics": int(self.K),
...
if __name__ == "__main__":
    ...
    def _csv(s: Optional[str]) -> Optional[List[float]]:
    ...
    p = argparse.ArgumentParser(
        description=("Gaussian DLM with harmonic seasonality. "
                     "**NCP FFBS**: x̃= S^{-1}x, Q̃=I; σ² from ε; log-normal priors on process SDs from η.")
    )
    ...
    # Option B (full-length dummies for projection)
    ...
    p.add_argument("--print-dummies-every", type=int, default=1)

    # Log-normal prior hyperparameters (process SDs)
    p.add_argument("--ln-s-alpha-mu", type=float, default=-1.0)
    p.add_argument("--ln-s-alpha-sd", type=float, default=3.0)
    p.add_argument("--ln-s-beta-mu",  type=float, default=-1.0)
    p.add_argument("--ln-s-beta-sd",  type=float, default=3.0)
    p.add_argument("--ln-s-gamma-mu", type=float, default=-1.0)
    p.add_argument("--ln-s-gamma-sd", type=float, default=3.0)
    ...
    # parse lists
    sim_m0_cos = _csv(args.sim_m0_cos)
    sim_m0_sin = _csv(args.sim_m0_sin)
    pri_m_cos  = _csv(args.prior_m_m0_cos)
    pri_m_sin  = _csv(args.prior_m_m0_sin)
    m0_cos_init = _csv(args.m0_cos_init)
    m0_sin_init = _csv(args.m0_sin_init)
    ...
    mts = Mean_Time_Series(
        ...
        season_dummies=season_dummies, start_date=_parse_date(args.start_date),
    ...
    if args.plot:
        ...
        plt.title(f"DLM harmonic (NCP FFBS): {args.level_mode}/{args.trend_mode}/{args.seasonal_mode} | "
