# %% optimization/dgev_pgas_ln_harmonic.py
...
def build_seasonal(period: int) -> np.ndarray:
    ...
    Still useful for deterministic dummies; dynamic season is in harmonics.
    """
...
class Priors:
    ...
    # For dynamic seasonal block (harmonic states); length = n_season if provided
    ...
    b_P0_gamma: float = 1.0   # shared across all seasonal harmonic coords
    ...
    m_slope: float = 0.0
    s_slope: float = 10.0
    # For deterministic season based on dummies (length period)
...
#
# State layout (harmonic seasonality)
#   - Dynamic state layout: [alpha] [beta] [c1 s1 | c2 s2 | ... | cK sK | nyq?]
#   - Seasonal block is harmonic, rotated by fixed matrices; observation loads
#     **cosine coordinates (and Nyquist)**.
# =============================================================================
...
class DGEVParticleGibbs:
    ...
    def __init__(
        ...
        y: np.ndarray,
        period: int,
        # harmonic spec (parallel to DLM harmonic)
        ...
        # Seasonal dynamic initial means (harmonic coords)
        # length = 2*K (+1 for nyq if used) if provided
        m0_season_init: Sequence[float] | None = None,
        ...
        # deterministic seasonal pattern as full dummies (length = period)
        ...
        K_full = (self.period - 1) // 2
        self.K = K_full if harmonics is None else int(harmonics)
        if not (0 <= self.K <= K_full):
            raise ValueError(f"harmonics K must be in [0, {K_full}] for period={self.period}")
        even = (self.period % 2) == 0
        if use_nyquist is None:
            # allow Nyquist if even period and we are using all frequencies up to it
            self.use_nyq = bool(even and (self.K >= (self.period // 2 - 1)))
        else:
        ...
        # Precompute harmonic rotation angles
        if self.K > 0:
            self._omegas = 2.0 * np.pi * (np.arange(1, self.K + 1, dtype=float)) / float(self.period)
            self._cosw = np.cos(self._omegas)
            self._sinw = np.sin(self._omegas)
        else:
            self._omegas = np.zeros(0)
            self._cosw = np.zeros(0)
            self._sinw = np.zeros(0)
        # Number of seasonal dynamic coordinates
        self.n_season = 2 * self.K + (1 if self.use_nyq else 0)
        ...
        def _idx_pair(k: int) -> int:
            """
            Return index of c_k in the flat state vector for harmonic k.
            s_k is at index+1.
            """
            pos = 0
        ...
        # Deterministic seasonal vector as dummies (length period)
        if self.seasonal_mode == "deterministic":
            if seasonal_vector_init is not None:
                base = np.asarray(seasonal_vector_init, float)
                if base.size != self.period:
                    raise ValueError("seasonal_vector_init must have length = period.")
            else:
                if self.priors.m_season is not None:
                    base = np.asarray(self.priors.m_season, float)
                    if base.size != self.period:
                        raise ValueError("priors.m_season must have length = period.")
                else:
                    base = build_seasonal(self.period)
            # we store as full length; no sum-to-zero constraint required here
            self.season_vec = base.astype(float)
        else:
        ...
        if self.seasonal_mode == "dynamic":
            if m0_season_init is None:
                self.m0_gamma = np.zeros(self.n_season, float)
            else:
                g = np.asarray(m0_season_init, float)
                if g.size != self.n_season:
                    raise ValueError(
                        "m0_season_init must have length = 2*K (+1 if Nyquist used)"
                    )
                self.m0_gamma = g
    ...
    def _current_m0_P0(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if self.seasonal_mode == "dynamic":
            ...
            P0.extend([self.P0_gamma] * self.n_season)
        return np.asarray(m0, float), np.asarray(P0, float)

    # ----------------------------- State model ------------------------------ #
    def _state_mean(self, x_prev: np.ndarray, t: int) -> np.ndarray:
        """
        Harmonic dynamic seasonal:
          For each k: [c_k, s_k]_t = R(ω_k) [c_k, s_k]_{t-1}
          Nyquist (if used): nyq_t = -nyq_{t-1}
        """
        ...
        # dynamic seasonal (harmonic rotation)
        if self.seasonal_mode == "dynamic":
            for k in range(1, self.K + 1):
                i = self._idx_pair(k)
                co = float(self._cosw[k - 1])
                si = float(self._sinw[k - 1])
                c_prev = x_prev[i]
                s_prev = x_prev[i + 1]
                m[i]     =  co * c_prev + si * s_prev
                m[i + 1] = -si * c_prev + co * s_prev
            if self.use_nyq and self.idx_nyq is not None:
                m[self.idx_nyq] = -x_prev[self.idx_nyq]
        return m
    ...
    def _season_contribution(self, x_t: np.ndarray, t: int) -> float:
        # dynamic harmonic seasonal: sum over cos coordinates (+ nyquist)
        if self.seasonal_mode == "dynamic":
            val = 0.0
            for k in range(1, self.K + 1):
                i = self._idx_pair(k)
                val += float(x_t[i])  # load cos only
            if self.use_nyq and self.idx_nyq is not None:
                val += float(x_t[self.idx_nyq])
            return float(val)
        # deterministic season from full-length seasonal dummies
        if self.seasonal_mode == "deterministic":
    ...
    def _transition_logpdf(self, x_prev: np.ndarray, x_cur: np.ndarray, t: int) -> float:
        ...
        for tag_idx, tag in enumerate(self._layout):
            ...
            elif tag.startswith("c") or tag.startswith("s") or tag == "nyq":
                var = (self.s_gamma ** 2) if self.s_gamma > 0 else eps
            else:
                var = eps
    ...
    def _transition_sample(self, x_prev: np.ndarray, t: int) -> np.ndarray:
        ...
        for tag_idx, tag in enumerate(self._layout):
            if tag == "alpha":
                ...
                var[tag_idx] = (self.s_gamma ** 2) if self.s_gamma > 0 else self.cfg.trans_eps
            else:
    ...
    def _innovation_ss_gamma(self) -> Tuple[float, int]:
        """
        Seasonal innovation for harmonic dynamics:

        For each k:
            [c_k, s_k]_t ~ N( R(ω_k) [c_k, s_k]_{t-1}, s_gamma^2 I_2 )
        For Nyquist (if used):
            nyq_t ~ N( -nyq_{t-1}, s_gamma^2 )

        We return SS = sum of squared innovation components and
        T_eff = number of scalar innovations (per_step * T).
        """
        if self.seasonal_mode != "dynamic" or self.n_season == 0:
            return 0.0, 0
        ss = 0.0
        per_step = 0
        # harmonic pairs
    ...
    def update_season_vec(self) -> None:
        """
        MH update for deterministic full-length seasonal vector (dummies).
        This remains in dummy space; dynamic seasonal is harmonic.
        """
        if self.seasonal_mode != "deterministic":
            return
        ...
        prop = v_cur.copy()
        prop = v_cur + np.random.normal(0.0, step, size=self.period)
        ...
        if self.priors.m_season is None:
            m_first = np.zeros(self.period, float)
        else:
            m_first = np.asarray(self.priors.m_season, float)
            if m_first.size != self.period:
                raise ValueError("priors.m_season must have length = period.")
        s = float(self.priors.s_season)

        lp_old = -0.5 * np.sum(((v_cur - m_first) / s) ** 2)
        lp_new = -0.5 * np.sum(((prop   - m_first) / s) ** 2)
        if self._mh_accept((ll_new + lp_new) - (ll_old + lp_old)):
            self.season_vec = prop
    ...
    def update_m0(self) -> None:
        ...
        if self.idx_beta is not None:
            ...
            pos += 1
        if self.seasonal_mode == "dynamic" and self.n_season > 0:
            if self.priors.m_m0_gamma is None:
                m_prior = np.zeros(self.n_season, float)
            else:
                m_prior = np.asarray(self.priors.m_m0_gamma, float)
                if m_prior.size != self.n_season:
                    raise ValueError(
                        "priors.m_m0_gamma must have length = 2*K (+1 if Nyquist used)"
                    )
            s = float(self.priors.s_m0_gamma)
            for k in range(self.n_season):
    ...
    def update_P0(self) -> None:
        ...
        if self.seasonal_mode == "dynamic" and self.n_season > 0:
            ...
            a = self.priors.a_P0_gamma + 0.5 * self.n_season
    ...
    def _progress_line(self, it: int) -> str:
        ...
        if self.seasonal_mode != "none":
            if self.seasonal_mode == "dynamic" and hasattr(self, "m0_gamma"):
                ...
                head = self.season_vec
                head_show = head[:6] if len(head) > 6 else head
                head_str = "[" + ", ".join(f"{float(x):.4g}" for x in head_show) + (", …]" if len(head) > 6 else "]")
                parts.append(f"season={head_str} P0γ=0 ({pct('season')})")
    ...
    def run(self) -> Dict[str, np.ndarray]:
        ...
        if self.seasonal_mode == "dynamic":
            self.keep.update({"Q_gamma": np.zeros(n_kept),
                              "m0_gamma": np.zeros((n_kept, self.n_season)),
...
# ------------------------- CLI / Example run (harmonic) ------------------------
if __name__ == "__main__":
    import argparse
    from datetime import datetime

    # make simulator + harmonic helpers importable
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Structural DGEV state-space model with **harmonic seasonality** "
            "(cos/sin pairs + optional Nyquist). "
            "States via PGAS; log-normal priors on process SDs; "
            "Inv-Gamma prior on σ² and bounded uniform prior on ξ."
        )
    )
    ...
    parser.add_argument(
        "--trend-mode", choices=["dynamic", "deterministic", "none"], default="dynamic"
    )
    parser.add_argument(
        "--seasonal-mode", choices=["dynamic", "deterministic", "none"], default="dynamic"
    )

    # Truth / simulator params (DGEV)
    ...
    # OLD dummy-style seasonal truth for simulator (p-1 NEWEST-FIRST)
    parser.add_argument(
        "--m0-season",
        type=str,
        default=None,
        help="comma-separated NEWEST-FIRST seasonal state means for simulator (length p-1)",
    )
    parser.add_argument(
        "--v0-season",
        type=str,
        default=None,
        help="comma-separated NEWEST-FIRST seasonal state variances for simulator (length p-1)",
    )

    # --- Harmonic spec for the SAMPLER (and optional projection from dummies)
    parser.add_argument(
        "--harmonics",
        type=int,
        default=None,
        help="K harmonics used in sampler. Default: floor((period-1)/2).",
    )
    parser.add_argument(
        "--use-nyquist",
        type=int,
        default=1,
        help="1/0 to include Nyquist harmonic when period is even.",
    )

    # Option A (preferred for initialization): provide full-length seasonal dummies
    parser.add_argument(
        "--season-dummies",
        type=str,
        default=None,
        help="CSV of length=period, full seasonal pattern (optional, used only for init).",
    )

    # Option B (direct sampler init): provide harmonic coefficients directly
    parser.add_argument(
        "--m0-cos-init",
        type=str,
        default=None,
        help="CSV of length K: starting cos coefficients for sampler (overrides projection).",
    )
    parser.add_argument(
        "--m0-sin-init",
        type=str,
        default=None,
        help="CSV of length K: starting sin coefficients for sampler (overrides projection).",
    )
    parser.add_argument(
        "--m0-nyq-init",
        type=float,
        default=None,
        help="Initial Nyquist coefficient for sampler (overrides projection; only if Nyquist used).",
    )

    # Sampler initialization for dynamic m0 / P0
    parser.add_argument(
        "--init-m0-level",
        type=float,
        default=None,
        help="Sampler x0 prior mean for level (default: use --m0-level or median(y)).",
    )
    parser.add_argument(
        "--init-p0-level",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Sampler x0 prior VARIANCE for harmonic seasonal coords (one scalar).",
    )
    ...
    # (Optional) prior for deterministic seasonal (if you use it inside sampler)
    parser.add_argument(
        "--prior-m-season",
        type=str,
        default=None,
        help="Unused in harmonic sampler; kept for backward compatibility.",
    )
    parser.add_argument("--prior-s-season", type=float, default=5.0)

    # Log-normal hyperparameters for process sds ln s_*
    parser.add_argument(
        "--prior-ln-s-alpha-m",
        type=float,
        default=-1.0,
        help="Mean of ln s_alpha prior",
    )
    parser.add_argument(
        "--prior-ln-s-alpha-sd",
        type=float,
        default=2.0,
        help="SD of ln s_alpha prior",
    )
    parser.add_argument(
        "--prior-ln-s-beta-m",
        type=float,
        default=-1.0,
        help="Mean of ln s_beta prior",
    )
    parser.add_argument(
        "--prior-ln-s-beta-sd",
        type=float,
        default=2.0,
        help="SD of ln s_beta prior",
    )
    parser.add_argument(
        "--prior-ln-s-gamma-m",
        type=float,
        default=-1.0,
        help="Mean of ln s_gamma prior",
    )
    parser.add_argument(
        "--prior-ln-s-gamma-sd",
        type=float,
        default=2.0,
        help="SD of ln s_gamma prior",
    )
    ...
    parser.add_argument("--out-dir", type=str, default="results/simulations/DGEV_harm")
    ...
    alpha_T = (
        np.asarray(truths["alpha"][1 : 1 + args.T], float)
        if args.level_mode == "dynamic"
        else None
    )
    beta_T = (
        np.asarray(truths["beta"][1 : 1 + args.T], float)
        if args.trend_mode == "dynamic"
        else None
    )
    gamma_T = (
        np.asarray(truths["gamma"][1 : 1 + args.T], float)
    ...
    # override log-normal priors for process SDs
    ...
    # --- sampler x0 priors (init), decoupled from simulator truth ----------
    init_m0_level = (
        args.init_m0_level if args.init_m0_level is not None else args.m0_level
    )
    init_p0_level = (
        args.init_p0_level if args.init_p0_level is not None else args.v0_level
    )

    init_m0_trend = (
        args.init_m0_trend
        if args.init_m0_trend is not None
        else (args.m0_trend if args.trend_mode != "none" else 0.0)
    )
    init_p0_trend = (
        args.init_p0_trend if args.init_p0_trend is not None else args.v0_trend
    )

    # --- derive harmonic sampler inits from full-length dummies if provided
    m0_cos_init = _csv_floats_or_none(args.m0_cos_init)
    m0_sin_init = _csv_floats_or_none(args.m0_sin_init)
    nyq_init = args.m0_nyq_init

    season_dummies = _csv_floats_or_none(args.season_dummies)
    if season_dummies is not None:
        if len(season_dummies) != args.period:
            raise ValueError(
                f"--season-dummies must have length period={args.period}"
            )
        ...
        if nyq_init is None and use_nyq:
            nyq_init = float(0.0 if nyq_val is None else nyq_val)
    ...
    sampler = DGEVParticleGibbs(
        ...
        seasonal_mode=args.seasonal_mode,
        # x0 priors
        m0_alpha_init=float(init_m0_level),
        P0_alpha_init=float(init_p0_level),
        m0_beta_init=float(init_m0_trend),
        P0_beta_init=float(init_p0_trend),
        m0_cos_init=m0_cos_init if args.seasonal_mode != "none" else None,
        m0_sin_init=m0_sin_init if args.seasonal_mode != "none" else None,
        m0_nyq_init=float(nyq_init) if (use_nyq and args.seasonal_mode != "none") else 0.0,
        P0_harm_init=float(args.P0_harm_init)
        if args.seasonal_mode == "dynamic"
        else 0.0,
        # process SD inits
        s_alpha_init=1e-1,
        ...
        priors=priors,
        cfg=cfg,
        # deterministic level warm start (if you kept these args in the class)
        level_value_init=(
            float(args.init_m0_level) if args.level_mode == "deterministic" else 0.0
        ),
    ...
    if args.seasonal_mode == "dynamic":
        n_season = 2 * K + (1 if use_nyq else 0)
        true_Q += [args.q_season] * n_season
    ...
    if args.print_summary:
        with np.printoptions(suppress=True, precision=4):
            ...
            print(
                f"level={args.level_mode}, trend={args.trend_mode}, season={args.seasonal_mode}"
            )
            print(
                f"sigma={args.sigma}, xi={args.xi}, "
            ...
            if args.seasonal_mode == "none":
                print(
                    f"m0_season=[0.0]*(period-1) (none), v0_season=0.0 (none)"
                )
            else:
                print(f"m0_season(sim)={np.array(args.m0_season)}")
                print(f"v0_season(sim)={np.array(args.v0_season)}")
            print("\n--- Sampler x0 prior (init) ---")
            print(f"m0_alpha_init={init_m0_level}, P0_alpha_init={init_p0_level}")
            print(f"m0_beta_init={init_m0_trend}, P0_beta_init={init_p0_trend}")
            if args.seasonal_mode != "none":
                print(f"m0_cos_init={np.array(m0_cos_init)}")
                print(f"m0_sin_init={np.array(m0_sin_init)}")
                if use_nyq:
                    print(f"m0_nyq_init={nyq_init}")
                print(f"P0_harm_init={args.P0_harm_init}")
            print(
                f"period={args.period}, start={dates_T[0]}, end={dates_T[-1]}"
            )
            print(f"y mean={y.mean():.3f}, sd={y.std(ddof=1):.3f}")
            print(
                f"InvGamma prior on σ²: a={priors.a_sigma:.3g}, b={priors.b_sigma:.3g}"
            )
            print(
                f"ξ ~ Uniform[{priors.xi_lower}, {priors.xi_upper}] | "
                f"K={K}, Nyquist={use_nyq}"
            )
    ...
    out_dir = os.path.join(
        args.out_dir,
        f"{args.level_mode}-{args.trend_mode}-{args.seasonal_mode}_"
        f"K{K}_nyq{int(use_nyq)}_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
    )
    ...
    if args.print_summary:
        ...
        with np.printoptions(suppress=True, precision=4):
            ...
            print(
                f"ξ: {xi_mean:.4g}  (prior Uniform[{priors.xi_lower}, {priors.xi_upper}])"
            )
            ...
            if args.level_mode == "dynamic":
                ...
                m0a_det = (
                    float(np.mean(lvl_samples))
                    if isinstance(lvl_samples, np.ndarray) and lvl_samples.size
            ...
            if args.trend_mode == "dynamic":
                ...
                m0b_det = (
                    float(np.mean(slp_samples))
                    if isinstance(slp_samples, np.ndarray) and slp_samples.size
            ...
            if args.seasonal_mode == "dynamic":
                if "m0_gamma" in post and post["m0_gamma"].size:
                    ...
                    print(f"m0γ (harmonic): {_fmt_vec(m0g_vec)} | P0γ: {P0g:.4g}")
                else:
                    print("m0γ: n/a | P0γ: n/a")
            elif args.seasonal_mode == "deterministic":
                # depending on how you store deterministic harmonic coefs in keep[]
                print("seasonal deterministic coefficients: check m0_cos/m0_sin in posterior npz")
            else:
    ...
    if args.plot:
        ...
        plt.title(
            ...
            f"| K={K}, nyq={use_nyq}, ξ prior=[{priors.xi_lower},{priors.xi_upper}]"
        )
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
