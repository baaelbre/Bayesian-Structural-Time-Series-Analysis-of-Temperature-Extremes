...
class PCPrior:
    ...
    If lambda_s is None, we still auto-calibrate an initial λ from the data;
    after that, λ is updated via the Gamma hyperprior each MCMC iteration.
    """
...
class DGEVParticleGibbs:
    ...
    def _slice(self, f, z0: float) -> float:
        # simple stepping-out slice sampler (same defaults as your Kalman code)
        w = 0.4; m = 40; limit = 1000
    ...
    def _slice_logsd(self, z0: float, SS: float, T_eff: int, lam: float) -> float:
        # log target for z = log s:  -(T z) - 0.5*SS*e^{-2z} - lam*e^{z} + z   (Jacobian)
        def f(z: float) -> float:
    ...
    def _gibbs_lambda_single(self, which: str) -> None:
        if which == "alpha":
            ...
            self.lambda_alpha = float(
                np.random.gamma(shape=pc.a_lambda + 1.0, scale=1.0 / (pc.b_lambda + max(0.0, self.s[self.idx_alpha])))
            )
        elif which == "beta":
            ...
            self.lambda_beta = float(
                np.random.gamma(shape=pc.a_lambda + 1.0, scale=1.0 / (pc.b_lambda + max(0.0, self.s[self.idx_beta])))
            )
        elif which == "gamma":
            ...
            self.lambda_gamma = float(
                np.random.gamma(shape=pc.a_lambda + 1.0, scale=1.0 / (pc.b_lambda + max(0.0, self.s[self.idx_gamma_end])))
            )
    ...
    # ---------- Innovation sums-of-squares for each coordinate ----------
    ...
    def _innovation_ss_gamma(self) -> Tuple[float, int]:
        """
        Seasonal innovation on the LAST seasonal coord (γ_last):
            γ_last(t) ~ N( -sum(prev seasonal coords), s_γ^2 )
        """
        if self.seasonal_mode != "dynamic" or self.idx_gamma_end is None:
            return 0.0, 0
        g0, gL = self.idx_gamma_start, self.idx_gamma_end
        ss = 0.0
        for t in range(1, self.T + 1):
            prev = self.x[t - 1, g0:gL + 1]
            mean_new_last = -float(np.sum(prev))
            ss += (self.x[t, gL] - mean_new_last) ** 2
        return float(ss), self.T
    ...
    def update_process_sds(self) -> None:
        ...
        if self.idx_alpha is not None and (self.lambda_alpha is not None):
            ...
            z0 = math.log(max(1e-18, float(self.s[self.idx_alpha])))
            z = self._slice_logsd(z0, ss, T_eff, float(self.lambda_alpha))
            s_new = float(math.exp(z))
            self.s[self.idx_alpha] = s_new
            self.Q[self.idx_alpha] = s_new * s_new

        # β
        if self.idx_beta is not None and (self.lambda_beta is not None):
            ss, T_eff = self._innovation_ss_beta()
            z0 = math.log(max(1e-18, float(self.s[self.idx_beta])))
            z = self._slice_logsd(z0, ss, T_eff, float(self.lambda_beta))
            s_new = float(math.exp(z))
            self.s[self.idx_beta] = s_new
            self.Q[self.idx_beta] = s_new * s_new

        # γ_last
        if self.seasonal_mode == "dynamic" and (self.lambda_gamma is not None):
            ss, T_eff = self._innovation_ss_gamma()
            z0 = math.log(max(1e-18, float(self.s[self.idx_gamma_end])))
            z = self._slice_logsd(z0, ss, T_eff, float(self.lambda_gamma))
            s_new = float(math.exp(z))
            self.s[self.idx_gamma_end] = s_new
            self.Q[self.idx_gamma_end] = s_new * s_new
    ...
    def _progress_line(self, it: int, ema_logZ: Optional[float]) -> str:
        ...
        lam_a = (f"{self.lambda_alpha:.3g}" if self.idx_alpha is not None else "–")
        lam_b = (f"{self.lambda_beta:.3g}"  if self.idx_beta  is not None else "–")
        lam_g = (f"{self.lambda_gamma:.3g}" if self.seasonal_mode == "dynamic" else "–")
        q_block = f"| Q_α={self.Q[self.idx_alpha]:.3e}  Q_β={self.Q[self.idx_beta] if self.idx_beta is not None else 0.0:.3e}  Q_γ={self.Q[self.idx_gamma_end] if self.seasonal_mode == 'dynamic' else 0.0:.3e} | λ=({lam_a},{lam_b},{lam_g})"
    ...
    def run(self) -> Dict[str, np.ndarray]:
        ...
        for it in range(cfg.n_iter):
            ...
            if self.dim > 0:
                ...
                # PC hyperprior on λ (Gamma); skipped if a fixed λ was supplied
