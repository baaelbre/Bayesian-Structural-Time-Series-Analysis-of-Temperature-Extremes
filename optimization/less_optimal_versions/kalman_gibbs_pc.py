...
@dataclass
class PCPrior:
    """PC prior for an innovation sd s>0: p(s) = λ exp(-λ s).
       If lambda_s is None, calibrate from data using:
...
class Priors:
    ...
    # PC priors for dynamic state-noise sds (s = sqrt(q))
...
class DLMGibbs:
    ...
    FFBS samples latent x_{0:T}; RW–MH updates:
      ...
      - process sds s_alpha, s_beta, s_gamma with PC priors on s (exp-rate λ)
    """
    ...
    def __init__(
        ...
        # ---- Auto-calibrate PC lambdas from data
        self.lambda_alpha, self.lambda_beta, self.lambda_gamma = self._auto_pc_lambdas()
    ...
    def _auto_pc_lambdas(self) -> Tuple[Optional[float], Optional[float], Optional[float]]:
        """Return (lambda_alpha, lambda_beta, lambda_gamma) using data-scaled defaults unless provided."""
        y = self.y
        sd1 = _robust_sd(np.diff(y)) if y.size >= 2 else 0.0      # for alpha
        sd2 = _robust_sd(np.diff(y, n=2)) if y.size >= 3 else 0.0 # for beta
        sdg = sd1                                                 # proxy for seasonal closure
        ...
        lam_a = _calc_lambda(self.priors.pc_alpha, sd1) if self.idx_alpha is not None else None
        lam_b = _calc_lambda(self.priors.pc_beta,  sd2) if self.idx_beta  is not None else None
        lam_g = _calc_lambda(self.priors.pc_gamma, sdg) if self.seasonal_mode == "dynamic" else None
        if self.cfg.progress:
            la = f"{lam_a:.4g}" if lam_a is not None else "n/a"
            lb = f"{lam_b:.4g}" if lam_b is not None else "n/a"
            lg = f"{lam_g:.4g}" if lam_g is not None else "n/a"
            print(f"Auto-calibrated PC lambdas: λ_alpha={la}, λ_beta={lb}, λ_gamma={lg}")
        return lam_a, lam_b, lam_g

    # ----------------------------- Helpers ---------------------------------- #
    def _H_t(self) -> np.ndarray:
        """Observation selector for dynamic state at time t (time-invariant)."""
        if self.dim == 0:
            return np.zeros((1, 0))
    ...
    def _G_t(self) -> np.ndarray:
        ...
        # alpha drift from beta handled by G
        ...
        if self.seasonal_mode == "dynamic":
            ...
            for k in range(ge - gs):
                G[gs + k, gs + k] = 0.0
                G[gs + k, gs + k + 1] = 1.0
            G[ge, gs:ge + 1] = 0.0  # last row handled via u_t closure term
        return G

    def _u_t(self, x_prev: np.ndarray) -> np.ndarray:
        """Deterministic part of transition: deterministic slope + seasonal closure."""
        if self.dim == 0:
            return np.zeros(0)
        ...
        # seasonal closure: new last coord mean = -sum(prev)
        if self.seasonal_mode == "dynamic":
            gs, ge = self.idx_g_start, self.idx_g_end
            prev = x_prev[gs:ge + 1]
    ...
    def _Q_mat(self) -> np.ndarray:
        ...
        if self.idx_alpha is not None and self.s_alpha > 0.0:
            Q[self.idx_alpha, self.idx_alpha] = self.s_alpha ** 2
        if self.idx_beta is not None and self.s_beta > 0.0:
            Q[self.idx_beta, self.idx_beta] = self.s_beta ** 2
        if self.seasonal_mode == "dynamic" and self.s_gamma > 0.0:
    ...
    def _ffbs(self) -> np.ndarray:
        """Sample full dynamic state trajectory given params."""
        if self.dim == 0:
            return self.x.copy()
        ...
        for t in range(self.T - 1, -1, -1):
            ...
            cov  = C[t] - J @ Rm[t + 1] @ J.T         # <-- correct conditional covariance
            # numerical guard
            cov = 0.5 * (cov + cov.T)
            eig = np.linalg.eigvalsh(cov)
            if eig.min() <= 0:
                cov += (1e-10 - eig.min()) * np.eye(cov.shape[0])
            x[t] = np.random.multivariate_normal(mean, cov)

        return x

    def _propagate_initial_path(self, Q_init: np.ndarray) -> None:
        """Tiny forward simulation to create a plausible initial path."""
        if self.dim == 0:
            return
        G = self._G_t()
        for t in range(1, self.T + 1):
    ...
    def update_logsigma(self) -> None:
        ...
        sigma2_cur = float(np.exp(2.0 * cur))
        sigma2_prop = float(np.exp(2.0 * prop))

        mu_vec = self._mu_vec_current()
        ll_old = self._gaussian_loglike(mu_vec)  # uses self.sigma2
        # Temporarily compute LL at proposed sigma^2
        e = self.y - mu_vec
        ll_new = float(-0.5 * self.T * (math.log(2.0 * math.pi * sigma2_prop)) - 0.5 * np.sum(e * e) / sigma2_prop)

        self.proposals["logsigma"] += 1
        # Prior on logsigma (normal)
    ...
    # ---- PC prior MH for a single sd s = exp(z)
    def _update_log_sd_single(self, key: str, get_s: callable, set_s: callable, lambda_s: Optional[float]) -> None:
        if lambda_s is None:
            return
        ...
        # PC prior: log p(s) = -λ s (no Jacobian since we parameterize in z and include in ll)
        # include Jacobian term (+z)
    ...
    def update_process_sds(self) -> None:
        if self.idx_alpha is not None:
            self._update_log_sd_single("log_s_alpha", lambda: self.s_alpha, lambda v: setattr(self, "s_alpha", v), self.lambda_alpha)
        if self.idx_beta is not None:
            self._update_log_sd_single("log_s_beta",  lambda: self.s_beta,  lambda v: setattr(self, "s_beta", v),  self.lambda_beta)
        if self.seasonal_mode == "dynamic":
            self._update_log_sd_single("log_s_gamma", lambda: self.s_gamma, lambda v: setattr(self, "s_gamma", v), self.lambda_gamma)
    ...
    def run(self) -> Dict[str, np.ndarray]:
        ...
        for it in range(cfg.n_iter):
            ...
            # 1) Sample dynamic states via FFBS (exact, given current params)
            if self.dim > 0:
                self.x = self._ffbs()

            # 2) Update process sds (PC priors, RW–MH)
            if self.dim > 0:
                if cfg.progress: print("  Updating process sds (PC prior)...")
                self.update_process_sds()

            # 3) Update deterministic structural params (RW–MH with Normal priors)
            if self.level_mode == "deterministic":
            ...
            if self.trend_mode == "deterministic":
                ...
                self.update_slope_value()
            if self.seasonal_mode == "deterministic":
                if cfg.progress: print("  Updating deterministic seasonality...")
                self.update_season_vec()

            # 4) Update observation variance via logsigma MH (Normal prior)
            if cfg.progress: print("  Updating logsigma ...")
            self.update_logsigma()

            # --- adapt proposal scales if enabled ---
            self._adapt_steps(it)

            # 5) compact progress line (like DGEV)
            if cfg.progress and ((it + 1) % print_every == 0 or it == cfg.n_iter - 1):
                ...
                print(
                    ...
                    f"σ={math.exp(self.logsigma):.3f} "
                    f"{pc_info}{det_info}{q_info}"
                )

                # Early warnings
                if self.dim > 0:
                    Q_now = self._Q_mat()
                    if np.any(np.diag(Q_now) < 1e-14):
                        bad = np.where(np.diag(Q_now) < 1e-14)[0].tolist()
                        print(f"  [warn] Q extremely small at diag idx {bad}.")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Gaussian DLM (FFBS) with RW–MH and PC priors (DLMGibbs)")
    # Modes
    ...
    # Priors: Normal on logsigma & deterministic params; PC on sds
