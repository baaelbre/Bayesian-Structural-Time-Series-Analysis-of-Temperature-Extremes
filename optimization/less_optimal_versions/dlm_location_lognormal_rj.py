...
(Log-Normal priors on process SDs; slice sampling on log s).
...
class Slice1D:
    ...
    def sample(self, z0: float, n: int = 1) -> np.ndarray:
        ...
        for i in range(n):
            hz = self.h(z)
            # vertical level: y = log u + h(z)
            y = hz + math.log(self.rng.uniform())
            # stepping-out
            w = self.w
            L = z - w * self.rng.uniform()
            R = L + w
...
# =============================================================================
# Priors & Config (LOG-NORMAL on process SDs)
# =============================================================================
...
class Priors:
    ...
    mu_log_s_alpha: float = -2.0
    sd_log_s_alpha: float = 0.7
    mu_log_s_beta:  float = -3.0
    sd_log_s_beta:  float = 0.7
    mu_log_s_gamma: float = -4.0
...
class DLMRJGibbs:
    """Gaussian structural DLM with level/trend/season blocks and RJ–MCMC (Log-Normal prior on process SDs)."""
    ...
    def __init__(
        ...
        # process SDs (dynamic blocks only)
        ...
        # slice samplers for z = log s (built via helper so we can rebuild after RJ)
        self._slice_alpha = None
        self._slice_beta  = None
        self._slice_gamma = None
        self._rebuild_slice_samplers()
    ...
    @staticmethod
    def _log_prior_lognormal_s(s: float, mu: float, var: float) -> float:
        """Log prior for s ~ LogNormal(mu, var); constant terms dropped."""
        s = max(float(s), 1e-300)
        z = math.log(s)
        return -0.5 * ((z - mu) ** 2) / var - z  # Jacobian
    ...
    def _log_prior_current(self) -> float:
        ...
        # dynamic level
        if self._layout.idx_alpha is not None:
            lp += self._log_prior_lognormal_s(self.s_alpha, self.priors.mu_log_s_alpha, self.priors.sd_log_s_alpha**2)
        ...
        # dynamic trend
        if self._layout.idx_beta is not None:
            lp += self._log_prior_lognormal_s(self.s_beta, self.priors.mu_log_s_beta, self.priors.sd_log_s_beta**2)
        ...
        # dynamic seasonal
        if self.seasonal_mode == "dynamic":
            lp += self._log_prior_lognormal_s(self.s_gamma, self.priors.mu_log_s_gamma, self.priors.sd_log_s_gamma**2)
            base = (np.zeros(self.period - 1)
                    if self.priors.m_m0_gamma is None
                    else np.asarray(self.priors.m_m0_gamma, float))
    ...
    # h(z) = -T z - 0.5 * SS * exp(-2z) - (z - mu)^2/(2v)   (const dropped)
    @staticmethod
    def _h_lognorm(z: float, SS: float, T_eff: int, mu: float, var: float) -> float:
    ...
    def _innovation_ss_gamma(self) -> Tuple[float, int]:
        ...
        prev = self.x[:-1, gs: ge + 1]
        new_first = self.x[1:, gs]
        mean_first = -np.sum(prev, axis=1)
        d = new_first - mean_first
        return float(d @ d), self.T

    def _ln_params_alpha(self) -> Tuple[float, int, float, float]:
        SS, T_eff = self._innovation_ss_alpha()
        return SS, T_eff, float(self.priors.mu_log_s_alpha), float(self.priors.sd_log_s_alpha) ** 2

    def _ln_params_beta(self) -> Tuple[float, int, float, float]:
        SS, T_eff = self._innovation_ss_beta()
        return SS, T_eff, float(self.priors.mu_log_s_beta), float(self.priors.sd_log_s_beta) ** 2

    def _ln_params_gamma(self) -> Tuple[float, int, float, float]:
        SS, T_eff = self._innovation_ss_gamma()
        return SS, T_eff, float(self.priors.mu_log_s_gamma), float(self.priors.sd_log_s_gamma) ** 2

    def _rebuild_slice_samplers(self) -> None:
        """(Re)build slice samplers for currently dynamic blocks."""
        self._slice_alpha = None
        self._slice_beta  = None
        self._slice_gamma = None
        if self._layout.idx_alpha is not None:
            self._slice_alpha = Slice1D(lambda z: self._h_lognorm(z, *self._ln_params_alpha()),
                                        w=self.cfg.slice_w, m=self.cfg.slice_m, rng=self.rng)
        if self._layout.idx_beta is not None:
    ...
    def update_process_Q_lognormal_slice(self) -> None:
        # α
        if self._layout.idx_alpha is not None and self._slice_alpha is not None:
            z0 = math.log(max(self.s_alpha, 1e-12))
            z  = float(self._slice_alpha.sample(z0, 1)[0])
            self.s_alpha = float(np.exp(z))
        # β
        if self._layout.idx_beta is not None and self._slice_beta is not None:
            z0 = math.log(max(self.s_beta, 1e-12))
            z  = float(self._slice_beta.sample(z0, 1)[0])
            self.s_beta = float(np.exp(z))
        # γ
        if self.seasonal_mode == "dynamic" and self._slice_gamma is not None:
    ...
    def update_sigma2(self) -> None:
        ...
        tau = self.rng.gamma(shape=a, scale=1.0 / max(b,1e-300))
    ...
    def update_P0(self) -> None:
        ...
        if self._layout.idx_alpha is not None:
            ...
            self.P0_alpha = 1.0 / self.rng.gamma(a, 1.0 / max(b,1e-300)); pos += 1
        if self._layout.idx_beta is not None:
            a = self.priors.a_P0_beta + 0.5
            b = self.priors.b_P0_beta + 0.5 * (float(self.x[0, pos]) - self.m0_beta) ** 2
            self.P0_beta = 1.0 / self.rng.gamma(a, 1.0 / max(b,1e-300)); pos += 1
        if self.seasonal_mode == "dynamic":
            diffsq = 0.0
            for k in range(self.period - 1):
            ...
            self.P0_gamma = 1.0 / self.rng.gamma(a, 1.0 / max(b,1e-300))
    ...
    def _draw_prior_dyn_block(self, which: str) -> None:
        # Draw s from LN prior; draw m0 and P0 from their priors.
        if which == "level":
            z = self.rng.normal(self.priors.mu_log_s_alpha, self.priors.sd_log_s_alpha)
            self.s_alpha = float(np.exp(z))
            ...
            z = self.rng.normal(self.priors.mu_log_s_beta, self.priors.sd_log_s_beta)
            self.s_beta = float(np.exp(z))
            ...
            z = self.rng.normal(self.priors.mu_log_s_gamma, self.priors.sd_log_s_gamma)
            self.s_gamma = float(np.exp(z))
            base = (np.zeros(self.period - 1)
                    if self.priors.m_m0_gamma is None
                    else np.asarray(self.priors.m_m0_gamma, float))
    ...
    def _apply_modes_and_layout(self, new_level: str, new_trend: str, new_season: str) -> None:
        """Apply modes, rebuild layout, reset bits that depend on modes, and rebuild slice samplers."""
        ...
        if self.seasonal_mode == "deterministic":
            ...
            self._m0_gamma_full = None
        # states holder consistent with layout
        self._alloc_state_holder()
        # critical: rebuild slice samplers whenever modes/layout change
        self._rebuild_slice_samplers()
    ...
    def _rj_move_one(self) -> None:
        ...
        # apply proposal to compute proposed log posterior
        self._apply_modes_and_layout(new_level, new_trend, new_season)
        logpost_prop = self._safe_logpost()

        # restore current for MH ratio
        self._load_snapshot(cur_snap)
        logpost_cur = self._safe_logpost()

        if not np.isfinite(logpost_prop) or not np.isfinite(logpost_cur):
            self._rj_record(comp, False)
            # ensure samplers match the *current* modes after restore
            self._rebuild_slice_samplers()
            return
        ...
        if accept:
            ...
            # stay at current; make sure samplers match current modes
            self._rebuild_slice_samplers()
    ...
    def run(self) -> Dict[str, np.ndarray]:
        ...
        for it in range(cfg.n_iter):
            if self._layout.dim > 0:
                self.x = self._ffbs()
                # ensure samplers are valid (e.g., after previous RJ accept)
                if (self._layout.idx_alpha is not None and self._slice_alpha is None) \
                   or (self._layout.idx_beta is not None and self._slice_beta is None) \
                   or (self.seasonal_mode == "dynamic" and self._slice_gamma is None):
                    self._rebuild_slice_samplers()
                self.update_process_Q_lognormal_slice()
                self.update_m0()
                self.update_P0()
...
if __name__ == "__main__":
    ...
    p.add_argument("--q-season", type=float, default=0.1)
    ...
    # Priors (incl. Log-Normal on process SDs)
    ...
    if args.print_summary:
        ...
        print("Log-Normal priors on process SDs:")
        print(f"  alpha: log s ~ N({priors.mu_log_s_alpha:.3f}, {priors.sd_log_s_alpha**2:.3f})")
        print(f"  beta : log s ~ N({priors.mu_log_s_beta :.3f}, {priors.sd_log_s_beta **2:.3f})")
        print(f"  gamma: log s ~ N({priors.mu_log_s_gamma:.3f}, {priors.sd_log_s_gamma**2:.3f})\n")
        print("Model priors (normalized):")
    ...
    if args.plot:
        ...
        plt.title(
            "Structural DLM (LN priors + RJ) - current modes: "
