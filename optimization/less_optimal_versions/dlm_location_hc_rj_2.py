...
class DLMRJGibbs:
    ...
    RJ–MCMC toggles block modes; acceptance via Δ log posterior:
      Δ logpost = Δ loglik + Δ log prior(params|model) + Δ log model prior.
    (Proposal/Jacobian cancel: new params drawn from priors; deaths drop them.)
    ...
    def __init__(
        ...
        # model prior weights (Occam tilt)
        ...
        # model priors (default: slight preference for deterministic seasonality)
        ...
        # initial state means/vars for dynamic blocks; also serve as deterministic params when non-dynamic
        self.m0_alpha = float(m0_alpha_init) if self.level_mode == "dynamic" else float(self.priors.m_m0_alpha)
        self.P0_alpha = float(P0_alpha_init) if self.level_mode == "dynamic" else 0.0
        self.m0_beta  = float(m0_beta_init)  if self.trend_mode == "dynamic" else float(self.priors.m_m0_beta)
    ...
    def _u(self) -> np.ndarray:
        ...
        # deterministic slope enters α-drift if α is dynamic
    ...
    def _mu_det_t(self, t: int) -> float:
        ...
        # if level is dynamic and trend deterministic, slope is in state drift via _u.
        # if level is NOT dynamic and trend deterministic, slope enters obs mean directly:
    ...
    def _log_prior_current(self) -> float:
        """
        Sum of log priors for the *currently active* parameters + model prior.
        Constants are dropped (cancel in RJ diffs).
        """
        lp = 0.0

        def lIG(x, shape, scale):
            # Inv-Gamma(x; shape α, scale β): proportional to x^{-(α+1)} * exp(-β/x)
            x = max(float(x), 1e-300)
            return -(shape + 1.0) * math.log(x) - (scale / x)

        # observation variance via precision tau ~ Gamma(a,b)
        ...
        # dynamic level block
        if self.idx_alpha is not None:
            Qa = max(self.s_alpha**2, 1e-300); aa = max(self._a_alpha, 1e-300)
        ...
        # dynamic trend block
        if self.idx_beta is not None:
            Qb = max(self.s_beta**2, 1e-300); ab = max(self._a_beta, 1e-300)
        ...
        # dynamic seasonal block
        if self.seasonal_mode == "dynamic":
            Qg = max(self.s_gamma**2, 1e-300); ag = max(self._a_gamma, 1e-300)
        ...
        # deterministic params (use m0_* priors only)
        if self.level_mode == "deterministic":
            lp += -0.5 * ((self.m0_alpha - self.priors.m_m0_alpha)**2) / (self.priors.s_m0_alpha**2)
    ...
    def update_deterministic_params(self) -> None:
        ...
        # TREND (deterministic): m0_beta
        if self.trend_mode == "deterministic":
            if self.idx_alpha is not None:
                # β explains alpha increments
    ...
    def _load_snapshot(self, S: dict) -> None:
        ...
        # rebuild seasonal full vector if needed
        if self.seasonal_mode == "deterministic":
            mg = np.asarray(self.m0_gamma, float)
            self._m0_gamma_full = np.r_[mg, -mg.sum()]
        else:
    ...
    def _maybe_print_switch(self, it: int, before: Tuple[str,str,str], after: Tuple[str,str,str],
                            block: str, dlp: float, accepted: bool) -> None:
        ...
        msg = (f"[switch @ it {it+1}] block={block} | "
               f"{' '.join(changed)} | Δlogpost={dlp:+.4f} | RJ {self._fmt_rj_all()}")
        print(msg)
    ...
    def _rj_move_one(self, it: int) -> None:
        ...
        # births: draw proposed dynamic block params from priors
        ...
        # if proposing deterministic season, ensure m0_gamma defaults exist
        ...
        lp_prop = self._log_prior_current()
        logpost_prop = ll_prop + lp_prop

        # current log posterior
        self._load_snapshot(cur_snap)
        ll_cur  = self._kalman_loglik()
        lp_cur  = self._log_prior_current()
        logpost_cur = ll_cur + lp_cur

        dlogpost = logpost_prop - logpost_cur
        accept = (np.log(np.random.rand()) < dlogpost)
        ...
        if accept:
            ...
            self._rebuild_layout()
            self._alloc_state_holder()  # new dim holder

            after = (self.level_mode, self.trend_mode, self.seasonal_mode)
            self._maybe_print_switch(it, before, after, comp, dlogpost, accepted=True)
    ...
    def run(self) -> Dict[str, np.ndarray]:
        ...
        for it in range(cfg.n_iter):
            ...
            # initial means/vars (dynamic blocks)
            if self.dim > 0:
                self.update_m0()
                self.update_P0()

            # deterministic params (write into m0_*)
            self.update_deterministic_params()
            # observation variance
            self.update_sigma2()

            # RJ toggles (Δ log posterior based)
...
# ------------------------- CLI / Example run (RJ + Gibbs; Δ log posterior) ------------------ #
if __name__ == "__main__":
    ...
    # assumes DLMRJGibbs and Priors, SamplerConfig defined above
    ...
    def _csv_model_prior_block(s: str | None, allow_none: bool, defaults: dict) -> dict:
        """
        Parse 'dynamic:0.5,deterministic:0.5,none:0.1' into a dict; normalize.
        If allow_none=False, forces tiny mass on 'none' to avoid log(0).
        """
        out = dict(defaults)
        if s:
    ...
    p = argparse.ArgumentParser(
        description=(
            ...
            "Data are simulated via simulator.mean_time_series.Mean_Time_Series.\n"
            "NOTE: When a block is non-dynamic, its m0_* parameter is treated as the deterministic parameter."
        )
    )
    ...
    p.add_argument("--m0-level", type=float, default=3.0, help="Intercept for level; deterministic if level non-dynamic")
    p.add_argument("--v0-level", type=float, default=0.05)
    p.add_argument("--m0-trend", type=float, default=0.015, help="Slope; deterministic if trend non-dynamic")
    p.add_argument("--v0-trend", type=float, default=0.05)
    p.add_argument("--m0-season", type=str, default=None, help="CSV of length (period-1); deterministic if season non-dynamic")
    p.add_argument("--v0-season", type=str, default=None, help="CSV of length (period-1)")
    ...
    # Half-Cauchy scales for process SDs (A_k).
    ...
    p.add_argument("--prior-model-level",  type=str, default=None,
                   help="CSV 'dynamic:0.5,deterministic:0.5,none:0.0' (normalized internally)")
    ...
    sampler = DLMRJGibbs(
        ...
        priors=priors, cfg=cfg, model_prior=model_prior,
    )
    ...
    if args.print_summary:
        ...
        # If dynamic blocks were present during saved iterations, print their m0/P0 posterior means
        for key_m0, key_P0 in (("m0_alpha", "P0_alpha"), ("m0_beta", "P0_beta")):
            if key_m0 in post and key_P0 in post:
                print(f"{key_m0}: {np.mean(post[key_m0]):.4g} | {key_P0}: {np.mean(post[key_P0]):.4g}")
        if "m0_gamma" in post:
            mg = np.mean(post["m0_gamma"], axis=0)
            print(f"m0_gamma (mean): {mg}")
        ...
        # RJ acceptance (cumulative)
        print("\n--- RJ acceptance rates (cumulative) ---")
        for b in ("level", "trend", "season"):
            prop = max(1, int(sampler.rj_stats[b]["proposed"]))
            acc  = int(sampler.rj_stats[b]["accepted"])
            print(f"{b:6s}: {acc}/{prop}  ({100.0*acc/prop:.1f}%)")
    ...
    # Also save inclusion probabilities and RJ stats to JSON
    inc = sampler.inclusion_probabilities()
