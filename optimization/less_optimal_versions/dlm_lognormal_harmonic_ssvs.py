...
"""
Harmonic Gaussian DLM with **non-centered SSVS (Frühwirth-Schnatter)** on u=ln s
and **dimension-reducing FFBS** (only δ=1 blocks are dynamic).

Key points
----------
• Time is **centered** for deterministic trend stability.
• σ² is updated **collapsed** w.r.t. deterministic regressors.
• SSVS on u with slice sampling; δ updated via spike–slab odds.
• FIX: no deterministic-trend drift in α-transition or α-innovation SS.
"""
...
class HarmonicDLM_SSVS_NC_Reduced:
    ...
    def _Q(self, dyn_names: List[str], idx: Dict[str,int]) -> np.ndarray:
        ...
        s_alpha = math.exp(self.u_alpha); s_beta = math.exp(self.u_beta); s_gamma = math.exp(self.u_gamma)
    ...
    def _u_drift(self, dyn_names: List[str], idx: Dict[str,int]) -> np.ndarray:
        # FIX: with centered time, deterministic trend stays in μ_det only.
    ...
    def _mu_det_t(self, t: int) -> float:
        ...
        if self.delta_beta==0:  out += self.m0_beta * self.tc[t]   # centered time!
    ...
    def _innov_ss_alpha(self, x_dyn: np.ndarray, dyn_names: List[str], idx: Dict[str,int]) -> Tuple[float,int]:
        ...
        ss = 0.0
        for t in range(1, self.T+1):
            # FIX: if β is deterministic, trend is handled in μ_det, not here.
            if "beta" in idx:
    ...
    # ===================== SSVS updates on u and deltas =====================

    def _logpost_u(self, u: float, SS: float, n: int, m: float, v: float) -> float:
        return (-n*u - 0.5*SS*math.exp(-2.0*u)) - 0.5*((u-m)**2)/max(v,1e-300)

    def _update_u_delta(self, u0: float, SS: float, n: int, prior: SSVSLogScalePrior, rng) -> Tuple[float,int]:
        v_cur = self._v_for_current_delta  # set by caller
        ...
        p1 = 1.0 / (1.0 + math.exp(-max(min(log1 - log0, 50.0), -50.0)))
    ...
    def update_nc_ssvs(self, x_dyn: np.ndarray, dyn_names: List[str], idx: Dict[str,int]) -> None:
        ...
        self._v_for_current_delta = self.priors.ssvs_u_alpha.v1 if (self.delta_alpha==1) else self.priors.ssvs_u_alpha.v0
        self.u_alpha, self.delta_alpha = self._update_u_delta(self.u_alpha, SS, n, self.priors.ssvs_u_alpha, rng)

        # β
        SS, n = self._innov_ss_beta(x_dyn, dyn_names, idx)
        self._v_for_current_delta = self.priors.ssvs_u_beta.v1 if (self.delta_beta==1) else self.priors.ssvs_u_beta.v0
        self.u_beta,  self.delta_beta  = self._update_u_delta(self.u_beta, SS, n, self.priors.ssvs_u_beta, rng)
        ...
        self._v_for_current_delta = self.priors.ssvs_u_gamma.v1 if (self.delta_gamma==1) else self.priors.ssvs_u_gamma.v0
        self.u_gamma, self.delta_gamma = self._update_u_delta(self.u_gamma, SS, n, self.priors.ssvs_u_gamma, rng)
    ...
    def _update_sigma2_collapsed(self, x_dyn: np.ndarray, dyn_names: List[str], idx: Dict[str,int]) -> None:
        ...
        # Build deterministic design Z and prior N(m0,S0)
        cols = []
        m0_list = []
        s2_list = []
        ...
        # trend deterministic (centered time)
        if self.delta_beta==0:
            cols.append(self.tc)
        ...
        if len(cols) == 0:
            # no deterministic part → standard IG update
            e = r
        ...
        # Normal–Gamma collapse:
        # A0=S0^{-1}, A0N=A0+Z'Z, mN=A0N^{-1}(A0 m0 + Z'r)
        # aN=a0+T/2, bN=b0 + 0.5*( r'r + m0'A0 m0 - mN'A0N mN )
        A0 = np.linalg.inv(S0)
        A0N = A0 + Z.T @ Z
        rhs = A0 @ m0 + Z.T @ r
    ...
    def run(self) -> Dict[str,np.ndarray]:
        ...
        self.keep = {
            ...
            "x": np.zeros((n_kept, self.T, self.full_dim))  # projected path (NaN for deterministic)
        }
...
# ---------------- CLI (kept compact; mirror your options) ----------------
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(description="Harmonic DLM with NC-SSVS and reduced-dimension FFBS.")
    # sim
    ...
    p.add_argument("--q-season", type=float, default=1e-3)
    ...
    p.add_argument("--u-alpha-pi", type=float, default=0.99)
    ...
    p.add_argument("--u-beta-pi", type=float, default=0.99)
    ...
    p.add_argument("--u-gamma-pi", type=float, default=0.99)
    ...
    if args.plot:
        ...
        plt.title(f"Harmonic DLM + NC-SSVS (reduced FFBS) | K={sampler.K}, nyq={sampler.use_nyq}")
