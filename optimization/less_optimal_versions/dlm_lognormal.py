...
# =============================================================================
# DLM Sampler (conjugate Gibbs + SLICE for log-normal process SDs)
# =============================================================================
...
class DLMGibbsConjugate:
    ...
    Gaussian structural DLM with:
      ...
      • Log-Normal priors on process SDs s_k, sampled via univariate slice sampling on z_k = log s_k.
    ...
    # ---------- Log-posterior for z = log s (LN prior) ---------- #
    ...
    # --------------- Slice updates for process SDs --------------- #
    def update_process_Q_lognormal_slice(self) -> None:
        # α
        if self.idx_alpha is not None:
            SS, T_eff, mu0, v0 = self._ln_params_alpha()
    ...
    def run(self) -> Dict[str, np.ndarray]:
        ...
        for it in range(cfg.n_iter):
            ...
            # 2) process SDs via slice sampling on z = log s
            if self.dim > 0:
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(
        description=(
            ...
            "Process SDs have Log-Normal priors and are updated via SLICE sampling (stepping-out)."
        )
    )
    ...
    p.add_argument("--ln-mu-alpha", type=float, default=-2)
    p.add_argument("--ln-sd-alpha", type=float, default=0.7)
    p.add_argument("--ln-mu-beta",  type=float, default=-3)
    ...
    if args.print_summary:
        ...
        print("Log-Normal priors for process SDs (s_k):")
    ...
    if args.plot:
        ...
        plt.title(f"DLM (LN priors + SLICE) {args.level_mode}/{args.trend_mode}/{args.seasonal_mode}")
