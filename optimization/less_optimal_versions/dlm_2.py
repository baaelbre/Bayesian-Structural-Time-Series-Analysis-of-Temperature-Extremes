...
class Priors:
    ...
    # Baselines (used in centred-time regression update; see update_theta)
    m0_alpha: float = 0.0
    P0_alpha: float = 10.0
    m0_beta: float = 0.0
    P0_beta: float = 10.0
...
class DLMGibbsConjugate:
    """
    Gaussian structural DLM (dynamic/dynamic/dynamic) with NCP + Bayesian lasso
    on signed process SDs.
    ...
    def _build_Q_tilde(self) -> np.ndarray:
        ...
        Unit innovations in NCP:
          tilde_alpha, tilde_beta, and seasonal block have unit noise; A is deterministic.
        """
        d = self.dim_ncp
    ...
    def update_sigma2(self) -> None:
        ...
        Prior part for signed s_k is Gaussian given τ_k:
          s_k | τ_k, σ² ~ N(0, σ² τ_k)  => contributes s_k^2 / τ_k to SSR-like term.
        """
    ...
    def update_theta(self) -> None:
        ...
        Joint Gaussian update for:
          θ_c = (α_c, β, γ0, s_alpha, s_beta, s_gamma)
        using centred time t_c = t - t̄:

          y_t = α_c + β * t_c + S[t,:]γ0
                + s_alpha*tilde_alpha_t + s_beta*A_t + s_gamma*tilde_g1_t + eps_t

        Priors (diagonal):
          α_c ~ N(m0_alpha + m0_beta*t̄, P0_alpha)
          β   ~ N(m0_beta,               P0_beta)
          γ0  ~ N(m0_gamma,              P0_gamma I)
          s_k ~ N(0, σ² τ_k)
        ...
        # prior mean
        m0 = np.zeros(d, float)
        m0[0] = self.priors.m0_alpha + self.priors.m0_beta * self._tbar
        ...
        s2[k0 + 0] = max(self.sigma2 * self.tau_alpha, eps)
        s2[k0 + 1] = max(self.sigma2 * self.tau_beta, eps)
        s2[k0 + 2] = max(self.sigma2 * self.tau_gamma, eps)
        ...
        prec = XtX / self.sigma2 + np.diag(inv_s2)
        prec = symmetrize(prec) + 1e-12 * np.eye(d)

        I = np.eye(d)
        cov = spd_solve(prec, I, jitter=1e-12)  # ~= inv(prec)
        cov = symmetrize(cov)
        mean = cov @ (Xty / self.sigma2 + inv_s2 * m0)
