...
class DLMRJGibbsHarmonic:
    ...
    RJ acceptance is *corrected*:
      α = min{1, exp(Δ log posterior + log q(prop→cur) − log q(cur→prop)) }
    with explicit legality-filtered proposal probabilities.
    """
    ...
    # --------------------------- Truth overlays --------------------------- #
    def set_truth_paths(self, mu=None, sigma=None, Q=None, alpha=None, beta=None, gamma=None) -> None:
        if mu is not None: self.true_mu_t = np.asarray(mu, float)
        if sigma is not None: self.true_sigma = float(sigma)
        if Q is not None: self.true_Q = np.asarray(Q, float)
    ...
    def _legal_alternatives_for(self, block: str) -> List[str]:
        cur = {"level": self.level_mode, "trend": self.trend_mode, "season": self.seasonal_mode}[block]
        cand_all = ["dynamic", "deterministic"] + (["none"] if getattr(self.cfg, f"allow_none_{'season' if block=='season' else block}") else [])
        # Try each candidate and keep only legal ones
        legal = []
        for m in cand_all:
            if m == cur: continue
            L, T, S = self.level_mode, self.trend_mode, self.seasonal_mode
            if block == "level":   L = m
            elif block == "trend": T = m
            else:                  S = m
            if self._legal_modes(L, T, S):
                legal.append(m)
        return legal
    ...
    def _mh_accept(self, dlog: float) -> bool:
        if not np.isfinite(dlog): return False
        if dlog >= 0: return True
        return (math.log(self.rng.uniform()) < dlog)
    ...
    def _draw_prior_dyn_block(self, which: str) -> None:
        # Draw from priors (so proposal density cancels prior in RJ ratio)
        if which == "level":
            A = float(self.priors.hc_scale_alpha)
    ...
    def _apply_modes(self, L: str, T: str, S: str) -> None:
        self.level_mode, self.trend_mode, self.seasonal_mode = L, T, S
    ...
    # --------- RJ kernel with explicit Hastings correction ---------

    def _rj_move_one(self) -> None:
        blocks = ["level", "trend", "season"]
        block = str(self.rng.choice(blocks))

        # Which alternatives are legal (given the *current* other blocks)?
        legal_alts = self._legal_alternatives_for(block)
        if not legal_alts:
            self._rj_record(block, False)
            return

        # q(current -> proposed)
        q_block_fwd = math.log(1.0 / len(blocks))
        q_mode_fwd  = math.log(1.0 / len(legal_alts))
        proposed_mode = str(self.rng.choice(legal_alts))

        # form proposed triple
        L, T, S = self.level_mode, self.trend_mode, self.seasonal_mode
        if block == "level":   L = proposed_mode
        elif block == "trend": T = proposed_mode
        else:                  S = proposed_mode

        # Double-check legality
        if not self._legal_modes(L, T, S):
            self._rj_record(block, False)
            return

        # Take a snapshot and draw birth params if needed
        cur_snap = self._snapshot()

        # birth: switch -> dynamic
        if block == "level" and self.level_mode != "dynamic" and L == "dynamic":
            self._draw_prior_dyn_block("level")
        if block == "trend" and self.trend_mode != "dynamic" and T == "dynamic":
            self._draw_prior_dyn_block("trend")
        if block == "season" and self.seasonal_mode != "dynamic" and S == "dynamic":
            self._draw_prior_dyn_block("season")

        # if becoming deterministic season, set defaults
        if block == "season" and S == "deterministic":
            self._ensure_det_season_defaults()
            self.P0_harm = 0.0

        # Apply proposed, compute log posterior
        self._apply_modes(L, T, S)
        logpost_prop = self._safe_logpost()

        # Now compute reverse proposal prob q(proposed -> current)
        # In the *proposed* state, what are the legal alternatives for the same block?
        # Temporarily set to proposed to query properly (already applied).
        legal_back = self._legal_alternatives_for(block)
        # The reverse must be able to pick the *current* mode
        # (If not, reversibility would be broken — but legality construction ensures it is present)
        if {"level": self.level_mode, "trend": self.trend_mode, "season": self.seasonal_mode}[block] not in legal_back + [proposed_mode]:
            # Very defensive; practically shouldn’t happen
            self._load_snapshot(cur_snap)
            self._rebuild_layout()
            self._rj_record(block, False)
            return
        # Reverse probability: choose same block, then choose current mode among the proposed-state legal alternatives
        q_block_rev = math.log(1.0 / len(blocks))
        # Build back legal alts in proposed state
        self._apply_modes(L, T, S)  # ensure still proposed
        back_alts = self._legal_alternatives_for(block)
        # The "current" mode (pre-move) must be among these alts
        curr_mode = {"level": cur_snap["level"], "trend": cur_snap["trend"], "season": cur_snap["season"]}[block]
        # remove "proposed_mode" from the alternatives set if present since alternatives exclude the current of that state
        # but _legal_alternatives_for excludes the *current* of that state automatically, so 'curr_mode' should be present here.
        if curr_mode not in back_alts:
            # If not present, something is inconsistent; be conservative and reject
            self._load_snapshot(cur_snap)
            self._rebuild_layout()
            self._rj_record(block, False)
            return
        q_mode_rev = math.log(1.0 / len(back_alts))

        # Restore current and evaluate current log posterior
        self._load_snapshot(cur_snap)
        self._rebuild_layout()
        ...
        dlog = (logpost_prop - logpost_cur) + (q_block_rev + q_mode_rev) - (q_block_fwd + q_mode_fwd)

        accept = self._mh_accept(dlog)
        self._rj_record(block, bool(accept))
        if not accept:
            return
        ...
        self._load_snapshot(cur_snap)
        self._apply_modes(L, T, S)
        # ensure state holder consistent (dimension may change)
        self._alloc_state_holder()
        after = (self.level_mode, self.trend_mode, self.seasonal_mode)
        print(f"[switch] block={block} | L:{before[0][:3]}→{after[0][:3]} T:{before[1][:3]}→{after[1][:3]} S:{before[2][:3]}→{after[2][:3]} | Δ={float(np.clip(dlog, -1e3, 1e3)):+.4f} | RJ {self._fmt_rj_all()}")
    ...
    def run(self) -> Dict[str, np.ndarray]:
        ...
        # pre-allocate with maximal possible dim
        max_dim = (1+1) + (2*self.K + (1 if self.use_nyq else 0))
    ...
    def save_posterior(self, out_npz_path: str, extra_meta: Optional[dict] = None) -> None:
        ...
        meta = {
            ...
            "modes": {"level": self.level_mode, "trend": self.trend_mode, "season": self.seasonal_mode},
...
if __name__ == "__main__":
    ...
    # RJ controls
    p.add_argument("--rj-moves-per-iter", type=int, default=1)
