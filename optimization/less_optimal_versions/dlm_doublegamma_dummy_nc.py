...
# =============================================================================
# Priors & Config (double-gamma prior on process SDs)
# =============================================================================
...
class SamplerConfig:
    ...
    # slice_w, slice_m kept for compatibility, but unused here
    slice_w: float = 1.0
    slice_m: int = 20


# =============================================================================
# DLM Sampler (double-gamma prior on process SDs + regression update)
# =============================================================================
...
class DLMGibbsConjugate:
    ...
    def __init__(
        ...
        # Double-gamma local and global scales
        # ξ_k for k ∈ {α, β, γ}, global τ shared across blocks
        self.xi_alpha = 1.0
        self.xi_beta = 1.0
        self.xi_gamma = 1.0
        self.tau = 1.0
    ...
    def update_double_gamma_scales(self) -> None:
        """
        Update local scales ξ_k and global scale τ under the hierarchy:
            s_k | ξ_k, τ, σ² ~ N(0, σ² / (ξ_k τ)),
            ξ_k ~ Gamma(a_xi, b_xi),
            τ ~ Gamma(a_tau, b_tau).
        """
        ...
        a_xi = self.priors.a_xi
        b_xi = self.priors.b_xi
        a_tau = self.priors.a_tau
        b_tau = self.priors.b_tau
        sigma2 = self.sigma2
        eps = 1e-12

        # --- local scales ξ_k | s_k, τ, σ² (Gamma) ---
        # ξ_k | ... ~ Gamma(a_xi + 1/2, b_xi + 0.5 * τ * s_k^2 / σ²)
        s2_alpha = self.s_alpha**2
        s2_beta  = self.s_beta**2
        s2_gamma = self.s_gamma**2

        # ensure τ > 0
        tau = max(self.tau, eps)

        # α
        shape_xi_alpha = a_xi + 0.5
        rate_xi_alpha  = b_xi + 0.5 * tau * s2_alpha / max(sigma2, eps)
        self.xi_alpha = np.random.gamma(shape=shape_xi_alpha, scale=1.0 / rate_xi_alpha)

        # β
        shape_xi_beta = a_xi + 0.5
        rate_xi_beta  = b_xi + 0.5 * tau * s2_beta / max(sigma2, eps)
        self.xi_beta = np.random.gamma(shape=shape_xi_beta, scale=1.0 / rate_xi_beta)

        # γ
        shape_xi_gamma = a_xi + 0.5
        rate_xi_gamma  = b_xi + 0.5 * tau * s2_gamma / max(sigma2, eps)
        self.xi_gamma = np.random.gamma(shape=shape_xi_gamma, scale=1.0 / rate_xi_gamma)

        # --- global scale τ | s, ξ, σ² (Gamma) ---
        # τ | ... ~ Gamma(a_tau + K/2, b_tau + 0.5 * Σ ξ_k s_k^2 / σ²), K = 3 here.
        K = 3
        shape_tau = a_tau + 0.5 * K
        rate_tau = b_tau + 0.5 * (
            self.xi_alpha * s2_alpha +
            self.xi_beta  * s2_beta  +
            self.xi_gamma * s2_gamma
        ) / max(sigma2, eps)
        self.tau = np.random.gamma(shape=shape_tau, scale=1.0 / rate_tau)
    ...
    def _progress_line(self, it: int) -> str:
        ...
        parts.append(
            "ξ=["
            + ", ".join(
                f"{x:.3g}" for x in [self.xi_alpha, self.xi_beta, self.xi_gamma]
            )
            + "]"
        )
        return " | ".join(parts)
    ...
    def run(self) -> Dict[str, np.ndarray]:
        ...
        # allocate storage (same interface as before, plus signed s_* and ξ, τ)
        self.keep = {
            ...
            # double-gamma scales
            "xi_alpha": np.zeros(n_kept),
            "xi_beta": np.zeros(n_kept),
            "xi_gamma": np.zeros(n_kept),
            "tau": np.zeros(n_kept),
        ...
        for it in range(cfg.n_iter):
            ...
            # 4) Update double-gamma local/global scales
            if self.dim_ncp > 0:
                self.update_double_gamma_scales()
            ...
            if it in save_iters:
                ...
                # double-gamma scales
                self.keep["xi_alpha"][keep_idx] = self.xi_alpha
                self.keep["xi_beta"][keep_idx] = self.xi_beta
                self.keep["xi_gamma"][keep_idx] = self.xi_gamma
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(
        description=(
            ...
            "Process SDs have a double-gamma global-local shrinkage prior "
            "s_k | ξ_k, τ, σ² ~ N(0, σ² / (ξ_k τ))."
        )
    )
    ...
    p.add_argument("--prior-m-m0-alpha", type=float, default=3.0)
    p.add_argument("--prior-s-m0-alpha", type=float, default=1.0)
    p.add_argument("--prior-m-m0-beta",  type=float, default=1.0)
    p.add_argument("--prior-s-m0-beta",  type=float, default=1.0)
    ...
    if args.print_summary:
        ...
        print("Double-gamma global-local prior for process SDs (s_k | ξ_k, τ, σ²):")
        print(f"  ξ_k ~ Gamma(a_xi={priors.a_xi:.3g}, b_xi={priors.b_xi:.3g})")
        print(f"  τ   ~ Gamma(a_tau={priors.a_tau:.3g}, b_tau={priors.b_tau:.3g})\n")
    ...
    if args.print_summary:
        ...
        print(f"mean τ   = {np.mean(post['tau']):.4g}")
        print(f"mean ξα  = {np.mean(post['xi_alpha']):.4g}")
        print(f"mean ξβ  = {np.mean(post['xi_beta']):.4g}")
        print(f"mean ξγ  = {np.mean(post['xi_gamma']):.4g}")
    ...
    if args.plot:
        ...
        plt.title("DLM (double-gamma prior, dynamic/dynamic/dynamic)")
