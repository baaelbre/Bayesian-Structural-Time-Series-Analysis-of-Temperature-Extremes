...
+ Frühwirth–Schnatter (FS) innovation prior on signed process SDs.

Changes vs previous:
- Replaced Half–Cauchy mixture for process SDs with FS prior:
    s_k ~ Normal(0, B0_k * sigma^2),  k in {alpha, beta, gamma}.
- Process scale update: sample q_k = s_k^2 from univariate log-concave
  target proportional to q^(-T/2) * exp( -SS/(2q) - q/(2 B0 sigma^2) ).
  Implemented via robust slice sampling on log-target; set s_k = ±sqrt(q_k).
- Kept: RJ logic (incl. “escape move”), FFBS, deterministic-parameter
  updates, storage, CLI, truth overlays, numerics.

Note: If you want “strict” non-centered state dynamics (z_t with unit
variance and linear-Gaussian regression in s_k), we can swap FFBS to run
on disturbances; this version focuses on getting the FS prior behavior
immediately with minimal surgery.
"""
...
def _slice_sample_positive(
    ...
    """
    q0 = float(max(q0, lower))
    logy = logf(q0) - rng.exponential()
    # Random initial interval
    u = rng.uniform()
    L = max(q0 - u * w, lower)
    R = min(L + w, upper)
    # Step out
    ...
    while tries < max_tries:
        ...
        if logf(q_new) >= logy and q_new > 0.0:
            return float(q_new)
        # shrink
        if q_new < q0:
            L = q_new
...
class PriorsFS:
    ...
    # FS prior scales B0 for signed SDs: s_k ~ N(0, B0_k * sigma^2)
    b0_alpha: float = 10.0
    b0_beta:  float = 10.0
    b0_gamma: float = 10.0
...
class _Layout:
    ...
    def __init__(self, period: int, level: str, trend: str, season: str):
        ...
        if season == "dynamic":
            for k in range(1, period): layout.append(f"g{k}")
            if period > 1:
...
class DLMRJGibbs:
    """Gaussian structural DLM with level/trend/season blocks and RJ–MCMC + FS prior."""
    ...
    def __init__(
        ...
        # process SDs (signed) for dynamic blocks (FS prior lives on s directly)
    ...
    def _update_one_fs(self, SS: float, T_eff: int, s_cur: float, B0: float) -> float:
        ...
        p = 1.0 - 0.5 * T_eff                  # GIG shape (for info)

        def log_target(q: float) -> float:
            if q <= 0.0: return -np.inf
            # log f(q) = -(T/2)*log q - SS/(2q) - q/(2*B0*sig2) + const
            return -(0.5 * T_eff) * math.log(q) - 0.5 * (b_lin / q) - 0.5 * (a_lin * q)

        q0 = max(s_cur*s_cur, 1e-12)
        q = _slice_sample_positive(self.rng, log_target, q0, w=max(1e-2, q0*0.5))
        s = math.sqrt(q)
    ...
    # ---------- RJ moves (unchanged except no HC draw on birth) ----------
    ...
    def _draw_prior_dyn_block(self, which: str) -> None:
        # When a block becomes dynamic, initialize signed SD from the FS prior scale roughly,
        # and draw m0/P0 from their priors. The sampler will refine by FS update.
        def rand_s(B0):
    ...
    def run(self) -> Dict[str, np.ndarray]:
        ...
        for it in range(cfg.n_iter):
            if self._layout.dim > 0:
                self.x = self._ffbs()
                # FS innovation-prior scale updates
...
if __name__ == "__main__":
    ...
    p.add_argument("--fs-b0-alpha", type=float, default=0.01)
    p.add_argument("--fs-b0-beta",  type=float, default=0.01)
    p.add_argument("--fs-b0-gamma", type=float, default=0.01)
