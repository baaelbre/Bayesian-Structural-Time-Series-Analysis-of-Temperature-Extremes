...
"""
Gaussian structural time-series model with RJ–MCMC + conjugate Gibbs (+ ASIS).

Sampling order per iteration
----------------------------
FFBS (centered) → ASIS for α,β,γ  → m0 (given x0) → P0 → deterministic params → σ² → RJ moves

ASIS step
---------
- Works blockwise for α, β, γ.
- Updates Half-Cauchy process variances (via IG mixture) in the *non-centered* parameterization.
- Interweaves states: transform centered path to standardized innovations z, sample new Q, then rebuild the block's
  path with the new SD (same standardized innovations). This preserves likelihood but improves mixing.

Progress line prints running ESS for σ and Q’s (lag-1 ACF, windowed).
"""
...
class _ESSMeter:
    """
    Windowed lag-1 ESS estimator.
    ESS ≈ N * (1 - ρ1) / (1 + ρ1), with ρ1 estimated from the last W samples.
    """
    ...
    def update(self, name: str, value: float) -> None:
        if name not in self.buf:
            self.buf[name] = deque(maxlen=self.window)
    ...
    def _lag1_acf(x: np.ndarray) -> float:
        ...
        r1 = num / denom
        return float(np.clip(r1, -0.999, 0.999))

    def ess(self, name: str) -> Optional[float]:
        if name not in self.buf:
            return None
        v = np.asarray(self.buf[name], float)
        n = v.size
        if n < 4:
            return None
        r1 = self._lag1_acf(v)
...
class DLMRJGibbs:
    """Gaussian structural DLM with level/trend/season blocks, ASIS, and RJ–MCMC."""
    def __init__(
        self,
        y: np.ndarray,
        period: int,
        ...
        # process SDs + Half-Cauchy auxiliaries
        ...
        self.m0_alpha = (float(m0_alpha_init) if self._layout.idx_alpha is not None
                         else float(self.priors.m_m0_alpha))
        self.P0_alpha = float(P0_alpha_init) if self._layout.idx_alpha is not None else 0.0
        self.m0_beta  = (float(m0_beta_init) if self._layout.idx_beta  is not None
                         else float(self.priors.m_m0_beta))
        ...
        # ESS meters
        self._ess = _ESSMeter(window=self.cfg.ess_window)

        # truth overlays
        self.true_sigma = self.true_Q = None
        self.true_mu_t = self.true_alpha_t = self.true_beta_t = self.true_gamma_t = None
    ...
    def set_truth(self, sigma=None, Q=None, m0_level=None, m0_trend=None, m0_season=None,
                  P0_level=None, P0_trend=None, P0_season=None) -> None:
    ...
    def set_truth_paths(self, mu=None, alpha=None, beta=None, gamma=None) -> None:
    ...
    def _ffbs(self) -> np.ndarray:
        ...
        for t in range(self.T - 1, -1, -1):
            J = C[t] @ self._layout.A().T
            J = _spd_solve(Rm[t + 1], J.T).T
            mean = m[t] + J @ (x[t + 1] - (self._layout.A() @ m[t] + u))
    ...
    # ----------------------- ASIS (incl. Half-Cauchy Q) -------------------- #

    @staticmethod
    def _rinvgamma(rng: np.random.Generator, shape: float, scale: float) -> float:
        return 1.0 / rng.gamma(shape, 1.0 / scale)

    def _asis_block_alpha(self) -> None:
        """ASIS for α: standardize α-innovations, sample Qα via Half-Cauchy mixture, rebuild α-path."""
        li = self._layout.idx_alpha
        if li is None: return
        # compute standardized innovations z_t for α
        z = np.zeros(self.T)
        for t in range(1, self.T + 1):
            drift = 0.0
            if self._layout.idx_beta is not None:
            ...
            s = max(self.s_alpha, 1e-12)
            z[t - 1] = (self.x[t, li] - mean) / s

        # update Half-Cauchy aux + Qα in *non-centered*
        A = float(self.priors.hc_scale_alpha)
        # IG mixture: a ~ IG(1, 1/A^2), Q ~ IG(T/2 + 1/2, 0.5*sum(z^2) + 1/a)
        self._a_alpha = self._rinvgamma(self.rng, 1.0, 1.0 / (A * A))
        SSz = 0.5 * float(np.dot(z, z))
        Q_alpha = self._rinvgamma(self.rng, 0.5 * self.T + 0.5, SSz + 1.0 / max(self._a_alpha, 1e-300))
        self.s_alpha = math.sqrt(max(Q_alpha, 0.0))

        # rebuild α path using same z but new s_alpha
        for t in range(1, self.T + 1):
            drift = 0.0
            if self._layout.idx_beta is not None:
                drift = self.x[t - 1, self._layout.idx_beta]
                ...
                drift = float(self.m0_beta)
            self.x[t, li] = self.x[t - 1, li] + drift + self.s_alpha * z[t - 1]

    def _asis_block_beta(self) -> None:
        bi = self._layout.idx_beta
        if bi is None: return
        # standardized innovations for β
        z = (self.x[1:, bi] - self.x[:-1, bi]) / max(self.s_beta, 1e-12)
        A = float(self.priors.hc_scale_beta)
        self._a_beta = self._rinvgamma(self.rng, 1.0, 1.0 / (A * A))
        SSz = 0.5 * float(np.dot(z, z))
        Q_beta = self._rinvgamma(self.rng, 0.5 * self.T + 0.5, SSz + 1.0 / max(self._a_beta, 1e-300))
        self.s_beta = math.sqrt(max(Q_beta, 0.0))
        # rebuild β path with new s_beta
        for t in range(1, self.T + 1):
            self.x[t, bi] = self.x[t - 1, bi] + self.s_beta * z[t - 1]

    def _asis_block_gamma(self) -> None:
        if self.seasonal_mode != "dynamic": return
        gs, ge = self._layout.idx_g_start, self._layout.idx_g_end
        # z_t for the leading seasonal state (sum-to-zero rotation embedded in A)
        z = np.zeros(self.T)
        for t in range(1, self.T + 1):
            prev = self.x[t - 1, gs: ge + 1]
            mean_new = -float(np.sum(prev))
            s = max(self.s_gamma, 1e-12)
            z[t - 1] = (self.x[t, gs] - mean_new) / s

        A = float(self.priors.hc_scale_gamma)
        self._a_gamma = self._rinvgamma(self.rng, 1.0, 1.0 / (A * A))
        SSz = 0.5 * float(np.dot(z, z))
        Q_gamma = self._rinvgamma(self.rng, 0.5 * self.T + 0.5, SSz + 1.0 / max(self._a_gamma, 1e-300))
        self.s_gamma = math.sqrt(max(Q_gamma, 0.0))
        # rebuild first seasonal coordinate; others follow via A structure
        for t in range(1, self.T + 1):
            prev = self.x[t - 1, gs: ge + 1]
            mean_new = -float(np.sum(prev))
            self.x[t, gs] = mean_new + self.s_gamma * z[t - 1]
            # rotate the block forward (companion structure)
            K = ge - gs + 1
            if K > 1:
                self.x[t, gs + 1: ge + 1] = prev[:-1]

    def _asis_interweave(self) -> None:
        """Run ASIS for α, β, γ (includes Half-Cauchy updates for Q's)."""
        if self._layout.dim == 0: return
        self._asis_block_alpha()
        self._asis_block_beta()
        self._asis_block_gamma()
    ...
    def update_sigma2(self) -> None:
        ...
        # ESS track
        self._ess.update("sigma", math.sqrt(self.sigma2))
    ...
    def update_m0(self) -> None:
        ...
        pos = 0
        if self._layout.idx_alpha is not None:
            self.m0_alpha = self._gibbs_m0_scalar(self.rng, float(self.x[0, pos]), self.priors.m_m0_alpha, self.priors.s_m0_alpha, self.P0_alpha)
            pos += 1
        if self._layout.idx_beta is not None:
            self.m0_beta = self._gibbs_m0_scalar(self.rng, float(self.x[0, pos]), self.priors.m_m0_beta, self.priors.s_m0_beta, self.P0_beta)
            pos += 1
        if self.seasonal_mode == "dynamic":
            base = (np.zeros(self.period - 1)
                    if self.priors.m_m0_gamma is None else np.asarray(self.priors.m_m0_gamma, float))
    ...
    def _fmt_rj_block(self, block: str) -> str:
        s = self.rj_stats[block]; prop = max(1, int(s["proposed"])); acc = int(s["accepted"])
    ...
    def _ess_str(self, name: str) -> str:
        val = self._ess.ess(name)
        return "–" if val is None else f"{val:.0f}"

    def _progress_line(self, it: int) -> str:
        parts = [f"[it {it + 1}/{self.cfg.n_iter}]",
                 f"σ={math.sqrt(self.sigma2):.3f}(ESS {self._ess_str('sigma')})",
                 f"L/T/S={self.level_mode[:3]}/{self.trend_mode[:3]}/{self.seasonal_mode[:3]}"]
        if self._layout.idx_alpha is not None:
            self._ess.update("Q_alpha", self.s_alpha**2)
            parts.append(f"Qα={self.s_alpha**2:.4g}(ESS {self._ess_str('Q_alpha')})")
        if self._layout.idx_beta is not None:
            self._ess.update("Q_beta", self.s_beta**2)
            parts.append(f"Qβ={self.s_beta**2:.4g}(ESS {self._ess_str('Q_beta')})")
        if self.seasonal_mode == "dynamic":
            self._ess.update("Q_gamma", self.s_gamma**2)
            parts.append(f"Qγ={self.s_gamma**2:.4g}(ESS {self._ess_str('Q_gamma')})")
    ...
    def run(self) -> Dict[str, np.ndarray]:
        ...
        for it in range(cfg.n_iter):
            if self._layout.dim > 0:
                # 1) FFBS (centered)
                self.x = self._ffbs()
                # 2) ASIS (includes Half-Cauchy Q updates + interweaving)
                self._asis_interweave()
            # 3) m0, 4) P0, 5) deterministic params, 6) σ²
            self.update_m0()
            self.update_P0()
    ...
    def save_posterior(self, out_npz_path: str, extra_meta: Optional[dict] = None) -> None:
        ...
        meta = {
            ...
            "rj_accept": {
                b: {"proposed": int(self.rj_stats[b]["proposed"]),
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(
        description=(
            "Gaussian structural TS (level/trend/season) with ASIS + RJ-MCMC (Δ log posterior) + conjugate Gibbs.\n"
    ...
    p.add_argument("--out-dir", type=str, default="results/simulations/DLM_ASIS")
    ...
    if args.print_summary:
        if Mean_Time_Series is not None:
            print(f"\nSimulated {args.T} observations (σ={args.sigma}) with TRUE modes {args.level_mode}/{args.trend_mode}/{args.seasonal_mode}.")
        else:
    ...
    if args.plot:
        ...
        plt.title(
            "Structural DLM + ASIS - current modes: "
    ...
    rj_dump = {
        b: {"proposed": int(sampler.rj_stats[b]["proposed"]),
    ...
    # --- quick posterior text summaries
    ...
    def _print_summ(name: str, a: np.ndarray) -> None:
        ...
        if s is None:
            ...
            print(f"  {name:18s} : mean={s['mean']:.4g} | sd={s['sd']:.4g} | [{s['q05']:.4g}, {s['q50']:.4g}, {s['q95']:.4g}]  (n={s['n']})")
    ...
    if "modes" in post:
        ...
        if modes.ndim == 2 and modes.size:
            ...
            best = uniq[idx]; best_lab = "-".join(lab[k] for k in best)
