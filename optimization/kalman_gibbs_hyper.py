...
"""
Kalman filter and Gibbs sampling for structural Gaussian state-space models
with fully conjugate updates:

  y_t = mu_t + eps_t,  eps_t ~ N(0, sigma^2)

Dynamic state innovations have variances q_alpha, q_beta, q_gamma (last seasonal coord).
We use inverse-gamma priors on these variances via Gamma priors on precisions:

  tau = 1/v ~ Gamma(a, b)      (shape–rate)
  v = 1/tau ~ IG(a, b)

NEW: Gamma hyperpriors on each rate b:
  b ~ Gamma(kappa, theta)

This preserves conjugacy and yields a Gibbs step for b | tau.
"""
...
# =============================================================================
# Priors & Config (IG for variances via Gamma on precisions; Normal for det. params)
# =============================================================================

@dataclass
class Priors:
    # Observation precision prior: tau_sigma ~ Gamma(a_sigma, b_sigma)
    a_sigma: float = 1.0
    b_sigma: float = 1.0

    # Process precisions (only used if the component is dynamic)
    a_alpha: float = 1.0
    b_alpha: float = 1.0
    a_beta:  float = 1.0
    b_beta:  float = 1.0
    ...
    # --- Hyperpriors on the rate parameters b_*: b_* ~ Gamma(kappa_*, theta_*)
    # If learn_b_* = False, b_* stays fixed at the value above.
    learn_b_sigma: bool = False
    k_b_sigma: float = 1.0
    t_b_sigma: float = 1.0

    learn_b_alpha: bool = False
    k_b_alpha: float = 1.0
    t_b_alpha: float = 1.0

    learn_b_beta:  bool = False
    k_b_beta: float = 1.0
    t_b_beta: float = 1.0

    learn_b_gamma: bool = False
    k_b_gamma: float = 1.0
    t_b_gamma: float = 1.0
...
class DLMGibbs:
    ...
    Variance priors are IG (via Gamma on precision), and each prior rate b_* may
    itself be random with a Gamma hyperprior, yielding conjugate Gibbs updates.
    """
    ...
    def __init__(
        ...
        # ---- Prior hyper-rate states b_* (learnable)
        self._b_sigma = float(self.priors.b_sigma)
        self._b_alpha = float(self.priors.b_alpha)
        self._b_beta  = float(self.priors.b_beta)
        self._b_gamma = float(self.priors.b_gamma)
    ...
    # ------------------- Conjugate variance & hyper-rate updates ----------- #
    # We work in precision form: tau = 1/v. Priors are tau ~ Gamma(a, b).
    # Data contributions are sums of squared residuals (Gaussian innovations).

    def _sample_sigma2_and_b(self) -> None:
        # residuals: y_t - (deterministic + dynamic)
        ss = 0.0
        H = self._H_t()
        for t in range(1, self.T + 1):
        ...
        b_post = self._b_sigma + 0.5 * ss                   # <-- current rate
        tau_sigma = np.random.gamma(shape=a_post, scale=1.0 / b_post)  # precision
        self.sigma2 = float(1.0 / max(tau_sigma, 1e-300))

        # hyper-rate Gibbs (optional)
        if self.priors.learn_b_sigma:
            k, th = self.priors.k_b_sigma, self.priors.t_b_sigma
            # b | tau ~ Gamma(a + k, theta + tau)
            self._b_sigma = float(np.random.gamma(shape=self.priors.a_sigma + k,
                                                  scale=1.0 / (th + tau_sigma)))

    def _sample_q_alpha_and_b(self) -> None:
        ...
        b_post = self._b_alpha + 0.5 * ss
        tau_alpha = np.random.gamma(shape=a_post, scale=1.0 / b_post)
        self.q_alpha = float(1.0 / max(tau_alpha, 1e-300))

        if self.priors.learn_b_alpha:
            k, th = self.priors.k_b_alpha, self.priors.t_b_alpha
            self._b_alpha = float(np.random.gamma(shape=self.priors.a_alpha + k,
                                                  scale=1.0 / (th + tau_alpha)))

    def _sample_q_beta_and_b(self) -> None:
        ...
        b_post = self._b_beta + 0.5 * ss
        tau_beta = np.random.gamma(shape=a_post, scale=1.0 / b_post)
        self.q_beta = float(1.0 / max(tau_beta, 1e-300))

        if self.priors.learn_b_beta:
            k, th = self.priors.k_b_beta, self.priors.t_b_beta
            self._b_beta = float(np.random.gamma(shape=self.priors.a_beta + k,
                                                 scale=1.0 / (th + tau_beta)))

    def _sample_q_gamma_and_b(self) -> None:
        if self.seasonal_mode != "dynamic":
            return
        gs, ge = self.idx_g_start, self.idx_g_end
        ss = 0.0
        ...
        b_post = self._b_gamma + 0.5 * ss
        tau_gamma = np.random.gamma(shape=a_post, scale=1.0 / b_post)
        self.q_gamma = float(1.0 / max(tau_gamma, 1e-300))

        if self.priors.learn_b_gamma:
            k, th = self.priors.k_b_gamma, self.priors.t_b_gamma
            self._b_gamma = float(np.random.gamma(shape=self.priors.a_gamma + k,
                                                  scale=1.0 / (th + tau_gamma)))
    ...
    def _progress_line(self, it: int, logZ: float, ema_logZ: float) -> str:
        ...
        # (Optional) show current hyper-rates when being learned
        hb = []
        if self.priors.learn_b_sigma: hb.append(f"bσ={self._b_sigma:.3g}")
        if self.idx_alpha is not None and self.priors.learn_b_alpha: hb.append(f"bα={self._b_alpha:.3g}")
        if self.idx_beta  is not None and self.priors.learn_b_beta:  hb.append(f"bβ={self._b_beta:.3g}")
        if self.seasonal_mode == "dynamic" and self.priors.learn_b_gamma: hb.append(f"bγ={self._b_gamma:.3g}")
        grp3 = (" | " + ", ".join(hb)) if hb else ""

        return left + mid + grp1 + grp2 + grp3
    ...
    def run(self) -> Dict[str, np.ndarray]:
        ...
        # also store learned hyper-rates for inspection
        if self.priors.learn_b_sigma: self.keep["b_sigma"] = np.zeros(n_kept, float)
        if self.idx_alpha is not None and self.priors.learn_b_alpha: self.keep["b_alpha"] = np.zeros(n_kept, float)
        if self.idx_beta  is not None and self.priors.learn_b_beta:  self.keep["b_beta"]  = np.zeros(n_kept, float)
        if self.seasonal_mode == "dynamic" and self.priors.learn_b_gamma: self.keep["b_gamma"] = np.zeros(n_kept, float)
        ...
        for it in range(cfg.n_iter):
            ...
            # 3) Sample variances AND (optionally) their hyper-rates (all conjugate)
            self._sample_sigma2_and_b()
            if self.dim > 0:
                self._sample_q_alpha_and_b()
                self._sample_q_beta_and_b()
                self._sample_q_gamma_and_b()
            ...
            if it in save_iters and keep_idx < n_kept:
                ...
                # save hyper-rates if learned
                if "b_sigma" in self.keep: self.keep["b_sigma"][keep_idx] = self._b_sigma
                if "b_alpha" in self.keep: self.keep["b_alpha"][keep_idx] = self._b_alpha
                if "b_beta"  in self.keep: self.keep["b_beta"][keep_idx]  = self._b_beta
                if "b_gamma" in self.keep: self.keep["b_gamma"][keep_idx] = self._b_gamma
    ...
    def save_posterior(self, out_npz_path: str, extra_meta: Optional[dict] = None) -> None:
        ...
        meta = {
            ...
            "learned_b": {
                "sigma": self._b_sigma,
                "alpha": (self._b_alpha if self.idx_alpha is not None else None),
                "beta":  (self._b_beta  if self.idx_beta  is not None else None),
                "gamma": (self._b_gamma if self.seasonal_mode == "dynamic" else None),
            }
        }
        if extra_meta:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Gaussian DLM Gibbs (FFBS) with IG priors and Gamma hyperpriors on rates b")
    ...
    # Priors (Gamma on precisions; Normal on deterministic params)
    ...
    # Hyperpriors: toggle learning of b_* and set (kappa, theta)
    parser.add_argument("--learn-b-sigma", default=True)
    parser.add_argument("--k-b-sigma", type=float, default=1.0)
    parser.add_argument("--t-b-sigma", type=float, default=1.0)

    parser.add_argument("--learn-b-alpha", default=True)
    parser.add_argument("--k-b-alpha", type=float, default=1.0)
    parser.add_argument("--t-b-alpha", type=float, default=1.0)

    parser.add_argument("--learn-b-beta", default=True)
    parser.add_argument("--k-b-beta", type=float, default=1.0)
    parser.add_argument("--t-b-beta", type=float, default=1.0)

    parser.add_argument("--learn-b-gamma", default=True)
    parser.add_argument("--k-b-gamma", type=float, default=1.0)
    parser.add_argument("--t-b-gamma", type=float, default=1.0)
    ...
    priors = Priors(
        ...
        learn_b_sigma=bool(args.learn_b_sigma), k_b_sigma=float(args.k_b_sigma), t_b_sigma=float(args.t_b_sigma),
        learn_b_alpha=bool(args.learn_b_alpha), k_b_alpha=float(args.k_b_alpha), t_b_alpha=float(args.t_b_alpha),
        learn_b_beta=bool(args.learn_b_beta),   k_b_beta=float(args.k_b_beta),   t_b_beta=float(args.t_b_beta),
        learn_b_gamma=bool(args.learn_b_gamma), k_b_gamma=float(args.k_b_gamma), t_b_gamma=float(args.t_b_gamma),
    ...
    for key in ["b_sigma","b_alpha","b_beta","b_gamma"]:
        if key in posterior and posterior[key].size:
            print(f"Posterior mean {key}: {np.mean(posterior[key]):.6g}")
