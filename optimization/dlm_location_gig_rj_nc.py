...
"""
Structural Gaussian DLM with RJ–MCMC + conjugate Gibbs
+ Frühwirth–Schnatter prior on signed process SDs.

Highlights
----------
• Blocks: level (α), trend (β), season (γ; newest-first, length p−1).
• RJ moves with legal-mode constraint and "escape" fix (trend dyn ⇒ level dyn).
• FFBS with SPD-safe algebra and tiny jitter.
• FS prior on signed SDs: s_k ~ N(0, B0_k * σ²). Update via slice on q = s².
• Deterministic-parameter posteriors when blocks are non-dynamic.
• Rich posterior dump (NPZ + JSON) + quick plot and text summary.
"""
...
def _spd_chol(M: np.ndarray, base_eps: float = 1e-12):
    """Cholesky with escalating jitter; returns (L, eps_used)."""
    n = M.shape[0]
    I = np.eye(n)
    S = 0.5 * (M + M.T)
    for k in range(4):
        eps = base_eps * (10 ** k)
        try:
            L = np.linalg.cholesky(S + eps * I)
            return L, eps
        except np.linalg.LinAlgError:
            continue
    # last resort: add diag up to positivity
    ev = np.linalg.eigvalsh(S)
    fix = max(1e-10, -float(ev.min()) + 1e-10)
    L = np.linalg.cholesky(S + fix * I)
    return L, fix

def _spd_solve(M: np.ndarray, B: np.ndarray, base_eps: float = 1e-12) -> np.ndarray:
    """Solve M X = B for SPD-like M; uses Cholesky with jitter."""
    L, _ = _spd_chol(M, base_eps)
...
# ---------------- slice sampler for 1D positive variable --------------------

def _slice_sample_positive(
    rng: np.random.Generator,
    logf,              # function: logf(q) for q>0
    q0: float,
    w: float = 1.0,
    m: int = 100,
    max_tries: int = 200,
    ...
    """
    Univariate slice sampler on (0, +inf) using stepping-out / shrinkage.
    logf is the log target (up to a constant). Numerically robust.
    """
    q0 = float(np.clip(q0, lower, upper))
    logy = logf(q0) - rng.exponential()
    ...
    k = (m - 1) - j
    while j > 0 and L > lower and logf(L) > logy:
        L = max(L - w, lower); j -= 1
    while k > 0 and R < upper and logf(R) > logy:
        R = min(R + w, upper); k -= 1
    # shrinkage
    tries = 0
    while tries < max_tries:
        q_new = rng.uniform(L, R)
        if q_new > 0.0 and logf(q_new) >= logy:
            return float(q_new)
        if q_new < q0: L = q_new
        else:          R = q_new
        tries += 1
...
class PriorsFS:
    ...
    # Normal priors for deterministic parameters & initial means of dynamic blocks
    ...
    # Inverse-Gamma priors for initial variances of dynamic blocks (on P0)
    ...
    # FS prior scales for signed process SDs: s_k ~ Normal(0, B0_k * sigma^2)
    b0_alpha: float = 10.0
    b0_beta:  float = 10.0
    b0_gamma: float = 10.0
...
class SamplerConfig:
    ...
    progress: bool = True
    progress_every: int = 0  # if 0, auto (~2%)
    ...
    # RJ acceptance rolling window
    rj_window: int = 500

# =============================================================================
# Model layout (indices + system matrices)
# =============================================================================
...
class _Layout:
    ...
    def A(self) -> np.ndarray:
        ...
        # dummy-coded seasonal: new γ_t equals negative sum of previous seasonal coords
    ...
    def u(self, m0_beta: float) -> np.ndarray:
        """Drift when trend is deterministic and level is dynamic."""
    ...
    def Q(self, s_alpha: float, s_beta: float, s_gamma: float) -> np.ndarray:
        ...
        if (self.idx_alpha is not None) and (s_alpha != 0.0): Q[self.idx_alpha, self.idx_alpha] = s_alpha**2
        if (self.idx_beta  is not None) and (s_beta  != 0.0): Q[self.idx_beta,  self.idx_beta ] = s_beta**2
        if (self.season_mode == "dynamic") and (s_gamma != 0.0):
...
class DLMRJGibbs:
    """Gaussian structural DLM with level/trend/season and RJ–MCMC + FS prior."""
    ...
    def __init__(
        ...
        # signed process SDs (FS prior lives on s directly)
        ...
        # m0/P0 for dynamic blocks (or deterministic parameters when non-dynamic)
        ...
        if self._layout.dim > 0:
            ...
            self.x[0] = self.rng.multivariate_normal(m0_vec, np.diag(P0_diag) + EPS * np.eye(self._layout.dim))
        ...
        self.true_m0_level=self.true_m0_trend=self.true_m0_season=None
        self.true_P0_level=self.true_P0_trend=self.true_P0_season=None

        if self.cfg.progress:
            sd1 = _robust_sd(np.diff(self.y)) if self.T >= 2 else 0.0
            sd2 = _robust_sd(np.diff(self.y, n=2)) if self.T >= 3 else 0.0
            print(f"[init] modes L/T/S={self.level_mode[:3]}/{self.trend_mode[:3]}/{self.seasonal_mode[:3]} "
                  f"| sd1={sd1:.4g} sd2={sd2:.4g}")

    # -------- truth overlays --------
    def set_truth(self, sigma=None, Q=None, m0_level=None, m0_trend=None, m0_season=None,
                  P0_level=None, P0_trend=None, P0_season=None):
    ...
    def set_truth_paths(self, mu=None, alpha=None, beta=None, gamma=None):
    ...
    def _current_m0_P0(self) -> Tuple[np.ndarray, np.ndarray]:
        m0, P0 = [], []
        if self._layout.idx_alpha is not None: m0.append(self.m0_alpha); P0.append(self.P0_alpha)
        if self._layout.idx_beta  is not None: m0.append(self.m0_beta ); P0.append(self.P0_beta )
        if self.seasonal_mode == "dynamic":
    ...
    def _log_prior_current(self) -> float:
        ...
        # FS normal priors on signed SDs (only when dynamic)
        def n0(x, var): return -0.5 * (x * x) / max(var, 1e-300)

        if self._layout.idx_alpha is not None:
            lp += n0(self.s_alpha, self.priors.b0_alpha * self.sigma2)
            lp += -0.5 * ((self.m0_alpha - self.priors.m_m0_alpha) ** 2) / (self.priors.s_m0_alpha**2)
            lp += -(self.priors.a_P0_alpha + 1.0) * math.log(max(self.P0_alpha, 1e-300)) - (self.priors.b_P0_alpha / max(self.P0_alpha, 1e-300))
        if self._layout.idx_beta is not None:
            lp += n0(self.s_beta, self.priors.b0_beta * self.sigma2)
            lp += -0.5 * ((self.m0_beta - self.priors.m_m0_beta) ** 2) / (self.priors.s_m0_beta**2)
            lp += -(self.priors.a_P0_beta + 1.0) * math.log(max(self.P0_beta, 1e-300)) - (self.priors.b_P0_beta / max(self.P0_beta, 1e-300))
        if self.seasonal_mode == "dynamic":
            lp += n0(self.s_gamma, self.priors.b0_gamma * self.sigma2)
            base = (np.zeros(self.period - 1)
                    if self.priors.m_m0_gamma is None
                    else np.asarray(self.priors.m_m0_gamma, float))
            s2 = self.priors.s_m0_gamma**2
            lp += -0.5 * float(np.sum((self.m0_gamma - base) ** 2)) / s2
            lp += -(self.priors.a_P0_gamma + 1.0) * math.log(max(self.P0_gamma, 1e-300)) - (self.priors.b_P0_gamma / max(self.P0_gamma, 1e-300))

        # deterministic param priors
        if self.level_mode == "deterministic":
            lp += -0.5 * ((self.m0_alpha - self.priors.m_m0_alpha) ** 2) / (self.priors.s_m0_alpha**2)
        ...
        if self.seasonal_mode == "deterministic":
            K = self.period - 1
            mu = (np.zeros(K) if self.priors.m_m0_gamma is None
    ...
    # -------- FS process-scale updates (slice on q = s^2) --------
    def _update_one_fs(self, SS: float, T_eff: int, s_cur: float, B0: float) -> float:
        if T_eff <= 0: return 0.0
        sig2 = float(self.sigma2)
        a_lin = 1.0 / max(B0 * sig2, 1e-300)  # coefficient on q in exponent
        b_lin = float(SS)                      # coefficient on 1/q

        def log_target(q: float) -> float:
            if q <= 0.0: return -np.inf
            # log f(q) ∝ -(T/2)*log q - SS/(2q) - q/(2*B0*σ²)
            return -(0.5 * T_eff) * math.log(q) - 0.5 * (b_lin / q) - 0.5 * (a_lin * q)

        q0 = max(s_cur * s_cur, 1e-12)
        # heuristic step width: around current q0 or bounded by SS/T
        guess = max(q0, (SS / max(T_eff, 1)) if SS > 0 else q0)
        w = max(1e-3, 0.5 * guess)
        q = _slice_sample_positive(self.rng, log_target, q0, w=w)
        s = math.sqrt(q)
        if self.rng.uniform() < 0.5: s = -s
        return float(s)

    def update_process_scales_fs(self) -> None:
        if self._layout.idx_alpha is not None:
            SS, T_eff = self._innovation_ss_alpha()
            self.s_alpha = self._update_one_fs(SS, T_eff, self.s_alpha, self.priors.b0_alpha)
        if self._layout.idx_beta is not None:
            SS, T_eff = self._innovation_ss_beta()
            self.s_beta = self._update_one_fs(SS, T_eff, self.s_beta, self.priors.b0_beta)
        if self.seasonal_mode == "dynamic":
            SS, T_eff = self._innovation_ss_gamma()
            self.s_gamma = self._update_one_fs(SS, T_eff, self.s_gamma, self.priors.b0_gamma)

    # -------- other conjugate updates --------
    @staticmethod
    ...
    def update_m0(self) -> None:
        ...
        pos = 0
        if self._layout.idx_alpha is not None:
            self.m0_alpha = self._gibbs_m0_scalar(self.rng, float(self.x[0, pos]),
                                                  self.priors.m_m0_alpha, self.priors.s_m0_alpha, self.P0_alpha)
            pos += 1
        if self._layout.idx_beta is not None:
            self.m0_beta  = self._gibbs_m0_scalar(self.rng, float(self.x[0, pos]),
    ...
    def _rj_record(self, block: str, accepted: bool) -> None:
        s = self.rj_stats[block]; s["proposed"] += 1;  s["accepted"] += int(accepted);  s["win"].append(1 if accepted else 0)

    def _fmt_rj_block(self, block: str) -> str:
        s = self.rj_stats[block]; prop = max(1, int(s["proposed"])); acc = int(s["accepted"]); return f"{100.0 * acc / prop:.1f}%"
    ...
    def _draw_prior_dyn_block(self, which: str) -> None:
        def rand_s(B0):
            s = math.sqrt(max(B0 * self.sigma2, 1e-6)) * float(self.rng.standard_normal())
            return float(np.clip(s, -10.0, 10.0))
        if which == "level":
            self.s_alpha = rand_s(self.priors.b0_alpha)
            ...
            self.s_beta = rand_s(self.priors.b0_beta)
            ...
            self.s_gamma = rand_s(self.priors.b0_gamma)
            base = (np.zeros(self.period - 1)
                    if self.priors.m_m0_gamma is None
                    else np.asarray(self.priors.m_m0_gamma, float))
            s = float(self.priors.s_m0_gamma)
    ...
    def _rj_move_one(self) -> None:
        ...
        if comp == "level":
            ...
            new_trend = prop
            if new_trend == "dynamic" and new_level != "dynamic":  # escape move
                new_level = "dynamic"
        else:
            new_season = prop
        ...
        # initialize newly dynamic blocks under FS prior
    ...
    def save_posterior(self, out_npz_path: str, extra_meta: Optional[dict] = None) -> None:
        ...
        if self.true_m0_level is not None: arrays["true_m0_level"] = float(self.true_m0_level)
        if self.true_m0_trend is not None: arrays["true_m0_trend"] = float(self.true_m0_trend)
        if self.true_m0_season is not None: arrays["true_m0_season"] = np.asarray(self.true_m0_season, float)
        if self.true_P0_level is not None: arrays["true_P0_level"] = float(self.true_P0_level)
        if self.true_P0_trend is not None: arrays["true_P0_trend"] = float(self.true_P0_trend)
        if self.true_P0_season is not None: arrays["true_P0_season"] = np.asarray(self.true_P0_season, float)
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(
        description=(
            "Gaussian structural TS (level/trend/season) with RJ-MCMC (Δ log posterior) + FS innovation priors.\n"
    ...
    # FS innovation prior B0 scales
    p.add_argument("--fs-b0-alpha", type=float, default=1000)
    p.add_argument("--fs-b0-beta",  type=float, default=1000)
    p.add_argument("--fs-b0-gamma", type=float, default=1000)
    ...
    if Mean_Time_Series is not None:
        ...
        mts = Mean_Time_Series(
            ...
            m0_season=m0_season, v0_season=v0_season, start_date=start_date,
        )
    ...
    priors = PriorsFS(
        ...
        b0_alpha=args.fs_b0_alpha, b0_beta=args.fs_b0_beta, b0_gamma=args.fs_b0_gamma,
    )
    ...
    sampler = DLMRJGibbs(
        ...
        m0_alpha_init=args.m0_level, m0_beta_init=args.m0_trend, m0_gamma_init=m0_gamma_init,
    ...
    if args.print_summary:
        ...
        print("FS innovation prior scales (B0) for process SDs (s ~ N(0, B0*σ²)):")
    ...
    sampler.save_posterior(
        ...
        extra_meta={
            ...
            "fs_b0_scales": {"alpha": priors.b0_alpha, "beta": priors.b0_beta, "gamma": priors.b0_gamma},
    ...
    if args.plot:
        ...
        plt.title("Structural DLM (FS prior) - current modes: "
    ...
    rj_dump = {b: {"proposed": int(sampler.rj_stats[b]["proposed"]),
    ...
    def _finite_flat(a: np.ndarray) -> np.ndarray:
        v = np.asarray(a, float).ravel(); return v[np.isfinite(v)]
    def _summ_1d(a: np.ndarray) -> Optional[dict]:
        ...
        return {"mean": float(v.mean()),
                ...
                "q05": float(q[0]), "q50": float(q[1]), "q95": float(q[2]), "n": int(v.size)}
    def _print_summ(name: str, a: np.ndarray) -> None:
        s = _summ_1d(a)
        if s is None:
            print(f"  {name:18s} : (no finite draws)")
        else:
            print(f"  {name:18s} : mean={s['mean']:.4g} | sd={s['sd']:.4g} | "
                  f"[{s['q05']:.4g}, {s['q50']:.4g}, {s['q95']:.4g}]  (n={s['n']})")

    print("================ Posterior summaries (FS) ================")
    ...
    if "m0_gamma" in post:
        ...
        if arr.ndim == 2 and arr.size:
            for j in range(arr.shape[1]): _print_summ(f"m0_gamma[{j}]", arr[:, j])
    if "season_det" in post:
        arr = np.asarray(post["season_det"])
        if arr.ndim == 2 and arr.size:
            for j in range(arr.shape[1]): _print_summ(f"season_det[{j}]", arr[:, j])
    if "mu" in post:
        mu = np.asarray(post["mu"], float)
        if mu.ndim == 2 and mu.size:
            mu_mean = mu.mean(axis=0); mu_sd = mu.std(axis=0, ddof=1) if mu.shape[0] > 1 else np.zeros(mu.shape[1])
            print("\n  μ_t (path) summary:")
