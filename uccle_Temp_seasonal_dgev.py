# %% scripts/run_uccle_dgev_seasonal_pgas.py
...
from optimization.dgev_lognormal_nc_3 import (
    DGEVParticleGibbs,
    Priors,
    SamplerConfig,
    build_seasonal,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="DGEV PGAS (NCP/disturbance-based) on Uccle SEASONAL TX/TN extremes (meteorological seasons)"
    )
    ...
    parser.add_argument("--init-xi", type=float, default=None,
                        help="Initial ξ for GEV (default: 0.0, clipped to [xi_lower, xi_upper]).")

    # ---------------- Priors (match new DGEV PGAS NCP Priors) ----------------
    ...
    # Deterministic level / slope / season (Gaussian)
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated first (p-1) means for deterministic seasonal prior (length = period-1, NEWEST-FIRST).",
    )
    parser.add_argument("--prior-s-season", type=float, default=5.0)

    # Log-normal priors for process SDs s_alpha, s_beta, s_gamma
    ...
    parser.add_argument("--ess-frac", type=float, default=0.5,
                        help="Resample in PGAS when ESS < ess_frac * N.")
    ...
    # ---------------- Deterministic seasonal prior (first p-1 entries) ----------------
    ...
    if args.seasonal_mode == "deterministic":
        ...
        if len(m_season_prior) != args.period - 1:
            raise ValueError(f"--prior-m-season must have length {args.period - 1} (NEWEST-FIRST).")

    # ---------------- Priors for new DGEV PGAS NCP ----------------
    priors = Priors(
        ...
        m_season=(m_season_prior if args.seasonal_mode == "deterministic" else None),
        s_season=float(args.prior_s_season),
    )
    # Log-normal hyperparameters for process SDs
    ...
    # Initial m0 / P0 for dynamic blocks
    ...
    if args.seasonal_mode == "dynamic":
        init_m0_season = np.zeros(args.period - 1, float)
        init_P0_season = 0.5
    else:
        init_m0_season = None
        init_P0_season = 0.0

    # Deterministic seasonal vector (p entries, NEWEST-FIRST, sum-to-zero)
    seasonal_init_pminus1 = (
        np.asarray(m_season_prior, float)
        if (args.seasonal_mode == "deterministic" and m_season_prior is not None)
    ...
    if args.init_sigma is not None:
        ...
        # robust-ish: sample sd, lower bounded
    ...
    sampler = DGEVParticleGibbs(
        ...
        # dynamic x0 priors
        m0_level_init=init_m0_level,
        P0_level_init=init_P0_level,
        m0_trend_init=init_m0_trend,
        P0_trend_init=init_P0_trend,
        m0_season_init=(init_m0_season if args.seasonal_mode == "dynamic" else None),
        P0_season_init=(init_P0_season if args.seasonal_mode == "dynamic" else 0.0),
        # observation inits
        sigma_init=init_sigma,
        xi_init=init_xi,
        # process SD inits (small to reflect smoothness)
        s_alpha_init=1e-2,
        s_beta_init=1e-3,
        s_gamma_init=1e-3,
        # deterministic parameters (used if corresponding modes are deterministic)
        level_value_init=float(args.level_init),
        slope_value_init=float(args.slope_init),
    ...
    # ---------------- Output directories ----------------
    tag = f"UccleSeasonalPGAS-{args.series}_{args.level_mode}-{args.trend_mode}-{args.seasonal_mode}"
    ...
    # ---------------- Save posterior + metadata via sampler.save_posterior ----------------
